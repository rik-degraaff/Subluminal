...
{
	var dialog = ShowPromptWithTextAreaDialog( '回報評論', '', null, null, 1000 );
	var explanation = $J('<div/>', { 'class': 'user_review_report_dialog_explanation' } );
	explanation.html( '請說明您回報此評論違反 Steam 服務條款或線上行為守則的原因。此動作無法取消。' );
...
{
	var dialog = ShowConfirmDialog( '清除開發人員的標註說明', '這篇評論已被開發人員標註。您確定要清除標註狀態嗎？' );
	dialog.done( function() {
...
function UserReview_SetDeveloperResponse( recommendationID, recommendation, baseURL, callback )
{
	...
	var explanation = $J('<div/>', { 'class': 'user_review_report_dialog_explanation' } );
	explanation.html( '您能選擇以官方的身分回覆這篇評論，任何能看見此評論的人將能看見此回覆，且此回覆將會標示為來自開發人員。' );
...
function UserReview_ShowReportsDialog( recommendationID, baseURL )
{
	...
	.done( function( results ) {
		if ( results.success == 1 )
		{
			...
			var dialog = ShowAlertDialog( '清除報告', container );
		}
	} );
}
...
{
	var dialog = ShowConfirmDialog( '清除報告', '您確定要清除所有的報告嗎？請注意這將無法回復！' );
	dialog.done( function() {
