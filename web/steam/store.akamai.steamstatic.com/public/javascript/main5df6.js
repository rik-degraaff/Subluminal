		...
		$J.get( url, {ll: 'german'} ).done( function( data ) {
			...
			if ( rgYourPopularTags.length > 0 )
			{
				...
				for( var i = 0; i < rgYourPopularTags.length && i < 5; i++ )
				{
					...
					var $Link = $J('<a/>', {'class': 'popup_menu_item', 'href': 'http://store.steampowered.com/tag/de/' + encodeURIComponent( tag.name ) });
					$Link.text( tag.name );
					$Element.append( $Link );
				}
			}
...
function FollowCuratorWithCallback( clanID, bFollow, onComplete )
{
	...
	$J.post(
		...
		{
			ShowAlertDialog( 'Fehler', 'Beim Folgen des Steam-Kurators ist ein Fehler aufgetreten.' );
		}
	);
	return false;
}
...
function IgnoreCuratorWithCallback( clanID, bIgnore, onComplete )
{
	...
	if ( !bHaveUser )
	{
		ShowAlertDialog("Bitte anmelden", "Sie m\u00fcssen angemeldet sein, um einen Steam-Kurator zu ignorieren.");
		return;
	}
	...
	$J.post(
		...
		{
			ShowAlertDialog( 'Fehler', 'Beim Versuch diesen Steam-Kurator zu ignorieren, ist ein Fehler aufgetreten.' );
		}
	);
	return false;
}
