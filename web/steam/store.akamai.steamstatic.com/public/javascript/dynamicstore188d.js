...
GDynamicStore = {
	...
	CapsuleSettingsMenu: function( elSource )
	{
		...
		if( strAppIDs )
		{
			...
			if (!bOnWishlist)
			{
				...
				var strText = "\u0e40\u0e1e\u0e34\u0e48\u0e21\u0e43\u0e19\u0e2a\u0e34\u0e48\u0e07\u0e17\u0e35\u0e48\u0e04\u0e38\u0e13\u0e2d\u0e22\u0e32\u0e01\u0e44\u0e14\u0e49";
			...
			var $elPreferences = $J ( '<a/>' ).attr('href', 'https://store.steampowered.com/account/preferences' ).text ( "\u0e01\u0e32\u0e23\u0e1b\u0e23\u0e31\u0e1a\u0e41\u0e15\u0e48\u0e07" ).addClass( 'option' );
	...
	BuildBundleDescription: function( Bundle, $Description )
	{
		...
		else if ( Bundle.m_cUserItemsInBundle < Bundle.m_cTotalItemsInBundle )
		{
			...
			$Description.append( '<div>ซื้อชุดรวมนี้เพื่อประหยัด %1$s%% จาก %2$s ผลิตภัณฑ์ที่คุณยังไม่มี!</div>'.replace( '%1$s', Bundle.m_nDiscountPct ).replace( '%2$s', Bundle.m_cUserItemsInBundle ).replace( '%%', '%' ) );
		...
		{
			$Description.html( '<div>ซื้อชุดรวมนี้เพื่อประหยัด %1$s%% จากทั้งหมด %2$s ผลิตภัณฑ์!</div>'.replace( '%1$s', Bundle.m_nDiscountPct ).replace( '%2$s', Bundle.m_cTotalItemsInBundle ).replace( '%%', '%' ) );
		}
	...
	PopulateRecommendedTagList: function()
	{
		...
		for( var i = 0; i < GDynamicStore.s_rgRecommendedTags.length && i < 5; i++ )
		{
			var tag = GDynamicStore.s_rgRecommendedTags[i];
							var $Link = $J('<a/>', {'class': 'popup_menu_item', 'href': 'http://store.steampowered.com/tags/th/' + encodeURIComponent( tag.name ) });
						$Link.text( tag.name );
			$Element.append( $Link );
		}
	},
