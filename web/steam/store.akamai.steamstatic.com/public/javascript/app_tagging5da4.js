...
function InitAppTagModal( appid, rgAppTags, rgUserTags, strTagLinkSNR, strYourTagSNR, bShowBanOption )
{
	...
	var fnSetCheckboxTooltip = function( $Checkbox )
	{
		if ( $Checkbox.hasClass('checked') )
			$Checkbox.data('store-tooltip', 'このタグを適用しました。クリックで削除します。' );
		else
			$Checkbox.data('store-tooltip', 'クリックでこのタグを適用' );
	};

	var fnSetReportTooltip = function( $ReportFlag )
	{
		if ( $ReportFlag.hasClass('reported') )
			$ReportFlag.data('store-tooltip', 'このタグがこの製品に誤って適用されていると報告しました。もう一度クリックすると、報告を削除します。' );
		else
			$ReportFlag.data('store-tooltip', 'このタグがこの製品に誤って適用されていると報告するにはクリックしてください。' );
	};
	...
	var fnMakeTag = function( tagid, tag, checked, bPopular, bReported, bBrowsable )
	{
		...
		if ( bPopular )
		{
			...
			if ( $YourTags.length )
			{
				...
				if ( bShowBanOption )
				{
					...
					$Ban.data('store-tooltip', 'この製品からこのタグを削除するにはここをクリック。このオプションはデベロッパー及びSteam スタッフのみが使えます。' );
					BindStoreTooltip( $Ban );
					$Tag.append( $Ban );

					$Ban.click( function() {
						ShowConfirmDialog( 'タグを禁止する', '本当にこの製品から "%s" タグを取り除いても良いですか？'.replace( /%s/, tag )).done( function()
						{
		...
		else
		{
			...
			$Link.data('store-tooltip', '"%s" とタグが付けられているアイテムがまだ十分にありません。'.replace( /%s/, $Link.html() ) );
	...
	{
		var $Tag = $J('<div/>', {'class': 'app_tag previous_tag', 'data-tagid': tagid, 'data-store-tooltip': 'クリックでこのタグを適用' }).text( tag ).click( function() { fnApplyTag( tag ) } );
	...
	var fnBuildTagDisplay = function( Modal )
	{
		...
		if ( rgYourPopularTags === null )
		{
			...
			if ( !bYourPopularTagsRequested )
			{
				...
				$J.get( 'http://store.steampowered.com/tagdata/populartags/japanese').done( function ( data ) {
		...
		if ( rgUserTags.length )
		{
			...
			for( var i = 0; i < rgUserTags.length; i++ )
			{
				if ( !rgUserTags[i].is_reported &&  !rgUserTags[i].browseable )
				{
					...
					$AppTag.data('store-tooltip', '"%s" とタグが付けられているアイテムがまだ十分にありません。'.replace( /%s/, $AppTag.html() ) );
	...
	window.ShowAppTagBanModal = function() {
		...
		$BanModal.find( '.app_tag_report').click( function() {
			...
			ShowConfirmDialog( 'タグの禁止を解除',
				'本当に "%s" タグの禁止を解除しますか？'.replace( /%s/, tag ),
				'タグの禁止を解除')
				.done( function() {
					var rgParams = {
						appid: appid,
