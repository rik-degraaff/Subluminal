...
HighlightPlayer.prototype.LoadMovie = function( $Container, bUserAction )
{
	...
	else
	{
		...
		var strMessage = '이 페이지를 보려면 최신 Flash 플러그인을 <a href="http://www.adobe.com/go/getflashplayer" target="_blank">설치</a>해야 합니다.';
...
HighlightPlayer.prototype.ShowScreenshotPopup = function( screenshotid )
{
	...
	var fnUpdateFooter = function()
	{
		...
		$ScreenshotCount.text( '%2$s 개 중 %1$s 개의 스크린샷'.replace( /%1\$s/, iCurIndex + 1 ).replace( /%2\$s/, rgScreenshotIDs.length ) );
	};
	...
	$.fn.videoControls = function( options ) {
		...
		var overlaySrc = '<div class="html5_video_overlay">' +
				...
				'<div class="control_container">' +
					...
					'<div class="autoplay_label">비디오 자동재생</div>' +
				'</div>' +
