...
HighlightPlayer.prototype.LoadMovie = function( $Container, bUserAction )
{
	...
	else
	{
		...
		var strMessage = 'Wystąpił błąd podczas odtwarzania tego filmu. Upewnij się, że posiadasz najnowszą wersję przeglądarki.';
...
HighlightPlayer.prototype.ShowScreenshotPopup = function( screenshotid )
{
	...
	var $Title = $J('<a/>' );
	if ( Steam.BIsUserInSteamClient() )
		$Title.text( 'Pokaż wersję w pełnym rozmiarze w przeglądarce' );
	else
		$Title.text( 'Pobierz wersję w pełnym rozmiarze' );
	...
	var $BtnPrev = $J('<div/>', {'class': 'btnv6_blue_hoverfade btn_medium previous'}).append( $J('<span/>').text( 'Poprzedni' ) );
	var $BtnNext = $J('<div/>', {'class': 'btnv6_blue_hoverfade btn_medium next'}).append( $J('<span/>').text( 'Następny' ) );
	...
	var fnUpdateFooter = function()
	{
		...
		$ScreenshotCount.text( '%1$s z %2$s zrzutów ekranu'.replace( /%1\$s/, iCurIndex + 1 ).replace( /%2\$s/, rgScreenshotIDs.length ) );
	};
	...
	$.fn.videoControls = function( options ) {
		...
		var overlaySrc = '<div class="html5_video_overlay">' +
				...
				'<div class="control_container">' +
					...
					'<div class="autoplay_label">Autoodtwarzanie filmów</div>' +
				'</div>' +
