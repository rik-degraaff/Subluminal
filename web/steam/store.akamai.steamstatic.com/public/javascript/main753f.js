...
function GameHover( elem, event, divHover, rgHoverData )
{
	...
	else if ( ( !$Hover.is(':visible') || g_HoverState.target != $Elem[0] ) && !oElemState.timer )
	{
		...
		if ( !$HoverData.length && !oElemState.bAjaxRequestMade )
		{
			...
			{
				// is cc needed?
				rgAjaxParams = { /*cc: rgHoverData['cc'],*/ l: 'portuguese' };
			}
			if ( rgHoverData['v6'] )
			{
		...
		$J.get( url, {ll: 'portuguese'} ).done( function( data ) {
			...
			if ( rgYourPopularTags.length > 0 )
			{
				...
				for( var i = 0; i < rgYourPopularTags.length && i < 5; i++ )
				{
					...
					var $Link = $J('<a/>', {'class': 'popup_menu_item', 'href': 'http://store.steampowered.com/tag/pt/' + encodeURIComponent( tag.name ) });
					$Link.text( tag.name );
					$Element.append( $Link );
				}
			}
...
function AddFreeLicense( subid, strDisplayName )
{
	...
	$J.post( 'http://store.steampowered.com/checkout/addfreelicense/' + subid, { ajax: true, sessionid: g_sessionID }).done( function() {
		ShowAlertDialog(
			strDisplayName,
			'%s foi adicionado à tua conta e encontra-se disponível na tua Biblioteca Steam.'.replace( /%s/, strDisplayName )
		).done( function() {
			window.location.reload();
		});
		if ( typeof GDynamicStore != 'undefined' )
...
function FollowCuratorWithCallback( clanID, bFollow, onComplete )
{
	...
	$J.post(
		...
		{
			ShowAlertDialog( 'Erro', 'Ocorreu um problema ao tentar seguir o Curador Steam.' );
		}
	);
	return false;
}
...
function IgnoreCuratorWithCallback( clanID, bIgnore, onComplete )
{
	...
	$J.post(
		...
		{
			ShowAlertDialog( 'Erro', 'Ocorreu um problema ao tentar ignorar o Curador Steam.' );
		}
	);
	return false;
}
