...
GDynamicStore = {
		...
		$DynamicElements.each( function() {
			...
			if ( !$El.hasClass('ds_no_flags') )
			{
				...
				if ( bOwned )
				{
					...
					$El.append( '<div class="ds_flag ds_owned_flag">ライブラリ内&nbsp;&nbsp;</div>');
				}
				else if ( bWanted )
				{
					$El.addClass( 'ds_flagged ds_wishlist' );
					$El.append( '<div class="ds_flag ds_wishlist_flag">ウィッシュリストに追加済み&nbsp;&nbsp;</div>');
				}

				if ( bInCart )
				{
					$El.addClass( 'ds_flagged ds_incart' );
					$El.append( '<div class="ds_flag ds_incart_flag">カートの中&nbsp;&nbsp;</div>');
				}
		...
		if( strAppIDs )
		{
			...
			if (!bOnWishlist)
			{
				var fnClick = function ( event )
				{
					...
					$elSource.append( '<div class="ds_flag ds_wishlist_flag">ウィッシュリストに追加済み&nbsp;&nbsp;</div>');
			...
			var $elPreferences = $J ( '<a/>' ).attr('href', 'https://store.steampowered.com/account/preferences' ).text ( "\u500b\u4eba\u8a2d\u5b9a" ).addClass( 'option' );
	...
	UpdateDynamicBundleElements: function( Bundle, $El )
	{
		...
		if ( !Bundle.m_bIsCommercial && ( !Bundle.m_rgBundleItems.length || ( Bundle.m_bMustPurchaseAsSet && Bundle.m_cUserItemsInBundle < Bundle.m_cTotalItemsInBundle ) ) )
		{
			...
			var strTooltip = 'このバンドルに含まれるアイテム全てを所有しているのであなたのアカウント上で購入することはできません。';

			if ( Bundle.m_bMustPurchaseAsSet && !Bundle.m_bRestrictGifting )
			{
				strTooltip = '%s アイテム全てを同時に購入した場合に限り割引が適用されます。フレンドへのギフトとしてもバンドルを購入できます。'.replace(/%s/, Bundle.m_cTotalItemsInBundle);
				$CartBtn.find('span').text( 'ギフトとして購入' );
			}
			else
			{
		...
		else if ( !Bundle.m_bMustPurchaseAsSet )
		{
			...
			if ( !Bundle.m_bContainsDiscountedPackage )
			{
				...
				$DiscountBlocks.find('.discount_final_price' ).addClass('your_price' ).empty().append($J('<div/>', {'class': 'your_price_label'} ).text('あなたの価格:'), $J('<div/>' ).text( strFormattedFinalPrice ) );
			}
			else
			{
		...
		if ( $El.is( '.package_totals_area' ) && !Bundle.m_bMustPurchaseAsSet )
		{
			...
			else
			{
				...
				{
					$El.find('.bundle_final_package_price_desc' ).text( '未所有の %s アイテムの単体価格:'.replace( '%s', Bundle.m_cUserItemsInBundle ) );
				}
	...
	BuildBundleDescription: function( Bundle, $Description )
	{
		if ( Bundle.m_cUserItemsInBundle == 0 )
		{
			...
			$Description.html( '<span class="collectionComplete">コレクションをコンプリートしよう！</span>このコレクションの %2$s アイテム中 %1$s アイテムをライブラリ内に所有済みです。'.replace( '%1$s', Bundle.m_cTotalItemsInBundle ).replace( '%2$s', Bundle.m_cTotalItemsInBundle ) );
		}
		else if ( Bundle.m_cUserItemsInBundle < Bundle.m_cTotalItemsInBundle )
		{
			// own some but not all.
			$Description.html( '<div>このバンドルの %2$s アイテム中 %1$s アイテムをライブラリ内に所有済みです。</div>'.replace( '%1$s', Bundle.m_cTotalItemsInBundle - Bundle.m_cUserItemsInBundle ).replace( '%2$s', Bundle.m_cTotalItemsInBundle ) );
			$Description.append( '<div>このバンドルを購入すると、未所有の %2$s アイテムが %1$s%% オフ！</div>'.replace( '%1$s', Bundle.m_nDiscountPct ).replace( '%2$s', Bundle.m_cUserItemsInBundle ).replace( '%%', '%' ) );

			// add "complete the set" flag
			$Description.parents('.dynamic_bundle_description' ).append( $J('<div/>', {'class': 'ds_flag ds_completetheset'} ).text('コレクションをコンプリートしよう！') );
		}
		else
		{
			$Description.html( '<div>このバンドルを購入すると、%2$s アイテム全部が %1$s%% オフ！</div>'.replace( '%1$s', Bundle.m_nDiscountPct ).replace( '%2$s', Bundle.m_cTotalItemsInBundle ).replace( '%%', '%' ) );
		}
	...
	PopulateRecommendedTagList: function()
	{
		...
		for( var i = 0; i < GDynamicStore.s_rgRecommendedTags.length && i < 5; i++ )
		{
			var tag = GDynamicStore.s_rgRecommendedTags[i];
							var $Link = $J('<a/>', {'class': 'popup_menu_item', 'href': 'http://store.steampowered.com/tags/ja/' + encodeURIComponent( tag.name ) });
						$Link.text( tag.name );
			$Element.append( $Link );
		}
	},
	...
	DisplayBundleSimulator: function( unBundleID )
	{
		...
		$Form.append( $J('<h2/>').text('「所有」するためにはアイテムをチェック') );
		$Form.append( $J('<div/>', {'class': 'bundle_simulator_secondary' } ).append(
			$J('<a/>', {'href': 'javascript:void(0);'} ).click( function() { $Form.find('input').prop('checked', true ); } ).text( "すべて選択" ),
			' - ',
			$J('<a/>', {'href': 'javascript:void(0);'} ).click( function() { $Form.find('input').prop('checked', false ); } ).text( "何も選択しない" )
		) );
		...
		Modal = ShowConfirmDialog( '', $Form, 'ディスプレイアップデート' ).done( function() {
