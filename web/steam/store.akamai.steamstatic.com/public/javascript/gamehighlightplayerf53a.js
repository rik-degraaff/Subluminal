...
HighlightPlayer.prototype.LoadMovie = function( $Container, bUserAction )
{
	...
	else
	{
		...
		var strMessage = 'Det oppstod en feil under avspilling av denne videoen. Sørg for at nettleseren din er oppdatert.';
...
HighlightPlayer.prototype.ShowScreenshotPopup = function( screenshotid )
{
	...
	var $Title = $J('<a/>' );
	if ( Steam.BIsUserInSteamClient() )
		$Title.text( 'Vis i nettleser i full størrelse' );
	else
		$Title.text( 'Last ned i full størrelse' );
	...
	var $BtnNext = $J('<div/>', {'class': 'btnv6_blue_hoverfade btn_medium next'}).append( $J('<span/>').text( 'Neste' ) );
	...
	var fnUpdateFooter = function()
	{
		...
		$ScreenshotCount.text( '%1$s av %2$s skjermbilder'.replace( /%1\$s/, iCurIndex + 1 ).replace( /%2\$s/, rgScreenshotIDs.length ) );
	};
	...
	$.fn.videoControls = function( options ) {
		...
		var overlaySrc = '<div class="html5_video_overlay">' +
				...
				'<div class="control_container">' +
					...
					'<div class="autoplay_label">Auto-avspill videoer</div>' +
				'</div>' +
