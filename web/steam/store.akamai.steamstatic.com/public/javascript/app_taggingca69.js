...
function InitAppTagModal( appid, rgAppTags, rgUserTags, strTagLinkSNR, strYourTagSNR, bShowBanOption )
{
	...
	var fnSetCheckboxTooltip = function( $Checkbox )
	{
		if ( $Checkbox.hasClass('checked') )
			$Checkbox.data('store-tooltip', 'Приложихте този таг. Кликнете, за да го премахнете.' );
		else
			$Checkbox.data('store-tooltip', 'Кликнете тук, за да приложите този таг.' );
	};

	var fnSetReportTooltip = function( $ReportFlag )
	{
		if ( $ReportFlag.hasClass('reported') )
			$ReportFlag.data('store-tooltip', 'Докладвахте този таг като неправилно приложен към този продукт. Кликнете отново, за да премахнете доклада.' );
		else
			$ReportFlag.data('store-tooltip', 'Кликнете, за да докладвате този таг като неправилно приложен към този продукт.' );
	};
	...
	var fnMakeTag = function( tagid, tag, checked, bPopular, bReported, bBrowsable )
	{
		...
		if ( bPopular )
		{
			...
			if ( $YourTags.length )
			{
				...
				if ( bShowBanOption )
				{
					...
					$Ban.data('store-tooltip', 'Кликнете тук, за да премахнете този таг от съответния продукт. Тази опция е ограничена до разработчиците и Steam персонала.' );
					BindStoreTooltip( $Ban );
					$Tag.append( $Ban );

					$Ban.click( function() {
						ShowConfirmDialog( 'Забрана на тага', 'Сигурни ли сте, че искате да премахнете тага „%s“ от този продукт?'.replace( /%s/, tag )).done( function()
						{
		...
		else
		{
			...
			$Link.data('store-tooltip', 'Все още няма достатъчно артикули, отбелязани с „%s“'.replace( /%s/, $Link.html() ) );
	...
	{
		var $Tag = $J('<div/>', {'class': 'app_tag previous_tag', 'data-tagid': tagid, 'data-store-tooltip': 'Кликнете тук, за да приложите този таг.' }).text( tag ).click( function() { fnApplyTag( tag ) } );
	...
	var fnBuildTagDisplay = function( Modal )
	{
		...
		if ( rgYourPopularTags === null )
		{
			...
			if ( !bYourPopularTagsRequested )
			{
				...
				$J.get( 'http://store.steampowered.com/tagdata/populartags/bulgarian').done( function ( data ) {
		...
		else
		{
			...
			{
				$YourTags.append( $J('<div/>', {'class': 'app_tag_control no_tags_yet'}).text('Все още няма въведени тагове' ) );
			}
		...
		if ( rgUserTags.length )
		{
			...
			for( var i = 0; i < rgUserTags.length; i++ )
			{
				if ( !rgUserTags[i].is_reported &&  !rgUserTags[i].browseable )
				{
					...
					$AppTag.data('store-tooltip', 'Все още няма достатъчно артикули, отбелязани с „%s“'.replace( /%s/, $AppTag.html() ) );
	...
	var fnApplyTag = function( tag, withdraw, bPopularClick, eReportType, unTagID )
	{
		...
		$J.post( 'http://store.steampowered.com/tagdata/tagapp', rgParams ).done( function( data ) {
			...
			{
				ShowAlertDialog( 'Докладване на таг', 'Благодарим Ви за доклада. Таговете, които достигнат определен праг от доклади, ще бъдат премахнати от този продукт.');
			}
			...
			ShowAlertDialog( 'Преглед и редактиране на таговете за този продукт', 'There was a problem adding your tag.' );
		});
	};
	...
	window.ShowAppTagModal = function( appid )
	{
		...
		$AppTagModal.show();
		var Modal = ShowAlertDialog( 'Преглед и редактиране на таговете за този продукт', $AppTagModal, 'Затваряне' );
	...
	var fnDisplayTag = function( unTagID, strTagName ) {
		if ( !rgTagResults[unTagID] )
		{
			...
			$J.get( 'http://store.steampowered.com/tagdata/gettaggames/' + strTagLanguage + '/' + unTagID + '/', rgParams ).done( function ( html ) {
				...
				$GamesElement.find( '.browse_tag_loading').text( 'Съжаляваме, но имаше проблем при зареждането на артикулите. Моля, опитайте отново по-късно.' );
			});
...
function InitBannedTagModal( appid, $BanModal )
{
	...
	window.ShowAppTagBanModal = function() {
		$BanModal.show();
		ShowAlertDialog( 'Забранени тагове', $BanModal).always( function() {
			if ( bBansChanged )
				window.location.reload();
		} );
		...
		$BanModal.find( '.app_tag_report').click( function() {
			...
			ShowConfirmDialog( 'Премахване на забрана за таг',
				'Сигурни ли сте, че искате да премахнете забраната на тага „%s“?'.replace( /%s/, tag ),
				'Премахване на забрана за таг')
				.done( function() {
					var rgParams = {
						appid: appid,
						sessionid: g_sessionID,
					...
					$J.post( 'http://store.steampowered.com/tagdata/tagapp', rgParams ).done( function( data ) {
						...
						ShowAlertDialog( 'Премахване на забрана за таг', 'Имаше проблем при забраната или премахването ѝ за този таг. Моля, опитайте пак по-късно.' );
					});
					bBansChanged = true;
				});
		});
