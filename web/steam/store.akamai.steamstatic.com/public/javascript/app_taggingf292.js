...
function InitAppTagModal( appid, rgAppTags, rgUserTags, strTagLinkSNR, strYourTagSNR, bShowBanOption )
{
	...
	var fnSetCheckboxTooltip = function( $Checkbox )
	{
		if ( $Checkbox.hasClass('checked') )
			$Checkbox.data('store-tooltip', 'Ten tag został dodany przez ciebie. Kliknij, aby go usunąć.' );
		else
			$Checkbox.data('store-tooltip', 'Kliknij tutaj, aby użyć ten tag.' );
	};

	var fnSetReportTooltip = function( $ReportFlag )
	{
		if ( $ReportFlag.hasClass('reported') )
			$ReportFlag.data('store-tooltip', 'Tag został zgłoszony jako nieprawidłowy dla tego produktu. Kliknij ponownie, aby usunąć zgłoszenie.' );
		else
			$ReportFlag.data('store-tooltip', 'Kliknij, aby zgłosić ten tag jako nieprawidłowy dla tego produktu.' );
	};
	...
	var fnMakeTag = function( tagid, tag, checked, bPopular, bReported, bBrowsable )
	{
		...
		if ( bPopular )
		{
			...
			if ( $YourTags.length )
			{
				...
				if ( bShowBanOption )
				{
					...
					$Ban.data('store-tooltip', 'Kliknij tutaj, aby usunąć ten tag ze swojego produktu. Ta opcja jest dostępna dla twórców oraz załogi Steam.' );
					BindStoreTooltip( $Ban );
					$Tag.append( $Ban );

					$Ban.click( function() {
						ShowConfirmDialog( 'Zbanuj tag', 'Czy na pewno chcesz usunąć tag „%s” z tego produktu?'.replace( /%s/, tag )).done( function()
						{
		...
		else
		{
			...
			$Link.data('store-tooltip', 'Niewystarczająca ilość przedmiotów oznaczonych tagiem „%s”'.replace( /%s/, $Link.html() ) );
	...
	{
		var $Tag = $J('<div/>', {'class': 'app_tag previous_tag', 'data-tagid': tagid, 'data-store-tooltip': 'Kliknij tutaj, aby użyć ten tag.' }).text( tag ).click( function() { fnApplyTag( tag ) } );
	...
	var fnBuildTagDisplay = function( Modal )
	{
		...
		if ( rgYourPopularTags === null )
		{
			...
			if ( !bYourPopularTagsRequested )
			{
				...
				$J.get( 'http://store.steampowered.com/tagdata/populartags/polish').done( function ( data ) {
		...
		if ( rgUserTags.length )
		{
			...
			for( var i = 0; i < rgUserTags.length; i++ )
			{
				if ( !rgUserTags[i].is_reported &&  !rgUserTags[i].browseable )
				{
					...
					$AppTag.data('store-tooltip', 'Niewystarczająca ilość przedmiotów oznaczonych tagiem „%s”'.replace( /%s/, $AppTag.html() ) );
	...
	window.ShowAppTagBanModal = function() {
		...
		$BanModal.find( '.app_tag_report').click( function() {
			...
			ShowConfirmDialog( 'Odblokuj tag',
				'Czy na pewno chcesz odblokować tag „%s”?'.replace( /%s/, tag ),
				'Odblokuj tag')
				.done( function() {
					var rgParams = {
						appid: appid,
