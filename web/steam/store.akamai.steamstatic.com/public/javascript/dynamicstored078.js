...
GDynamicStore = {
	...
	CapsuleSettingsMenu: function( elSource )
	{
		...
		if( strAppIDs )
		{
			...
			var $elPreferences = $J ( '<a/>' ).attr('href', 'https://store.steampowered.com/account/preferences' ).text ( "\u03a0\u03c1\u03bf\u03c4\u03b9\u03bc\u03ae\u03c3\u03b5\u03b9\u03c2" ).addClass( 'option' );
	...
	UpdateDynamicBundleElements: function( Bundle, $El )
	{
		...
		if ( !Bundle.m_bIsCommercial && ( !Bundle.m_rgBundleItems.length || ( Bundle.m_bMustPurchaseAsSet && Bundle.m_cUserItemsInBundle < Bundle.m_cTotalItemsInBundle ) ) )
		{
			...
			var strTooltip = 'Αυτό το πακέτο δεν είναι διαθέσιμο για αγορά στο λογαριασμό σας επειδή κατέχετε ήδη όλα τα συμπεριλαμβανόμενα αντικείμενα.';

			if ( Bundle.m_bMustPurchaseAsSet && !Bundle.m_bRestrictGifting )
			{
				strTooltip = 'Αυτή η προσφορά είναι διαθέσιμη μόνο όταν αγοράζετε όλα τα %s αντικείμενα μαζί. Μπορείτε όμως να αγοράσετε το πακέτο ως δώρο για έναν φίλο.'.replace(/%s/, Bundle.m_cTotalItemsInBundle);
		...
		if ( $El.is( '.package_totals_area' ) && !Bundle.m_bMustPurchaseAsSet )
		{
			...
			else
			{
				...
				{
					$El.find('.bundle_final_package_price_desc' ).text( 'Ατομικές τιμές των %s αντικειμένων που δεν κατέχετε ήδη:'.replace( '%s', Bundle.m_cUserItemsInBundle ) );
				}
	...
	BuildBundleDescription: function( Bundle, $Description )
	{
		if ( Bundle.m_cUserItemsInBundle == 0 )
		{
			...
			$Description.html( '<span class="collectionComplete">Ολοκληρωμένη συλλογή!</span> %1$s από %2$s αντικείμενα από αυτή τη συλλογή είναι ήδη στη συλλογή σας.'.replace( '%1$s', Bundle.m_cTotalItemsInBundle ).replace( '%2$s', Bundle.m_cTotalItemsInBundle ) );
		}
		else if ( Bundle.m_cUserItemsInBundle < Bundle.m_cTotalItemsInBundle )
		{
			// own some but not all.
			$Description.html( '<div>%1$s από %2$s αντικείμενα από αυτό το πακέτο είναι ήδη στη συλλογή σας.</div>'.replace( '%1$s', Bundle.m_cTotalItemsInBundle - Bundle.m_cUserItemsInBundle ).replace( '%2$s', Bundle.m_cTotalItemsInBundle ) );
			$Description.append( '<div>Αγοράστε αυτό το πακέτο για να εξοικονομήσετε %1$s%% από τα %2$s αντικείμενα που δεν κατέχετε ακόμη!</div>'.replace( '%1$s', Bundle.m_nDiscountPct ).replace( '%2$s', Bundle.m_cUserItemsInBundle ).replace( '%%', '%' ) );
		...
		{
			$Description.html( '<div>Αγόραστε αυτό το πακέτο για να εξοικονομήσετε %1$s%% από όλα τα %2$s αντικείμενα!</div>'.replace( '%1$s', Bundle.m_nDiscountPct ).replace( '%2$s', Bundle.m_cTotalItemsInBundle ).replace( '%%', '%' ) );
		}
	...
	PopulateRecommendedTagList: function()
	{
		...
		for( var i = 0; i < GDynamicStore.s_rgRecommendedTags.length && i < 5; i++ )
		{
			var tag = GDynamicStore.s_rgRecommendedTags[i];
							var $Link = $J('<a/>', {'class': 'popup_menu_item', 'href': 'http://store.steampowered.com/tags/el/' + encodeURIComponent( tag.name ) });
						$Link.text( tag.name );
			$Element.append( $Link );
		}
	},
	...
	DisplayBundleSimulator: function( unBundleID )
	{
		...
		$Form.append( $J('<h2/>').text('Επιλέξτε τα αντικείμενα που «κατέχετε»') );
...
function ShowHowDoDiscoveryQueuesWorkDialog()
{
	$J.get(
		...
		function( data )
		{
			ShowAlertDialog( 'Πώς δουλεύει;', data );
		}
	);
}
