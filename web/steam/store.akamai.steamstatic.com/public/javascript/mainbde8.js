...
function GameHover( elem, event, divHover, rgHoverData )
{
	...
	else if ( ( !$Hover.is(':visible') || g_HoverState.target != $Elem[0] ) && !oElemState.timer )
	{
		...
		if ( !$HoverData.length && !oElemState.bAjaxRequestMade )
		{
			...
			{
				// is cc needed?
				rgAjaxParams = { /*cc: rgHoverData['cc'],*/ l: 'turkish' };
			}
			if ( rgHoverData['v6'] )
			{
		...
		.done( function( data ) {
			...
			if ( data && data.success ) {
				...
				if ( data.saleTaskCompleted ) {
					NewStickerPackModal( 'İstek listenize ekleyin' );
				}
			}
			else {
	...
	$J.post( url, {sessionid: g_sessionID, appid: appid} )
		.done( function( data ) {
			if ( data && data.success )
			{
				...
				$J('span',button).text("\u0130stek Listesinde");
			}
			else
			{
				$J('span',button).text( "Hay aksi, affedersiniz!" );
			}
...
function HideRecommendation( type, itemid, divBtn, elemContainer )
{
	...
	new Ajax.Request( 'http://store.steampowered.com/recommended/ignorerecommendation/', {
		...
		onSuccess: function( transport ) {
			...
			{
				if ( $(elemContainer) )
					$(elemContainer).update( '<div class="recommendation_ignored">Tamam, bir dahaki sefere farklı bir tavsiyede bulunacağız</div>' );
			}
			else
			{
				$(divBtn).show();
...
function EnsureStoreMenuTagsLoaded( strId )
{
	...
	if ( !$Element.data('tags-loaded') )
	{
		...
		$J.get( url, {ll: 'turkish'} ).done( function( data ) {
			...
			if ( rgYourPopularTags.length > 0 )
			{
				...
				for( var i = 0; i < rgYourPopularTags.length && i < 5; i++ )
				{
					...
					var $Link = $J('<a/>', {'class': 'popup_menu_item', 'href': 'http://store.steampowered.com/tag/tr/' + encodeURIComponent( tag.name ) });
					$Link.text( tag.name );
					$Element.append( $Link );
				}
			}
...
function AddFreeLicense( subid, strDisplayName )
{
	...
	$J.post( 'http://store.steampowered.com/checkout/addfreelicense/' + subid, { ajax: true, sessionid: g_sessionID }).done( function() {
		ShowAlertDialog(
			strDisplayName,
			'%s hesabınıza eklendi. Artık Steam Kütüphanenizden erişilebilir.'.replace( /%s/, strDisplayName )
		).done( function() {
			window.location.reload();
		});
		if ( typeof GDynamicStore != 'undefined' )
		...
		if ( data && data.purchaseresultdetail == 9 )
			ShowAlertDialog( strDisplayName, 'Bu ürün Steam Kütüphanenizde zaten var.' );
		else
			ShowAlertDialog( strDisplayName, 'Bu ürünü hesabınıza yüklerken bir hata oluştu. Lütfen daha sonra tekrar deneyin.' );
	}).always( function () {
...
function ChangeLanguage( strTargetLanguage, bStayOnPage )
{
	...
	.done( function() {
		...
		Modal.Dismiss();
		ShowAlertDialog( 'Dili değiştir', 'Değişikliklerinizi kaydederken bir sorun oluştu. Lütfen daha sonra tekrar deneyiniz.' );
	});
}
...
function FollowCuratorWithCallback( clanID, bFollow, onComplete )
{
	...
	if ( !bHaveUser )
	{
		ShowAlertDialog("L\u00fctfen giri\u015f yap\u0131n", "Bir k\u00fcrat\u00f6r\u00fc takip edebilmek i\u00e7in giri\u015f yapmal\u0131s\u0131n\u0131z");
		return;
	}
	...
	$J.post(
		...
		{
			ShowAlertDialog( 'Hata', 'Steam Küratörünü takip etmeye çalışırken bir hata meydana geldi.' );
		}
	);
	return false;
}
...
function IgnoreCuratorWithCallback( clanID, bIgnore, onComplete )
{
	...
	if ( !bHaveUser )
	{
		ShowAlertDialog("L\u00fctfen giri\u015f yap\u0131n", "Bir Steam K\u00fcrat\u00f6r\u00fcn\u00fc yok sayabilmek i\u00e7in giri\u015f yapmal\u0131s\u0131n\u0131z");
		return;
	}
	...
	$J.post(
		...
		{
			ShowAlertDialog( 'Hata', 'Steam Küratörünü yok saymaya çalışırken bir hata meydana geldi.' );
		}
	);
	return false;
}
