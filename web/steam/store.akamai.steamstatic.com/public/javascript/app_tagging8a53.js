...
function InitAppTagModal( appid, rgAppTags, rgUserTags, strTagLinkSNR, strYourTagSNR, bShowBanOption )
{
	...
	var fnSetCheckboxTooltip = function( $Checkbox )
	{
		if ( $Checkbox.hasClass('checked') )
			$Checkbox.data('store-tooltip', 'Te adtad ezt a címkét. Ide kattintva eltávolíthatod.' );
		else
			$Checkbox.data('store-tooltip', 'Kattints ide e címke használatához.' );
	};

	var fnSetReportTooltip = function( $ReportFlag )
	{
		if ( $ReportFlag.hasClass('reported') )
			$ReportFlag.data('store-tooltip', 'Jelentetted, hogy ezt a címkét helytelenül rendelték hozzá ehhez a termékhez. Kattints újra a jelentés eltávolításához.' );
		else
			$ReportFlag.data('store-tooltip', 'Kattints ide az e termékhez helytelenül hozzárendelt címke jelentéséhez.' );
	};
	...
	var fnMakeTag = function( tagid, tag, checked, bPopular, bReported, bBrowsable )
	{
		...
		if ( bPopular )
		{
			...
			if ( $YourTags.length )
			{
				...
				if ( bShowBanOption )
				{
					...
					$Ban.data('store-tooltip', 'Kattints ide e címke eltávolításához e termékről. Ez a lehetőség csak a fejlesztőknek és a Steam alkalmazottjainak érhető el.' );
					BindStoreTooltip( $Ban );
					$Tag.append( $Ban );

					$Ban.click( function() {
						ShowConfirmDialog( 'Címke letiltása', 'Biztosan el akarod távolítani a(z) „%s” címkét erről a termékről?'.replace( /%s/, tag )).done( function()
						{
		...
		else
		{
			...
			$Link.data('store-tooltip', 'Nincs még elég „%s” címkével ellátott elem'.replace( /%s/, $Link.html() ) );
	...
	{
		var $Tag = $J('<div/>', {'class': 'app_tag previous_tag', 'data-tagid': tagid, 'data-store-tooltip': 'Kattints ide e címke használatához.' }).text( tag ).click( function() { fnApplyTag( tag ) } );
	...
	var fnBuildTagDisplay = function( Modal )
	{
		...
		if ( rgYourPopularTags === null )
		{
			...
			if ( !bYourPopularTagsRequested )
			{
				...
				$J.get( 'http://store.steampowered.com/tagdata/populartags/hungarian').done( function ( data ) {
		...
		else
		{
			...
			{
				$YourTags.append( $J('<div/>', {'class': 'app_tag_control no_tags_yet'}).text('Még nincs címke megadva' ) );
			}
		...
		if ( rgUserTags.length )
		{
			...
			for( var i = 0; i < rgUserTags.length; i++ )
			{
				if ( !rgUserTags[i].is_reported &&  !rgUserTags[i].browseable )
				{
					...
					$AppTag.data('store-tooltip', 'Nincs még elég „%s” címkével ellátott elem'.replace( /%s/, $AppTag.html() ) );
	...
	var fnApplyTag = function( tag, withdraw, bPopularClick, eReportType, unTagID )
	{
		...
		$J.post( 'http://store.steampowered.com/tagdata/tagapp', rgParams ).done( function( data ) {
			...
			{
				ShowAlertDialog( 'Címke jelentése', 'Köszönjük jelentésed. Az egy bizonyos küszöbértéket elérő számban jelentett címkék eltávolításra kerülnek majd a termékről.');
			}
			...
			ShowAlertDialog( 'E termék címkéinek megnézése és szerkesztése', 'There was a problem adding your tag.' );
		});
	};
	...
	window.ShowAppTagModal = function( appid )
	{
		...
		$AppTagModal.show();
		var Modal = ShowAlertDialog( 'E termék címkéinek megnézése és szerkesztése', $AppTagModal, 'Bezárás' );
	...
	var fnDisplayTag = function( unTagID, strTagName ) {
		if ( !rgTagResults[unTagID] )
		{
			...
			$J.get( 'http://store.steampowered.com/tagdata/gettaggames/' + strTagLanguage + '/' + unTagID + '/', rgParams ).done( function ( html ) {
				...
				$GamesElement.find( '.browse_tag_loading').text( 'Probléma adódott az elemek betöltése közben. Próbáld újra később.' );
			});
...
function InitBannedTagModal( appid, $BanModal )
{
	...
	window.ShowAppTagBanModal = function() {
		$BanModal.show();
		ShowAlertDialog( 'Tiltott címkék', $BanModal).always( function() {
			if ( bBansChanged )
				window.location.reload();
		} );
		...
		$BanModal.find( '.app_tag_report').click( function() {
			...
			ShowConfirmDialog( 'Címke engedélyezése',
				'Biztosan engedélyezni akarod a(z) „%s” címkét?'.replace( /%s/, tag ),
				'Címke engedélyezése')
				.done( function() {
					var rgParams = {
						appid: appid,
					...
					$J.post( 'http://store.steampowered.com/tagdata/tagapp', rgParams ).done( function( data ) {
						...
						ShowAlertDialog( 'Címke engedélyezése', 'Probléma adódott e címke tiltása vagy engedélyezése során. Próbáld újra később.' );
					});
					bBansChanged = true;
				});
		});
