...
HighlightPlayer.prototype.LoadMovie = function( $Container, bUserAction )
{
	...
	else
	{
		...
		var strMessage = 'Hiba történt a videó lejátszásának megkísérlésekor. Kérjük, győződj meg róla, hogy böngésződ naprakész.';
...
HighlightPlayer.prototype.ShowScreenshotPopup = function( screenshotid )
{
	...
	var $Title = $J('<a/>' );
	if ( Steam.BIsUserInSteamClient() )
		$Title.text( 'Megnézés teljes méretben a böngészőben' );
	else
		$Title.text( 'Letöltés teljes méretben' );
	...
	var $BtnPrev = $J('<div/>', {'class': 'btnv6_blue_hoverfade btn_medium previous'}).append( $J('<span/>').text( 'Előző' ) );
	var $BtnNext = $J('<div/>', {'class': 'btnv6_blue_hoverfade btn_medium next'}).append( $J('<span/>').text( 'Következő' ) );
	...
	var fnUpdateFooter = function()
	{
		...
		$ScreenshotCount.text( '%1$s / %2$s képernyőmentés'.replace( /%1\$s/, iCurIndex + 1 ).replace( /%2\$s/, rgScreenshotIDs.length ) );
	};
	...
	$.fn.videoControls = function( options ) {
		...
		var overlaySrc = '<div class="html5_video_overlay">' +
				...
				'<div class="control_container">' +
					...
					'<div class="autoplay_label">Automatikus videolejátszás</div>' +
				'</div>' +
