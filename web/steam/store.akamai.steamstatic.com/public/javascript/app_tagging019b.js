...
function InitAppTagModal( appid, rgAppTags, rgUserTags, strTagLinkSNR, strYourTagSNR, bShowBanOption )
{
	...
	var fnSetCheckboxTooltip = function( $Checkbox )
	{
		if ( $Checkbox.hasClass('checked') )
			$Checkbox.data('store-tooltip', 'Je hebt deze tag toegepast. Klik om te verwijderen.' );
		else
			$Checkbox.data('store-tooltip', 'Klik hier om deze tag toe te passen.' );
	};

	var fnSetReportTooltip = function( $ReportFlag )
	{
		if ( $ReportFlag.hasClass('reported') )
			$ReportFlag.data('store-tooltip', 'Je hebt deze tag gerapporteerd omdat deze onjuist wordt gebruikt voor dit product. Klik opnieuw om de rapportering in te trekken.' );
		else
			$ReportFlag.data('store-tooltip', 'Klik om deze tag te rapporteren voor onjuist gebruik op dit product.' );
	};
	...
	var fnMakeTag = function( tagid, tag, checked, bPopular, bReported, bBrowsable )
	{
		...
		if ( bPopular )
		{
			...
			if ( $YourTags.length )
			{
				...
				if ( bShowBanOption )
				{
					...
					$Ban.data('store-tooltip', 'Klik hier om deze tag van dit product te verwijderen. Deze optie is alleen beschikbaar voor ontwikkelaars en Steam-personeel.' );
					BindStoreTooltip( $Ban );
					$Tag.append( $Ban );

					$Ban.click( function() {
						ShowConfirmDialog( 'Tag bannen', 'Weet je zeker dat je de tag "%s" van dit product wilt verwijderen?'.replace( /%s/, tag )).done( function()
						{
		...
		else
		{
			...
			$Link.data('store-tooltip', 'Er zijn nog niet genoeg items met de tag "%s"'.replace( /%s/, $Link.html() ) );
	...
	{
		var $Tag = $J('<div/>', {'class': 'app_tag previous_tag', 'data-tagid': tagid, 'data-store-tooltip': 'Klik hier om deze tag toe te passen.' }).text( tag ).click( function() { fnApplyTag( tag ) } );
	...
	var fnBuildTagDisplay = function( Modal )
	{
		...
		if ( rgYourPopularTags === null )
		{
			...
			if ( !bYourPopularTagsRequested )
			{
				...
				$J.get( 'http://store.steampowered.com/tagdata/populartags/dutch').done( function ( data ) {
		...
		if ( rgUserTags.length )
		{
			...
			for( var i = 0; i < rgUserTags.length; i++ )
			{
				if ( !rgUserTags[i].is_reported &&  !rgUserTags[i].browseable )
				{
					...
					$AppTag.data('store-tooltip', 'Er zijn nog niet genoeg items met de tag "%s"'.replace( /%s/, $AppTag.html() ) );
	...
	window.ShowAppTagBanModal = function() {
		...
		$BanModal.find( '.app_tag_report').click( function() {
			...
			ShowConfirmDialog( 'Verbanning tag opheffen',
				'Weet je zeker dat je de verbanning van de tag "%s" wilt opheffen?'.replace( /%s/, tag ),
