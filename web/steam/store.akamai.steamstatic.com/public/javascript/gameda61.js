...
function ShowEmbedWidget( )
{
	...
	var Modal = _BuildDialog( "Skapa Widget att kapsla in", $Content, [], fnOK, {} );
...
function ShowShareDialog( )
{
	...
	ShowAlertDialog( "Dela", $Content).always(
		function() {
			// save it away again for later
			$Content.hide();
			$J(document.body).append( $Content );
...
function InitQueueControls( store_appid, steamworks_appid, next_in_queue_appid, snr )
{
	...
	$IgnoreBtn.click( function() {
		$J.post( 'http://store.steampowered.com/recommended/ignorerecommendation/', {
			...
			if ( data && data.nSaleTaskCompleted ) { NewStickerPackModal( 'Markera något som "inte intresserad"' ); } // SummerSale2017
		}).fail( function( jqXHR ) {
			ShowAlertDialog( 'Inte intresserad', 'Ett problem uppstod när dina ändringar skulle sparas. Försök igen senare.' );
		});
	});
	...
	$FollowBtn.click( function() {
		$J.post( 'http://store.steampowered.com/explore/followgame/', {
			...
			ShowAlertDialog( 'Följ', 'Ett problem uppstod när dina ändringar skulle sparas. Försök igen senare.' );
		});
	});
	...
	$J('a.dq_settings_link').click( function() {
		CDiscoveryQueue.ShowCustomizeDialog( function( data ) {
			...
			{
				ShowConfirmDialog( 'Anpassa din Discovery-kö', 'Vi har skapat en ny Discovery-kö genom att tillämpa dina anpassningar. Vad vill du göra?',
					'Börja utforska den nya kön','Stanna här'
				).done( function() {
...
function InitAutocollapse()
{
	$J('.game_page_autocollapse').each( function() {
		...
		var $ReadMore = $J('<div/>', {'class': 'game_page_autocollapse_readmore' }).text( 'LÄS MER' );
...
function UserReviewVoteUp( id )
{
	UserReview_Rate( id, true, 'http://store.steampowered.com/',
		function( rgResults ) {
			if ( rgResults.nSaleTaskCompleted ) { NewStickerPackModal( 'Markera en recension som hjälpsam... eller inte');}
...
function BuildReviewHistogram()
{
	...
	$J.get( 'http://store.steampowered.com/appreviewhistogram/' + appid, { l: 'swedish' }
	).done( function( data ) {
		...
		var seriesRollup = [ { label: "Positiva", color: "#66c0f4", fillColor: "#66c0f4", data: chartDataPositive }, { label: "Negativa", color: "#A34C25", fillColor: "#A34C25", data: chartDataNegative } ];
		...
		if ( data.results.recent )
		{
			...
			seriesRecent = [ { color: "#66c0f4", label: "Positiva", data: chartDataPositive }, { color: "#A34C25", label: "Negativa", data: chartDataNegative } ];
		}
...
function UpdateActiveFilters()
{
	...
	if ( $J( "#review_type_positive" ).attr( "checked" ) )
	{
		...
		$J( "#reviews_filter_type" ).text( 'Positiva' );
	}
	else if ( $J( "#review_type_negative" ).attr( "checked" ) )
	{
		bAnyActiveFilters = true;
		$J( "#reviews_filter_type" ).show();
		$J( "#reviews_filter_type" ).text( 'Negativa' );
	}
	else
	{
	...
	if ( $J( "#purchase_type_steam" ).attr( "checked" ) )
	{
		...
		$J( "#reviews_filter_purchase_type" ).text( 'Steam-köpare' );
	}
	else if ( $J( "#purchase_type_non_steam" ).attr( "checked" ) )
	{
		bAnyActiveFilters = true;
		$J( "#reviews_filter_purchase_type" ).show();
		$J( "#reviews_filter_purchase_type" ).text( 'Inte köpt på Steam' );
	}
	else
	{
	...
	if ( $J( "#review_language_mine" ).attr( "checked" ) )
	{
		...
		$J( "#reviews_filter_language" ).text( 'Dina språk' );
	}
	else
	{
	...
	if ( $J( "#review_date_range_histogram" ).attr( "checked" ) )
	{
		...
		$J( "#review_selected_histogram_date_range_prefix" ).text( 'Visa endast ' );
	}
	else if ( $J( "#review_date_range_exclude_histogram" ).attr( "checked" ) )
	{
		bAnyActiveFilters = true;
		$J( "#reviews_filter_graph" ).show();
		$J( "#review_selected_histogram_date_range_prefix" ).text( 'Uteslut ' );
	}
	else
	{
...
function ShowReportDialog( nAppId )
{
	...
	content.append( $J('<div/>', {'class': 'app_report_dialog_intro' } ).text('Var god välj anledningen till att du anmäler denna produkt.') );

		var rgReportOptions = {"1":"Bedr\u00e4geri - <span class=\"sub\">Detta program f\u00f6rs\u00f6ker bedr\u00e4gligt insamla k\u00e4nslig information som Steam-uppgifter eller ekonomiska uppgifter (t.ex. kreditkortsinformation).<\/span>","2":"Skadlig - <span class=\"sub\">Detta program modifierar en kunds dator p\u00e5 ett ov\u00e4ntat eller skadligt s\u00e4tt (t.ex. skadlig kod eller virus)<\/span>","3":"Hatiska uttalanden - <span class=\"sub\">Inneh\u00e5ller hatiska uttalanden, som f\u00f6respr\u00e5kar hat, v\u00e5ld eller diskriminering mot grupper eller m\u00e4nniskor baserat p\u00e5 etnicitet, religion, k\u00f6n, \u00e5lder, handikapp eller sexuell l\u00e4ggning<\/span>","4":"Pornografi - <span class=\"sub\">Inneh\u00e5ller pornografi<\/span>","5":"Sexuellt Inneh\u00e5ll - <span class=\"sub\">Innhe\u00e5ller sexuellt ineh\u00e5ll som inte \u00e4r l\u00e4mpligt f\u00f6r m\u00e4rkt \u00e5ldersrekommendation<\/span>","6":"Kr\u00e4nkande - <span class=\"sub\">Inneh\u00e5ller kr\u00e4nkande uttalanden.<\/span>","7":"St\u00f6tande - <span class=\"sub\">Inneh\u00e5ller st\u00f6tande inneh\u00e5ll som uppenbart \u00e4r kr\u00e4nkande eller \u00e4mnade att chockera eller \u00e4ckla \u00e5sk\u00e5dare<\/span>","8":"Barnutnyttjande - <span class=\"sub\">Inneh\u00e5ller inneh\u00e5ll som utnyttjar barn p\u00e5 n\u00e5got s\u00e4tt<\/span>","11":"R\u00e4ttsligt brott - <span class=\"sub\">Inneh\u00e5ller inneh\u00e5ll som juridiskt bryter mot lagen<\/span>","13":"Trasig - <span class=\"sub\">Laddas inte ner, startas eller k\u00f6rs inkorrekt, \u00e4ven p\u00e5 en dator som m\u00f6ter minimumsystemkraven.<\/span>"};
	...
	content.append( $J('<div/>', {'class': 'app_report_dialog_intro' } ).text('Du kan ange ytterligare information som du tycker är relevant här:') );
	content.append( textArea );

	content.append( $J('<div/>', {'class': 'app_report_dialog_dmca' } ).html('Om du vill rapportera upphovsrättsintrång och är upphovsrättsinnehavare, gå vidare till formuläret för DMCA-kompatibel anmälan om upphovsrättsintrång <a href="http://steamcommunity.com/dmca/create/">här</a>.') );

	var dialog = ShowConfirmDialog( 'Anmäl denna produkt', content, 'Anmäl');
	...
	dialog.done( function() {
		...
		if ( eReportTypeSelected )
		{
			$J.post(
				'http://store.steampowered.com/appreport/' + nAppId + '/report/',
				{
					...
					if ( json.success == 29 )
					{
						ShowAlertDialog( 'Fel', 'Du har redan anmält denna produkt!' );
						gReportedApp = true;
						$J( "#ReportAppBtn").addClass( 'btn_active' );
					}
					else
		...
		{
			ShowAlertDialog( 'Fel', 'Du måste välja en anledning till att du anmäler denna produkt!' );
		}
	});
}
...
{
		var $ModalContent = $J("<div class=\"gotsteamModal\">\r\n\t<div class=\"got_steam_ctn\">\r\n\t<div class=\"got_steam_box\">\r\n\t\t<h1>Har du Steam?<\/h1>\r\n\t\t<p>Du beh\u00f6ver ha <a href=\"http:\/\/store.steampowered.com\/about\/\">Steams skrivbordsapplikation<\/a> installerad innan du kan installera och starta <strong class=\"gotSteam_AppName\"><\/strong>. Har du Steam installerat p\u00e5 denna dator?<\/p>\r\n\t\t<div class=\"gotsteam_buttons\">\r\n\t\t\t<a class=\"gotSteam_SteamURL btn_blue leftbtn\" href=\"\">\r\n\t\t\t\t<h3>Ja, Steam \u00e4r installerat<\/h3>\r\n\t\t\t\t<h5 class=\"gotsteam_action\"><\/h5>\r\n\t\t\t<\/a>\r\n\t\t\t<a href=\"http:\/\/store.steampowered.com\/about\/\" class=\"btn_blue\">\r\n\t\t\t\t<h3>Nej, jag beh\u00f6ver Steam<\/h3>\r\n\t\t\t\t<h5>L\u00e4s om och ladda ner Steam<\/h5>\r\n\t\t\t<\/a>\r\n\t\t\t<div style=\"clear: left;\"><\/div>\r\n\t\t<\/div>\r\n\t\t<div class=\"got_steam_low_block\">\r\n\t\t\t<div class=\"gotsteam_steam_ico\"><img src=\"http:\/\/store.akamai.steamstatic.com\/public\/images\/v6\/steam_ico.png\" width=\"40\" height=\"40\" border=\"0\" \/><\/div>\r\n\t\t\tSteam \u00e4r den fr\u00e4msta spelplattformen f\u00f6r datorer. Den \u00e4r gratis och enkel att anv\u00e4nda. <a href=\"http:\/\/store.steampowered.com\/about\/\">L\u00e4s mer om Steam.<\/a>\r\n\t\t<\/div><\/div>\r\n\t<\/div>\r\n<\/div>");
...
{
	var strTemplate = "<div class=\"recommended_more_info_modal\">\r\n\t\t\t\t\t\t\t\t<p class=\"intro\">Du har hittat n\u00e5got som inte liknar vad du har anv\u00e4nt tidigare. Steam l\u00e4r sig dina preferenser baserat p\u00e5 spelen du spelar, filmerna du tittar p\u00e5 och programmen du anv\u00e4nder p\u00e5 Steam. Vi kommer anv\u00e4nda denna aktivitet f\u00f6r att anpassa vilka artiklar som rekommenderas f\u00f6r dig.<\/p>\r\n\t\t\t\t\t\t\t\t<h2>Gillar du denna artikel?<\/h2>\r\n\t\t\t\t\t\t\t\t<p>Om du gillar denna artikel och g\u00e5r vidare med att k\u00f6pa och anv\u00e4nda den, s\u00e5 kommer vi ta h\u00e4nsyn till det n\u00e4r vi g\u00f6r framtida rekommendationer.<\/p>\r\n\t\t\t\t\t\t\t\t<h2>Inte intresserad av artiklar som denna?<\/h2>\r\n\t\t\t\t\t\t\t\t<p>Du kan \u00e4ven konfigurera dina butikspreferenser f\u00f6r att ange vilka taggar eller produkttyper du inte \u00e4r intresserad av. <a href=\"https:\/\/store.steampowered.com\/account\/preferences\/\">G\u00e5 till butikspreferenser<\/a>.<\/p>\r\n\t\t\t\t\t\t\t<\/div>";
	ShowAlertDialog( "Mer om rekommendationer", strTemplate);
