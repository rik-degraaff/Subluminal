...
function InitAppTagModal( appid, rgAppTags, rgUserTags, strTagLinkSNR, strYourTagSNR, bShowBanOption )
{
	...
	var fnSetCheckboxTooltip = function( $Checkbox )
	{
		if ( $Checkbox.hasClass('checked') )
			$Checkbox.data('store-tooltip', 'Použili jste tuto značku. Klikněte pro její odstranění.' );
		else
			$Checkbox.data('store-tooltip', 'Klikněte zde pro použití této značky.' );
	};

	var fnSetReportTooltip = function( $ReportFlag )
	{
		if ( $ReportFlag.hasClass('reported') )
			$ReportFlag.data('store-tooltip', 'Tuto značku jste nahlásili, protože byla k tomuto produktu nesprávně přiřazena. Klikněte pro zrušení Vašeho nahlášení.' );
		else
			$ReportFlag.data('store-tooltip', 'Klikněte pro nahlášení této značky, která byla k tomuto produktu nesprávně přiřazena.' );
	};
	...
	var fnMakeTag = function( tagid, tag, checked, bPopular, bReported, bBrowsable )
	{
		...
		if ( bPopular )
		{
			...
			if ( $YourTags.length )
			{
				...
				if ( bShowBanOption )
				{
					...
					$Ban.data('store-tooltip', 'Klikněte pro odebrání této značky od tohoto produktu. Toto oprávnění mají pouze vývojáři této hry a zaměstnanci společnosti Valve.' );
					BindStoreTooltip( $Ban );
					$Tag.append( $Ban );

					$Ban.click( function() {
						ShowConfirmDialog( 'Zablokovat značku', 'Opravdu chcete odebrat značku „%s“ od tohoto produktu?'.replace( /%s/, tag )).done( function()
						{
		...
		else
		{
			...
			$Link.data('store-tooltip', 'Značkou „%s“ dosud nebylo označeno dostatečné množství položek.'.replace( /%s/, $Link.html() ) );
	...
	{
		var $Tag = $J('<div/>', {'class': 'app_tag previous_tag', 'data-tagid': tagid, 'data-store-tooltip': 'Klikněte zde pro použití této značky.' }).text( tag ).click( function() { fnApplyTag( tag ) } );
	...
	var fnBuildTagDisplay = function( Modal )
	{
		...
		if ( rgYourPopularTags === null )
		{
			...
			if ( !bYourPopularTagsRequested )
			{
				...
				$J.get( 'http://store.steampowered.com/tagdata/populartags/czech').done( function ( data ) {
		...
		else
		{
			...
			{
				$YourTags.append( $J('<div/>', {'class': 'app_tag_control no_tags_yet'}).text('Dosud nebyly zadány žádné značky' ) );
			}
		...
		if ( rgUserTags.length )
		{
			...
			for( var i = 0; i < rgUserTags.length; i++ )
			{
				if ( !rgUserTags[i].is_reported &&  !rgUserTags[i].browseable )
				{
					...
					$AppTag.data('store-tooltip', 'Značkou „%s“ dosud nebylo označeno dostatečné množství položek.'.replace( /%s/, $AppTag.html() ) );
	...
	var fnApplyTag = function( tag, withdraw, bPopularClick, eReportType, unTagID )
	{
		...
		$J.post( 'http://store.steampowered.com/tagdata/tagapp', rgParams ).done( function( data ) {
			...
			ShowAlertDialog( 'Zobrazit a upravit značky u tohoto produktu', 'There was a problem adding your tag.' );
		});
	};
	...
	window.ShowAppTagModal = function( appid )
	{
		...
		$AppTagModal.show();
		var Modal = ShowAlertDialog( 'Zobrazit a upravit značky u tohoto produktu', $AppTagModal, 'Zavřít' );
	...
	var fnDisplayTag = function( unTagID, strTagName ) {
		if ( !rgTagResults[unTagID] )
		{
			...
			$J.get( 'http://store.steampowered.com/tagdata/gettaggames/' + strTagLanguage + '/' + unTagID + '/', rgParams ).done( function ( html ) {
				...
				$GamesElement.find( '.browse_tag_loading').text( 'Omlouváme se, ale při nahrávání položek došlo k chybě. Zkuste to prosím znovu později.' );
			});
	...
	window.ShowAppTagBanModal = function() {
		...
		$BanModal.find( '.app_tag_report').click( function() {
			...
			ShowConfirmDialog( 'Odblokovat značku',
				'Opravdu chcete odblokovat značku „%s“?'.replace( /%s/, tag ),
				'Odblokovat značku')
				.done( function() {
					var rgParams = {
						appid: appid,
					...
					$J.post( 'http://store.steampowered.com/tagdata/tagapp', rgParams ).done( function( data ) {
						...
						ShowAlertDialog( 'Odblokovat značku', 'Tuto značku se nepodařilo zablokovat nebo odblokovat. Zkuste to prosím později.' );
					});
					bBansChanged = true;
				});
		});
