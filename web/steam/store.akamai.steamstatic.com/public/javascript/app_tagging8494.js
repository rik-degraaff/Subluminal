...
function InitAppTagModal( appid, rgAppTags, rgUserTags, strTagLinkSNR, strYourTagSNR, bShowBanOption )
{
	...
	var fnSetCheckboxTooltip = function( $Checkbox )
	{
		if ( $Checkbox.hasClass('checked') )
			$Checkbox.data('store-tooltip', 'Vous avez appliqué ce tag. Cliquez pour supprimer.' );
		else
			$Checkbox.data('store-tooltip', 'Cliquez ici pour appliquer ce tag.' );
	};

	var fnSetReportTooltip = function( $ReportFlag )
	{
		if ( $ReportFlag.hasClass('reported') )
			$ReportFlag.data('store-tooltip', 'Vous avez signalé ce tag comme ne correspondant pas à ce produit. Cliquez à nouveau pour supprimer ce signalement.' );
		else
			$ReportFlag.data('store-tooltip', 'Cliquez pour signaler un tag qui ne correspond pas à ce produit.' );
	};
	...
	var fnMakeTag = function( tagid, tag, checked, bPopular, bReported, bBrowsable )
	{
		...
		if ( bPopular )
		{
			...
			if ( $YourTags.length )
			{
				...
				if ( bShowBanOption )
				{
					...
					$Ban.data('store-tooltip', 'Cliquez ici pour supprimer ce tag de ce produit. Cette option est limitée aux développeurs au personnel de Steam.' );
					BindStoreTooltip( $Ban );
					$Tag.append( $Ban );

					$Ban.click( function() {
						ShowConfirmDialog( 'Bannir ce tag', 'Voulez-vous vraiment supprimer le tag « %s » de ce produit ?'.replace( /%s/, tag )).done( function()
						{
		...
		else
		{
			...
			$Link.data('store-tooltip', 'Il n\'y a pas assez d\'articles associés au tag « %s » pour l\'instant'.replace( /%s/, $Link.html() ) );
	...
	{
		var $Tag = $J('<div/>', {'class': 'app_tag previous_tag', 'data-tagid': tagid, 'data-store-tooltip': 'Cliquez ici pour appliquer ce tag.' }).text( tag ).click( function() { fnApplyTag( tag ) } );
	...
	var fnBuildTagDisplay = function( Modal )
	{
		...
		if ( rgYourPopularTags === null )
		{
			...
			if ( !bYourPopularTagsRequested )
			{
				...
				$J.get( 'http://store.steampowered.com/tagdata/populartags/french').done( function ( data ) {
		...
		if ( rgUserTags.length )
		{
			...
			for( var i = 0; i < rgUserTags.length; i++ )
			{
				if ( !rgUserTags[i].is_reported &&  !rgUserTags[i].browseable )
				{
					...
					$AppTag.data('store-tooltip', 'Il n\'y a pas assez d\'articles associés au tag « %s » pour l\'instant'.replace( /%s/, $AppTag.html() ) );
	...
	window.ShowAppTagBanModal = function() {
		...
		$BanModal.find( '.app_tag_report').click( function() {
			...
			ShowConfirmDialog( 'Débannir le tag',
				'Voulez-vous vraiment débannir le tag « %s » ?'.replace( /%s/, tag ),
				'Débannir le tag')
				.done( function() {
					var rgParams = {
						appid: appid,
