...
GDynamicStore = {
		...
		$DynamicElements.each( function() {
			...
			if ( !$El.hasClass('ds_no_flags') )
			{
				...
				if ( bOwned )
				{
					...
					$El.append( '<div class="ds_flag ds_owned_flag">V KNIHOVNĚ&nbsp;&nbsp;</div>');
				}
				else if ( bWanted )
				{
					$El.addClass( 'ds_flagged ds_wishlist' );
					$El.append( '<div class="ds_flag ds_wishlist_flag">V SEZNAMU PŘÁNÍ&nbsp;&nbsp;</div>');
				}

				if ( bInCart )
				{
					$El.addClass( 'ds_flagged ds_incart' );
					$El.append( '<div class="ds_flag ds_incart_flag">V KOŠÍKU&nbsp;&nbsp;</div>');
				}
		...
		if( strAppIDs )
		{
			...
			if (!bOnWishlist)
			{
				var fnClick = function ( event )
				{
					...
					$elSource.append( '<div class="ds_flag ds_wishlist_flag">V SEZNAMU PŘÁNÍ&nbsp;&nbsp;</div>');
				...
				var strText = "P\u0159idat do seznamu p\u0159\u00e1n\u00ed";
			...
			var $elPreferences = $J ( '<a/>' ).attr('href', 'https://store.steampowered.com/account/preferences' ).text ( "Mo\u017enosti" ).addClass( 'option' );
	...
	UpdateDynamicBundleElements: function( Bundle, $El )
	{
		...
		if ( !Bundle.m_bIsCommercial && ( !Bundle.m_rgBundleItems.length || ( Bundle.m_bMustPurchaseAsSet && Bundle.m_cUserItemsInBundle < Bundle.m_cTotalItemsInBundle ) ) )
		{
			...
			var strTooltip = 'Tento balíček si nemůžete zakoupit, jelikož již vlastníte všechny položky v něm obsažené.';

			if ( Bundle.m_bMustPurchaseAsSet && !Bundle.m_bRestrictGifting )
			{
				strTooltip = 'Tato nabídka platí pouze v případě, že si všechny položky (%s) zakoupíte najednou. Balíček můžete stále zakoupit jako dárek pro svého přítele.'.replace(/%s/, Bundle.m_cTotalItemsInBundle);
				$CartBtn.find('span').text( 'Zakoupit jako dárek' );
			}
			else
			{
		...
		else if ( !Bundle.m_bMustPurchaseAsSet )
		{
			...
			if ( !Bundle.m_bContainsDiscountedPackage )
			{
				...
				$DiscountBlocks.find('.discount_final_price' ).addClass('your_price' ).empty().append($J('<div/>', {'class': 'your_price_label'} ).text('Vaše cena:'), $J('<div/>' ).text( strFormattedFinalPrice ) );
			}
			else
			{
		...
		if ( $El.is( '.package_totals_area' ) && !Bundle.m_bMustPurchaseAsSet )
		{
			...
			else
			{
				...
				{
					$El.find('.bundle_final_package_price_desc' ).text( 'Individuální cena položek (%s), které zatím nevlastníte:'.replace( '%s', Bundle.m_cUserItemsInBundle ) );
				}
	...
	BuildBundleDescription: function( Bundle, $Description )
	{
		if ( Bundle.m_cUserItemsInBundle == 0 )
		{
			...
			$Description.html( '<span class="collectionComplete">Kolekce je kompletní!</span> Ve Vaší knihovně se již nachází %1$s z %2$s položek z této kolekce.'.replace( '%1$s', Bundle.m_cTotalItemsInBundle ).replace( '%2$s', Bundle.m_cTotalItemsInBundle ) );
		}
		else if ( Bundle.m_cUserItemsInBundle < Bundle.m_cTotalItemsInBundle )
		{
			// own some but not all.
			$Description.html( '<div>Ve Vaší knihovně se již nachází %1$s z %2$s položek z této kolekce.</div>'.replace( '%1$s', Bundle.m_cTotalItemsInBundle - Bundle.m_cUserItemsInBundle ).replace( '%2$s', Bundle.m_cTotalItemsInBundle ) );
			$Description.append( '<div>Zakoupením tohoto balíčku získáte všech %2$s položek, které zatím nevlastníte, s %1$s%% slevou!</div>'.replace( '%1$s', Bundle.m_nDiscountPct ).replace( '%2$s', Bundle.m_cUserItemsInBundle ).replace( '%%', '%' ) );

			// add "complete the set" flag
			$Description.parents('.dynamic_bundle_description' ).append( $J('<div/>', {'class': 'ds_flag ds_completetheset'} ).text('DOKONČETE VAŠI KOLEKCI!') );
		}
		else
		{
			$Description.html( '<div>Zakoupením tohoto balíčku získáte všech %2$s položek s %1$s%% slevou!</div>'.replace( '%1$s', Bundle.m_nDiscountPct ).replace( '%2$s', Bundle.m_cTotalItemsInBundle ).replace( '%%', '%' ) );
		}
	...
	PopulateRecommendedTagList: function()
	{
		...
		for( var i = 0; i < GDynamicStore.s_rgRecommendedTags.length && i < 5; i++ )
		{
			var tag = GDynamicStore.s_rgRecommendedTags[i];
							var $Link = $J('<a/>', {'class': 'popup_menu_item', 'href': 'http://store.steampowered.com/tags/cs/' + encodeURIComponent( tag.name ) });
						$Link.text( tag.name );
			$Element.append( $Link );
		}
	},
	...
	DisplayBundleSimulator: function( unBundleID )
	{
		...
		$Form.append( $J('<h2/>').text('Označit položky jako „vlastněné“') );
		$Form.append( $J('<div/>', {'class': 'bundle_simulator_secondary' } ).append(
			$J('<a/>', {'href': 'javascript:void(0);'} ).click( function() { $Form.find('input').prop('checked', true ); } ).text( "Vybrat vše" ),
			' - ',
			$J('<a/>', {'href': 'javascript:void(0);'} ).click( function() { $Form.find('input').prop('checked', false ); } ).text( "Nevybrat nic" )
		) );
		...
		Modal = ShowConfirmDialog( '', $Form, 'Obnovit zobrazení' ).done( function() {
...
function ShowHowDoDiscoveryQueuesWorkDialog()
{
	$J.get(
		...
		function( data )
		{
			ShowAlertDialog( 'Jak to funguje?', data );
		}
	);
}
