...
function InitAppTagModal( appid, rgAppTags, rgUserTags, strTagLinkSNR, strYourTagSNR, bShowBanOption )
{
	...
	var fnSetCheckboxTooltip = function( $Checkbox )
	{
		if ( $Checkbox.hasClass('checked') )
			$Checkbox.data('store-tooltip', 'Bu etiketi uyguladınız. Silmek için tıklayın.' );
		else
			$Checkbox.data('store-tooltip', 'Bu etiketi yapıştırmak için buraya tıklayın.' );
	};

	var fnSetReportTooltip = function( $ReportFlag )
	{
		if ( $ReportFlag.hasClass('reported') )
			$ReportFlag.data('store-tooltip', 'Bu etiketi, bu ürüne yanlış bir şekilde uygulandığı için şikayet ettiniz. Şikayeti kaldırmak için tekrar tıklayın.' );
		else
			$ReportFlag.data('store-tooltip', 'Bu etiketi bu ürüne yanlış bir şekilde uygulandığı için şikayet etmek için tıklayın.' );
	};
	...
	var fnMakeTag = function( tagid, tag, checked, bPopular, bReported, bBrowsable )
	{
		...
		if ( bPopular )
		{
			...
			if ( $YourTags.length )
			{
				...
				if ( bShowBanOption )
				{
					...
					$Ban.data('store-tooltip', 'Bu ürünün bu etiketini kaldırmak için buraya tıklayın. Bu seçenek, geliştiriciler ve Steam personeli ile sınırlıdır.' );
					BindStoreTooltip( $Ban );
					$Tag.append( $Ban );

					$Ban.click( function() {
						ShowConfirmDialog( 'Etiketi Yasakla', 'Bu üründen "%s" etiketini kaldırmak istediğinizden emin misiniz?'.replace( /%s/, tag )).done( function()
						{
		...
		else
		{
			...
			$Link.data('store-tooltip', 'Henüz "%s" olarak yeteri kadar öğe etiketlenmedi'.replace( /%s/, $Link.html() ) );
	...
	{
		var $Tag = $J('<div/>', {'class': 'app_tag previous_tag', 'data-tagid': tagid, 'data-store-tooltip': 'Bu etiketi yapıştırmak için buraya tıklayın.' }).text( tag ).click( function() { fnApplyTag( tag ) } );
	...
	var fnBuildTagDisplay = function( Modal )
	{
		...
		if ( rgYourPopularTags === null )
		{
			...
			if ( !bYourPopularTagsRequested )
			{
				...
				$J.get( 'http://store.steampowered.com/tagdata/populartags/turkish').done( function ( data ) {
		...
		if ( rgUserTags.length )
		{
			...
			for( var i = 0; i < rgUserTags.length; i++ )
			{
				if ( !rgUserTags[i].is_reported &&  !rgUserTags[i].browseable )
				{
					...
					$AppTag.data('store-tooltip', 'Henüz "%s" olarak yeteri kadar öğe etiketlenmedi'.replace( /%s/, $AppTag.html() ) );
	...
	window.ShowAppTagBanModal = function() {
		...
		$BanModal.find( '.app_tag_report').click( function() {
			...
			ShowConfirmDialog( 'Etiketin yasağını kaldır',
				'"%s" etiketinin yasağını kaldırmak istediğinize emin misiniz?'.replace( /%s/, tag ),
