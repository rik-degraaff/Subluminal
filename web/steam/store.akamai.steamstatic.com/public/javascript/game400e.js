...
function ShowEmbedWidget( )
{
	...
	var Modal = _BuildDialog( "Cr\u00e9ez des widget \u00e0 int\u00e9grer", $Content, [], fnOK, {} );
...
function ShowShareDialog( )
{
	...
	ShowAlertDialog( "Partager", $Content).always(
		function() {
			// save it away again for later
			$Content.hide();
			$J(document.body).append( $Content );
...
function InitQueueControls( store_appid, steamworks_appid, next_in_queue_appid, snr )
{
	...
	$IgnoreBtn.click( function() {
		$J.post( 'http://store.steampowered.com/recommended/ignorerecommendation/', {
			...
			if ( data && data.nSaleTaskCompleted ) { NewStickerPackModal( 'Indiquez qu’un jeu ne vous intéresse pas.' ); } // SummerSale2017
		}).fail( function( jqXHR ) {
			ShowAlertDialog( 'Pas intéressé', 'Une erreur est survenue lors de l\'enregistrement de vos modifications. Veuillez réessayer plus tard.' );
		});
	});
	...
	$FollowBtn.click( function() {
		$J.post( 'http://store.steampowered.com/explore/followgame/', {
			...
			ShowAlertDialog( 'Suivre', 'Une erreur est survenue lors de l\'enregistrement de vos modifications. Veuillez réessayer plus tard.' );
		});
	});
	...
	$J('a.dq_settings_link').click( function() {
		CDiscoveryQueue.ShowCustomizeDialog( function( data ) {
			...
			{
				ShowConfirmDialog( 'Personnalisez votre liste de découvertes', 'Nous vous avons réalisé une nouvelle liste de découvertes en accord avec vos préférences. Que souhaitez-vous faire ?',
					'Commencer à explorer la nouvelle liste','Rester ici'
				).done( function() {
...
function InitAutocollapse()
{
	$J('.game_page_autocollapse').each( function() {
		...
		var $ReadMore = $J('<div/>', {'class': 'game_page_autocollapse_readmore' }).text( 'EN SAVOIR PLUS' );
...
function UserReviewVoteUp( id )
{
	UserReview_Rate( id, true, 'http://store.steampowered.com/',
		function( rgResults ) {
			if ( rgResults.nSaleTaskCompleted ) { NewStickerPackModal( 'Signalez une évaluation comme utile... ou pas');}
...
function BuildReviewHistogram()
{
	...
	$J.get( 'http://store.steampowered.com/appreviewhistogram/' + appid, { l: 'french' }
	).done( function( data ) {
		...
		var seriesRollup = [ { label: "Positives", color: "#66c0f4", fillColor: "#66c0f4", data: chartDataPositive }, { label: "Négatives", color: "#A34C25", fillColor: "#A34C25", data: chartDataNegative } ];
		...
		if ( data.results.recent )
		{
			...
			seriesRecent = [ { color: "#66c0f4", label: "Positives", data: chartDataPositive }, { color: "#A34C25", label: "Négatives", data: chartDataNegative } ];
		}
...
function UpdateActiveFilters()
{
	...
	if ( $J( "#review_type_positive" ).attr( "checked" ) )
	{
		...
		$J( "#reviews_filter_type" ).text( 'Positives' );
	}
	else if ( $J( "#review_type_negative" ).attr( "checked" ) )
	{
		bAnyActiveFilters = true;
		$J( "#reviews_filter_type" ).show();
		$J( "#reviews_filter_type" ).text( 'Négatives' );
	}
	else
	{
	...
	if ( $J( "#purchase_type_steam" ).attr( "checked" ) )
	{
		...
		$J( "#reviews_filter_purchase_type" ).text( 'Achat Steam' );
	}
	else if ( $J( "#purchase_type_non_steam" ).attr( "checked" ) )
	{
		bAnyActiveFilters = true;
		$J( "#reviews_filter_purchase_type" ).show();
		$J( "#reviews_filter_purchase_type" ).text( 'Pas acheté sur Steam' );
	}
	else
	{
	...
	if ( $J( "#review_language_mine" ).attr( "checked" ) )
	{
		...
		$J( "#reviews_filter_language" ).text( 'Vos langues' );
	}
	else
	{
	...
	if ( $J( "#review_date_range_histogram" ).attr( "checked" ) )
	{
		...
		$J( "#review_selected_histogram_date_range_prefix" ).text( 'Afficher les évaluations publiées ' );
	}
	else if ( $J( "#review_date_range_exclude_histogram" ).attr( "checked" ) )
	{
		bAnyActiveFilters = true;
		$J( "#reviews_filter_graph" ).show();
		$J( "#review_selected_histogram_date_range_prefix" ).text( 'Exclure ' );
	}
	else
	{
...
function ShowReportDialog( nAppId )
{
	...
	content.append( $J('<div/>', {'class': 'app_report_dialog_intro' } ).text('Veuillez choisir une raison pour laquelle vous signalez ce produit.') );

		var rgReportOptions = {"1":"Fraude - <span class=\"sub\">Ce logiciel tente de collecter frauduleusement des informations sensibles, telles que les informations de connexions \u00e0 Steam ou des donn\u00e9es financi\u00e8res (exemple : les informations des cartes bancaires)<\/span>","2":"Malfaisant - <span class=\"sub\">Ce logiciel modifie des donn\u00e9es de l'ordinateur de l'utilisateur de mani\u00e8re malfaisante et inattendue (exemple : malware ou virus)<\/span>","3":"Discours haineux - <span class=\"sub\">Contient un ou plusieurs discours haineux, comme par exemple des discours encourageant la haine, la violence ou la discrimination envers un groupe ou une personne en fonction de son appartenance ethnique, sa religion, son genre, son \u00e2ge ou son orientation sexuelle<\/span>","4":"Pornographie - <span class=\"sub\">Contient du contenu pornographique<\/span>","5":"Contenu pour adulte - <span class=\"sub\">Contient du contenu pour adulte mal cat\u00e9goris\u00e9 et non-filtr\u00e9 par \u00e2ge<\/span>","6":"Diffamatoire - <span class=\"sub\">Contient des d\u00e9clarations diffamatoires et calomnieuses<\/span>","7":"Choquant - <span class=\"sub\">Comporte du contenu ouvertement injurieux ou destin\u00e9 \u00e0 choquer ou d\u00e9go\u00fbter les utilisateurs<\/span>","8":"Exploitation de l'enfant - <span class=\"sub\">Contient du contenu qui exploite les enfants de diff\u00e9rentes fa\u00e7ons<\/span>","11":"Violation juridique - <span class=\"sub\">Contient du contenu qui viole les lois de votre territoire<\/span>","13":"Probl\u00e8me - <span class=\"sub\">Ne se t\u00e9l\u00e9charge pas, ne se lance pas, ou ne fonctionne pas correctement, m\u00eame sur une machine qui dispose de la configuration minimum requise.<\/span>"};
	...
	content.append( $J('<div/>', {'class': 'app_report_dialog_intro' } ).text('Vous pouvez ajouter ici des informations supplémentaires que vous jugez pertinentes :') );
	content.append( textArea );

	content.append( $J('<div/>', {'class': 'app_report_dialog_dmca' } ).html('Si vous souhaitez signaler une atteinte aux droits de Copyright dont vous êtes le titulaire, veuillez utiliser notre formulaire de demande de retrait pour infraction de copyright DMCA <a href="http://steamcommunity.com/dmca/create/">ici</a>.') );

	var dialog = ShowConfirmDialog( 'Signaler ce produit', content, 'Signaler');
	...
	dialog.done( function() {
		...
		if ( eReportTypeSelected )
		{
			$J.post(
				'http://store.steampowered.com/appreport/' + nAppId + '/report/',
				{
					...
					if ( json.success == 29 )
					{
						ShowAlertDialog( 'Erreur', 'Vous avez déjà signalé ce produit !' );
						gReportedApp = true;
						$J( "#ReportAppBtn").addClass( 'btn_active' );
					}
					else
					{
						ShowAlertDialog( 'Erreur', 'Un problème est survenu lors de l\'enregistrement de votre signalement. Veuillez réessayer plus tard.' );
					}
		...
		{
			ShowAlertDialog( 'Erreur', 'Vous devez sélectionner pour ce produit une raison de signalement !' );
		}
	});
}
...
{
		var $ModalContent = $J("<div class=\"gotsteamModal\">\r\n\t<div class=\"got_steam_ctn\">\r\n\t<div class=\"got_steam_box\">\r\n\t\t<h1>Vous avez Steam&nbsp;?<\/h1>\r\n\t\t<p>Vous devez avoir l'<a href=\"http:\/\/store.steampowered.com\/about\/\">application de bureau Steam<\/a> install\u00e9e avant de pouvoir installer et lancer <strong class=\"gotSteam_AppName\"><\/strong>. Steam est-il install\u00e9 sur cet ordinateur ?<\/p>\r\n\t\t<div class=\"gotsteam_buttons\">\r\n\t\t\t<a class=\"gotSteam_SteamURL btn_blue leftbtn\" href=\"\">\r\n\t\t\t\t<h3>Oui, Steam est install\u00e9<\/h3>\r\n\t\t\t\t<h5 class=\"gotsteam_action\"><\/h5>\r\n\t\t\t<\/a>\r\n\t\t\t<a href=\"http:\/\/store.steampowered.com\/about\/\" class=\"btn_blue\">\r\n\t\t\t\t<h3>Non, j'ai besoin de Steam<\/h3>\r\n\t\t\t\t<h5>En savoir plus et t\u00e9l\u00e9charger Steam<\/h5>\r\n\t\t\t<\/a>\r\n\t\t\t<div style=\"clear: left;\"><\/div>\r\n\t\t<\/div>\r\n\t\t<div class=\"got_steam_low_block\">\r\n\t\t\t<div class=\"gotsteam_steam_ico\"><img src=\"http:\/\/store.akamai.steamstatic.com\/public\/images\/v6\/steam_ico.png\" width=\"40\" height=\"40\" border=\"0\" \/><\/div>\r\n\t\t\tSteam est la premi\u00e8re plateforme de jeux sur ordinateur. C'est gratuit et la prise en main est tr\u00e8s facile. <a href=\"http:\/\/store.steampowered.com\/about\/\">En savoir plus sur Steam.<\/a>\r\n\t\t<\/div><\/div>\r\n\t<\/div>\r\n<\/div>");
...
{
	var strTemplate = "<div class=\"recommended_more_info_modal\">\r\n\t\t\t\t\t\t\t\t<p class=\"intro\">Vous avez trouv\u00e9 un article ne ressemblant \u00e0 aucun de vos produits habituels. Steam s'aidera de vos pr\u00e9f\u00e9rences en mati\u00e8re de jeux, de films et de logiciels. Nous utiliserons cette activit\u00e9 pour vous recommander des titres pouvant vous plaire.<\/p>\r\n\t\t\t\t\t\t\t\t<h2>Cet article vous pla\u00eet ?<\/h2>\r\n\t\t\t\t\t\t\t\t<p>Si cet article vous pla\u00eet et que vous d\u00e9cidez de l'acheter et d'y jouer, nous nous en souviendrons pour de futures recommandations.<\/p>\r\n\t\t\t\t\t\t\t\t<h2>Ce genre d'articles ne vous int\u00e9resse pas ?<\/h2>\r\n\t\t\t\t\t\t\t\t<p>Vous pouvez configurer vos pr\u00e9f\u00e9rences du magasin pour indiquer les tags ou types de produits qui ne vous int\u00e9ressent pas. <a href=\"https:\/\/store.steampowered.com\/account\/preferences\/\">Cliquez ici pour consulter vos pr\u00e9f\u00e9rences<\/a>.<\/p>\r\n\t\t\t\t\t\t\t<\/div>";
	ShowAlertDialog( "Plus d'informations sur les recommandations", strTemplate);
