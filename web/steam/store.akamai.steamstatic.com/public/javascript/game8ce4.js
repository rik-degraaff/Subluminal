...
function ShowEmbedWidget( )
{
	...
	var Modal = _BuildDialog( "\u0e2a\u0e23\u0e49\u0e32\u0e07\u0e27\u0e34\u0e14\u0e40\u0e08\u0e47\u0e15\u0e40\u0e1e\u0e37\u0e48\u0e2d\u0e19\u0e33\u0e44\u0e1b\u0e1d\u0e31\u0e07\u0e15\u0e31\u0e27", $Content, [], fnOK, {} );
...
function ShowShareDialog( )
{
	...
	ShowAlertDialog( "\u0e41\u0e1a\u0e48\u0e07\u0e1b\u0e31\u0e19", $Content).always(
		function() {
			// save it away again for later
			$Content.hide();
			$J(document.body).append( $Content );
...
function InitQueueControls( store_appid, steamworks_appid, next_in_queue_appid, snr )
{
	...
	$IgnoreBtn.click( function() {
		$J.post( 'http://store.steampowered.com/recommended/ignorerecommendation/', {
			...
			if ( data && data.nSaleTaskCompleted ) { NewStickerPackModal( 'ทำเครื่องหมายบางสิ่งเป็นไม่สนใจ' ); } // SummerSale2017
		}).fail( function( jqXHR ) {
			ShowAlertDialog( 'ยังไม่สนใจ', 'ตรวจพบปัญหาในการบันทึกการเปลี่ยนแปลงของคุณ กรุณาลองใหม่ภายหลัง' );
		});
	});
	...
	$FollowBtn.click( function() {
		$J.post( 'http://store.steampowered.com/explore/followgame/', {
			...
			ShowAlertDialog( 'ติดตาม', 'ตรวจพบปัญหาในการบันทึกการเปลี่ยนแปลงของคุณ กรุณาลองใหม่ภายหลัง' );
		});
	});
	...
	$J('a.dq_settings_link').click( function() {
		CDiscoveryQueue.ShowCustomizeDialog( function( data ) {
			...
			{
				ShowConfirmDialog( 'ปรับแต่งคิวค้นพบของคุณ', 'ทางเราได้สร้างคิวค้นพบใหม่โดยการเพิ่มการปรับแต่งของคุณ คุณต้องการทำอะไร?',
					'เริ่มสำรวจคิวใหม่','อยู่ที่นี่'
				).done( function() {
...
function InitAutocollapse()
{
	$J('.game_page_autocollapse').each( function() {
		...
		var $ReadMore = $J('<div/>', {'class': 'game_page_autocollapse_readmore' }).text( 'อ่านเพิ่มเติม' );
...
function UserReviewVoteUp( id )
{
	UserReview_Rate( id, true, 'http://store.steampowered.com/',
		function( rgResults ) {
			if ( rgResults.nSaleTaskCompleted ) { NewStickerPackModal( 'ทำเครื่องหมายบทวิจารณ์ว่าเป็นประโยชน์ ... หรือไม่เป็น');}
...
function BuildReviewHistogram()
{
	...
	$J.get( 'http://store.steampowered.com/appreviewhistogram/' + appid, { l: 'thai' }
	).done( function( data ) {
		...
		var seriesRollup = [ { label: "แง่บวก", color: "#66c0f4", fillColor: "#66c0f4", data: chartDataPositive }, { label: "แง่ลบ", color: "#A34C25", fillColor: "#A34C25", data: chartDataNegative } ];
		...
		if ( data.results.recent )
		{
			...
			seriesRecent = [ { color: "#66c0f4", label: "แง่บวก", data: chartDataPositive }, { color: "#A34C25", label: "แง่ลบ", data: chartDataNegative } ];
		}
...
function UpdateActiveFilters()
{
	...
	if ( $J( "#review_type_positive" ).attr( "checked" ) )
	{
		...
		$J( "#reviews_filter_type" ).text( 'แง่บวก' );
	}
	else if ( $J( "#review_type_negative" ).attr( "checked" ) )
	{
		bAnyActiveFilters = true;
		$J( "#reviews_filter_type" ).show();
		$J( "#reviews_filter_type" ).text( 'แง่ลบ' );
	}
	else
	{
	...
	if ( $J( "#purchase_type_steam" ).attr( "checked" ) )
	{
		...
		$J( "#reviews_filter_purchase_type" ).text( 'ผู้สั่งซื้อบน Steam' );
	}
	else if ( $J( "#purchase_type_non_steam" ).attr( "checked" ) )
	{
		bAnyActiveFilters = true;
		$J( "#reviews_filter_purchase_type" ).show();
		$J( "#reviews_filter_purchase_type" ).text( 'ไม่ได้สั่งซื้อบน Steam' );
	}
	else
	{
	...
	if ( $J( "#review_language_mine" ).attr( "checked" ) )
	{
		...
		$J( "#reviews_filter_language" ).text( 'ภาษาของคุณ' );
	}
	else
	{
	...
	if ( $J( "#review_date_range_histogram" ).attr( "checked" ) )
	{
		...
		$J( "#review_selected_histogram_date_range_prefix" ).text( 'ดูเฉพาะ ' );
	}
	else if ( $J( "#review_date_range_exclude_histogram" ).attr( "checked" ) )
	{
		bAnyActiveFilters = true;
		$J( "#reviews_filter_graph" ).show();
		$J( "#review_selected_histogram_date_range_prefix" ).text( 'ละเว้น ' );
	}
	else
	{
...
function ShowReportDialog( nAppId )
{
	...
	content.append( $J('<div/>', {'class': 'app_report_dialog_intro' } ).text('กรุณาเลือกเหตุผลว่าทำไมคุณถึงทำการรายงานผลิตภัณฑ์นี้') );

		var rgReportOptions = {"1":"\u0e09\u0e49\u0e2d\u0e42\u0e01\u0e07 - <span class=\"sub\">\u0e0b\u0e2d\u0e1f\u0e15\u0e4c\u0e41\u0e27\u0e23\u0e4c\u0e19\u0e35\u0e49\u0e1e\u0e22\u0e32\u0e22\u0e32\u0e21\u0e2b\u0e25\u0e2d\u0e01\u0e25\u0e27\u0e07\u0e40\u0e1e\u0e37\u0e48\u0e2d\u0e40\u0e01\u0e47\u0e1a\u0e23\u0e27\u0e1a\u0e23\u0e27\u0e21\u0e02\u0e49\u0e2d\u0e21\u0e39\u0e25\u0e2a\u0e33\u0e04\u0e31\u0e0d \u0e15\u0e31\u0e27\u0e2d\u0e22\u0e48\u0e32\u0e07\u0e40\u0e0a\u0e48\u0e19 \u0e02\u0e49\u0e2d\u0e21\u0e39\u0e25\u0e23\u0e31\u0e1a\u0e23\u0e2d\u0e07 Steam \u0e2b\u0e23\u0e37\u0e2d\u0e02\u0e49\u0e2d\u0e21\u0e39\u0e25\u0
- <span class=\"sub\">\u0e1a\u0e23\u0e23\u0e08\u0e38\u0e40\u0e19\u0e37\u0e49\u0e2d\u0e2b\u0e32\u0e17\u0e35\u0e48\u0e41\u0e2a\u0e27\u0e07\u0e2b\u0e32\u0e1b\u0e23\u0e30\u0e42\u0e22\u0e0a\u0e19\u0e4c\u0e08\u0e32\u0e01\u0e40\u0e14\u0e47\u0e01\u0e43\u0e19\u0e17\u0e32\u0e07\u0e43\u0e14\u0e17\u0e32\u0e07\u0e2b\u0e19\u0e36\u0e48\u0e07<\/span>","11":"\u0e01\u0e32\u0e23\u0e25\u0e30\u0e40\u0e21\u0e34\u0e14\u0e01\u0e0e\u0e2b\u0e21\u0e32\u0e22 - <span class=\"sub\">\u0e1a\u0e23\u0e23\u0e08\u0e38\u0e40\u0e19\u0e37\u0e49\u0e2d\u0e2b\u0e32\u0e17\u0e35\u0e48\u0e25\u0e30\u0e40\u0e21\u0e34\u0e14\u0e01\u0e0e\u0e2b\u0e21\u0e32\u0e22\u0e43\u0e19\u0e40\u0e02\u0e15\u0e2d\u0e33\u0e19\u0e32\u0e08\u0e28\u0e32\u0e25\u0e02\u0e2d\u0e07\u0e04\u0e38\u0e13<\/span>","13":"\u0e44\u0e21\u0e48\u0e2a\u0e21\u0e1a\u0e39\u0e23\u0e13\u0e4c - <span class=\"sub\">\u0e44\u0e21\u0e48\u0e2a\u0e32\u0e21\u0e32\u0e23\u0e16\u0e14\u0e32\u0e27\u0e19\u0e4c\u0e42\u0e2b\u0e25\u0e14 \u0e40\u0e23\u0e34\u0e48\u0e21\u0e40\u0e1b\u0e34\u0e14 \u0e2b\u0e23\u0e37\u0e2d\u0e43\u0e0a\u0e49\u0e07\u0e32\u0e19\u0e44\u0e14\u0e49\u0e2d\u0e22\u0e48\u0e32\u0e07\u0e16\u0e39\u0e01\u0e15\u0e49\u0e2d\u0e07 \u0e41\u0e21\u0e49\u0e27\u0e48\u0e32\u0e40\u0e04\u0e23\u0e37\u0e48\u0e2d\u0e07\u0e19\u0e31\u0e49\u0e19\u0e1c\u0e48\u0e32\u0e19\u0e04\u0e27\u0e32\u0e21\u0e15\u0e49\u0e2d\u0e07\u0e01\u0e32\u0e23\u0e23\u0e30\u0e1a\u0e1a\u0e02\u0e31\u0e49\u0e19\u0e15\u0e48\u0e33\u0e41\u0e25\u0e49\u0e27<\/span>"};
	...
	content.append( $J('<div/>', {'class': 'app_report_dialog_intro' } ).text('คุณสามารถกรอกข้อมูลเพิ่มเติมที่คุณรู้สึกว่ามีความเกี่ยวข้องได้ที่นี่:') );
	content.append( textArea );

	content.append( $J('<div/>', {'class': 'app_report_dialog_dmca' } ).html('ถ้าคุณต้องการจะรายงานการละเมิดลิขสิทธิ์ และคุณเป็นเจ้าของสิขสิทธิ์นี้ กรุณาดำเนินการต่อที่แบบฟอร์มแจ้งการร้องเรียน DMCA เกี่ยวกับการละเมิดลิขสิทธิ์ <a href="http://steamcommunity.com/dmca/create/">ที่นี่</a>') );

	var dialog = ShowConfirmDialog( 'รายงานผลิตภัณฑ์นี้', content, 'รายงาน');
	...
	dialog.done( function() {
		...
		if ( eReportTypeSelected )
		{
			$J.post(
				'http://store.steampowered.com/appreport/' + nAppId + '/report/',
				{
					...
					if ( json.success == 29 )
					{
						ShowAlertDialog( 'ข้อผิดพลาด', 'คุณได้รายงานผลิตภัณฑ์นี้เรียบร้อยแล้ว!' );
						gReportedApp = true;
						$J( "#ReportAppBtn").addClass( 'btn_active' );
					}
					else
					{
						ShowAlertDialog( 'ข้อผิดพลาด', 'ตรวจพบปัญหาในการบันทึกรายงานของคุณ กรุณาลองใหม่ภายหลัง' );
					}
		...
		{
			ShowAlertDialog( 'ข้อผิดพลาด', 'คุณต้องเลือกเหตุผลที่คุณรายงานผลิตภัณฑ์นี้!' );
		}
	});
}
...
{
		var $ModalContent = $J("<div class=\"gotsteamModal\">\r\n\t<div class=\"got_steam_ctn\">\r\n\t<div class=\"got_steam_box\">\r\n\t\t<h1