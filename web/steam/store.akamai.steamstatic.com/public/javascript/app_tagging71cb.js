...
function InitAppTagModal( appid, rgAppTags, rgUserTags, strTagLinkSNR, strYourTagSNR, bShowBanOption )
{
	...
	var fnSetCheckboxTooltip = function( $Checkbox )
	{
		if ( $Checkbox.hasClass('checked') )
			$Checkbox.data('store-tooltip', 'Hai già applicato questa etichetta. Clicca per rimuoverla.' );
		else
			$Checkbox.data('store-tooltip', 'Clicca qui per applicare questa etichetta.' );
	};

	var fnSetReportTooltip = function( $ReportFlag )
	{
		if ( $ReportFlag.hasClass('reported') )
			$ReportFlag.data('store-tooltip', 'Hai segnalato che questa etichetta è incorretta per il prodotto. Clicca di nuovo per rimuovere la segnalazione.' );
		else
			$ReportFlag.data('store-tooltip', 'Clicca per segnalare che questa etichetta è incorretta per il prodotto.' );
	};
	...
	var fnMakeTag = function( tagid, tag, checked, bPopular, bReported, bBrowsable )
	{
		...
		if ( bPopular )
		{
			...
			if ( $YourTags.length )
			{
				...
				if ( bShowBanOption )
				{
					...
					$Ban.data('store-tooltip', 'Clicca qui per rimuovere questa etichetta dal prodotto. L\'opzione è riservata agli sviluppatori e al personale di Steam.' );
					BindStoreTooltip( $Ban );
					$Tag.append( $Ban );

					$Ban.click( function() {
						ShowConfirmDialog( 'Bandisci etichetta', 'Sei sicuro di voler rimuovere l\'etichetta "%s" da questo prodotto?'.replace( /%s/, tag )).done( function()
						{
		...
		else
		{
			...
			$Link.data('store-tooltip', 'Non ci sono ancora abbastanza articoli etichettati con "%s"'.replace( /%s/, $Link.html() ) );
	...
	{
		var $Tag = $J('<div/>', {'class': 'app_tag previous_tag', 'data-tagid': tagid, 'data-store-tooltip': 'Clicca qui per applicare questa etichetta.' }).text( tag ).click( function() { fnApplyTag( tag ) } );
	...
	var fnBuildTagDisplay = function( Modal )
	{
		...
		if ( rgYourPopularTags === null )
		{
			...
			if ( !bYourPopularTagsRequested )
			{
				...
				$J.get( 'http://store.steampowered.com/tagdata/populartags/italian').done( function ( data ) {
		...
		if ( rgUserTags.length )
		{
			...
			for( var i = 0; i < rgUserTags.length; i++ )
			{
				if ( !rgUserTags[i].is_reported &&  !rgUserTags[i].browseable )
				{
					...
					$AppTag.data('store-tooltip', 'Non ci sono ancora abbastanza articoli etichettati con "%s"'.replace( /%s/, $AppTag.html() ) );
	...
	window.ShowAppTagBanModal = function() {
		...
		$BanModal.find( '.app_tag_report').click( function() {
			...
			ShowConfirmDialog( 'Rimuovi il ban dell\'etichetta',
				'Sei sicuro di voler rimuovere il ban per l\'etichetta "%s"?'.replace( /%s/, tag ),
