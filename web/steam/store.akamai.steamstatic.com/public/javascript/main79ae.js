...
function GameHover( elem, event, divHover, rgHoverData )
{
	...
	else if ( ( !$Hover.is(':visible') || g_HoverState.target != $Elem[0] ) && !oElemState.timer )
	{
		...
		if ( !$HoverData.length && !oElemState.bAjaxRequestMade )
		{
			...
			{
				// is cc needed?
				rgAjaxParams = { /*cc: rgHoverData['cc'],*/ l: 'finnish' };
			}
			if ( rgHoverData['v6'] )
			{
		...
		.done( function( data ) {
			...
			if ( data && data.success ) {
				...
				if ( data.saleTaskCompleted ) {
					NewStickerPackModal( 'Lisää toivelistaasi' );
				}
			}
			else {
	...
	$J.post( url, {sessionid: g_sessionID, appid: appid} )
		.done( function( data ) {
			if ( data && data.success )
			{
				...
				$J('span',button).text("Toivelistalla");
			}
			else
			{
				$J('span',button).text( "Oho, anteeksi." );
			}
...
function HideRecommendation( type, itemid, divBtn, elemContainer )
{
	...
	new Ajax.Request( 'http://store.steampowered.com/recommended/ignorerecommendation/', {
		...
		onSuccess: function( transport ) {
			...
			{
				if ( $(elemContainer) )
					$(elemContainer).update( '<div class="recommendation_ignored">Asia selvä, suosittelemme tässä jotakin muuta ensi kerralla</div>' );
			}
			else
			{
				$(divBtn).show();
...
function EnsureStoreMenuTagsLoaded( strId )
{
	...
	if ( !$Element.data('tags-loaded') )
	{
		...
		$J.get( url, {ll: 'finnish'} ).done( function( data ) {
			...
			if ( rgYourPopularTags.length > 0 )
			{
				...
				for( var i = 0; i < rgYourPopularTags.length && i < 5; i++ )
				{
					...
					var $Link = $J('<a/>', {'class': 'popup_menu_item', 'href': 'http://store.steampowered.com/tag/fi/' + encodeURIComponent( tag.name ) });
					$Link.text( tag.name );
					$Element.append( $Link );
				}
			}
...
function AddFreeLicense( subid, strDisplayName )
{
	...
	$J.post( 'http://store.steampowered.com/checkout/addfreelicense/' + subid, { ajax: true, sessionid: g_sessionID }).done( function() {
		ShowAlertDialog(
			strDisplayName,
			'%s on lisätty tilillesi ja on nyt saatavilla Steam-kokoelmassa.'.replace( /%s/, strDisplayName )
		).done( function() {
			window.location.reload();
		});
		if ( typeof GDynamicStore != 'undefined' )
		...
		if ( data && data.purchaseresultdetail == 9 )
			ShowAlertDialog( strDisplayName, 'Tämä tuote on jo saatavilla Steam-kokoelmassasi' );
		else
			ShowAlertDialog( strDisplayName, 'Ilmeni ongelma lisättäessä tuotetta tilillesi. Yritä myöhemmin uudelleen.' );
	}).always( function () {
...
function ChangeLanguage( strTargetLanguage, bStayOnPage )
{
	...
	.done( function() {
		...
		Modal.Dismiss();
		ShowAlertDialog( 'Vaihda kieli', 'Muutoksien tallentamisessa tapahtui virhe. Koeta uudestaan myöhemmin.' );
	});
}
...
function FollowCuratorWithCallback( clanID, bFollow, onComplete )
{
	...
	$J.post(
		...
		{
			ShowAlertDialog( 'Virhe', 'Steam-kuraattorin seuraamisessa tapahtui virhe.' );
		}
	);
	return false;
}
...
function IgnoreCuratorWithCallback( clanID, bIgnore, onComplete )
{
	...
	if ( !bHaveUser )
	{
		ShowAlertDialog("Kirjaudu sis\u00e4\u00e4n", "Sinun tulee olla kirjautuneena sis\u00e4\u00e4n, ennen kuin voit ohittaa Steam-kuraattorin.");
		return;
	}
	...
	$J.post(
		...
		{
			ShowAlertDialog( 'Virhe', 'Steam-kuraattorin ohittamisessa tapahtui virhe.' );
		}
	);
	return false;
}
