...
HighlightPlayer.prototype.LoadMovie = function( $Container, bUserAction )
{
	...
	else
	{
		...
		var strMessage = 'Ошибка при воспроизведении видео. Пожалуйста, убедитесь, что ваш браузер обновлён до последней версии.';
...
HighlightPlayer.prototype.ShowScreenshotPopup = function( screenshotid )
{
	...
	var $Title = $J('<a/>' );
	if ( Steam.BIsUserInSteamClient() )
		$Title.text( 'Просмотреть полноразмерную версию в браузере' );
	else
		$Title.text( 'Загрузить полноразмерную версию' );
	...
	var $BtnNext = $J('<div/>', {'class': 'btnv6_blue_hoverfade btn_medium next'}).append( $J('<span/>').text( 'Далее' ) );
	...
	var fnUpdateFooter = function()
	{
		...
		$ScreenshotCount.text( '%1$s из %2$s скриншотов'.replace( /%1\$s/, iCurIndex + 1 ).replace( /%2\$s/, rgScreenshotIDs.length ) );
	};
	...
	$.fn.videoControls = function( options ) {
		...
		var overlaySrc = '<div class="html5_video_overlay">' +
				...
				'<div class="control_container">' +
					...
					'<div class="autoplay_label">Автоматическое воспроизведение видео</div>' +
				'</div>' +
