...
GDynamicStore = {
		...
		$DynamicElements.each( function() {
			...
			if ( !$El.hasClass('ds_no_flags') )
			{
				...
				else if ( bWanted )
				{
					...
					$El.append( '<div class="ds_flag ds_wishlist_flag">I ØNSKELISTE&nbsp;&nbsp;</div>');
				}

				if ( bInCart )
				{
					$El.addClass( 'ds_flagged ds_incart' );
					$El.append( '<div class="ds_flag ds_incart_flag">I HANDLEKURV&nbsp;&nbsp;</div>');
				}
		...
		if( strAppIDs )
		{
			...
			if (!bOnWishlist)
			{
				var fnClick = function ( event )
				{
					...
					$elSource.append( '<div class="ds_flag ds_wishlist_flag">I ØNSKELISTE&nbsp;&nbsp;</div>');
				...
				var strText = "Legg til i \u00f8nskelisten";
	...
	UpdateDynamicBundleElements: function( Bundle, $El )
	{
		...
		if ( !Bundle.m_bIsCommercial && ( !Bundle.m_rgBundleItems.length || ( Bundle.m_bMustPurchaseAsSet && Bundle.m_cUserItemsInBundle < Bundle.m_cTotalItemsInBundle ) ) )
		{
			...
			if ( Bundle.m_bMustPurchaseAsSet && !Bundle.m_bRestrictGifting )
			{
				...
				$CartBtn.find('span').text( 'Kjøp som gave' );
			}
			else
			{
	...
	BuildBundleDescription: function( Bundle, $Description )
	{
		...
		else if ( Bundle.m_cUserItemsInBundle < Bundle.m_cTotalItemsInBundle )
		{
			...
			$Description.parents('.dynamic_bundle_description' ).append( $J('<div/>', {'class': 'ds_flag ds_completetheset'} ).text('FULLFØR SAMLINGEN DIN!') );
		}
		else
		{
	...
	PopulateRecommendedTagList: function()
	{
		...
		for( var i = 0; i < GDynamicStore.s_rgRecommendedTags.length && i < 5; i++ )
		{
			var tag = GDynamicStore.s_rgRecommendedTags[i];
							var $Link = $J('<a/>', {'class': 'popup_menu_item', 'href': 'http://store.steampowered.com/tags/no/' + encodeURIComponent( tag.name ) });
						$Link.text( tag.name );
			$Element.append( $Link );
		}
	},
	...
	DisplayBundleSimulator: function( unBundleID )
	{
		...
		$Form.append( $J('<div/>', {'class': 'bundle_simulator_secondary' } ).append(
			$J('<a/>', {'href': 'javascript:void(0);'} ).click( function() { $Form.find('input').prop('checked', true ); } ).text( "Velg alle" ),
			' - ',
			$J('<a/>', {'href': 'javascript:void(0);'} ).click( function() { $Form.find('input').prop('checked', false ); } ).text( "Velg ingen" )
		) );
		...
		Modal = ShowConfirmDialog( '', $Form, 'Oppdater visning' ).done( function() {
...
function ShowHowDoDiscoveryQueuesWorkDialog()
{
	$J.get(
		...
		function( data )
		{
			ShowAlertDialog( 'Hvordan fungerer dette?', data );
		}
	);
}
