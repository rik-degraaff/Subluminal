...
function GameHover( elem, event, divHover, rgHoverData )
{
	...
	else if ( ( !$Hover.is(':visible') || g_HoverState.target != $Elem[0] ) && !oElemState.timer )
	{
		...
		if ( !$HoverData.length && !oElemState.bAjaxRequestMade )
		{
			...
			{
				// is cc needed?
				rgAjaxParams = { /*cc: rgHoverData['cc'],*/ l: 'romanian' };
			}
			if ( rgHoverData['v6'] )
			{
		...
		.done( function( data ) {
			...
			if ( data && data.success ) {
				...
				if ( data.saleTaskCompleted ) {
					NewStickerPackModal( 'Adaugă în lista de dorințe' );
				}
			}
			else {
	...
	$J.post( url, {sessionid: g_sessionID, appid: appid} )
		.done( function( data ) {
			if ( data && data.success )
			{
				...
				$J('span',button).text("Pe lista de dorin\u021be");
			}
			else
			{
				$J('span',button).text( "Oops, scuze!" );
			}
...
function HideRecommendation( type, itemid, divBtn, elemContainer )
{
	...
	new Ajax.Request( 'http://store.steampowered.com/recommended/ignorerecommendation/', {
		...
		onSuccess: function( transport ) {
			...
			{
				if ( $(elemContainer) )
					$(elemContainer).update( '<div class="recommendation_ignored">Bun, vom recomanda altceva aici data viitoare</div>' );
			}
			else
			{
				$(divBtn).show();
...
function EnsureStoreMenuTagsLoaded( strId )
{
	...
	if ( !$Element.data('tags-loaded') )
	{
		...
		$J.get( url, {ll: 'romanian'} ).done( function( data ) {
			...
			if ( rgYourPopularTags.length > 0 )
			{
				...
				for( var i = 0; i < rgYourPopularTags.length && i < 5; i++ )
				{
					...
					var $Link = $J('<a/>', {'class': 'popup_menu_item', 'href': 'http://store.steampowered.com/tag/ro/' + encodeURIComponent( tag.name ) });
					$Link.text( tag.name );
					$Element.append( $Link );
				}
			}
...
function AddFreeLicense( subid, strDisplayName )
{
	...
	$J.post( 'http://store.steampowered.com/checkout/addfreelicense/' + subid, { ajax: true, sessionid: g_sessionID }).done( function() {
		ShowAlertDialog(
			strDisplayName,
			'%s a fost adăugat contului tău. Acum este disponibil în Colecția ta Steam.'.replace( /%s/, strDisplayName )
		).done( function() {
			window.location.reload();
		});
		if ( typeof GDynamicStore != 'undefined' )
		...
		if ( data && data.purchaseresultdetail == 9 )
			ShowAlertDialog( strDisplayName, 'Acest produs este deja disponibil în Colecția ta Steam.' );
		else
			ShowAlertDialog( strDisplayName, 'A apărut o problemă în adăugarea acestui produs contului tău. Te rugăm să încerci din nou mai târziu.' );
	}).always( function () {
...
function ChangeLanguage( strTargetLanguage, bStayOnPage )
{
	...
	.done( function() {
		...
		Modal.Dismiss();
		ShowAlertDialog( 'Schimbă limba', 'A apărut o problemă la salvarea schimbărilor tale. Te rugăm să încerci din nou mai târziu.' );
	});
}
...
function FollowCuratorWithCallback( clanID, bFollow, onComplete )
{
	...
	if ( !bHaveUser )
	{
		ShowAlertDialog("Te rug\u0103m s\u0103 te autentifici", "Trebuie s\u0103 fii autentificat pentru a urm\u0103ri un curator");
		return;
	}
	...
	$J.post(
		...
		{
			ShowAlertDialog( 'Eroare', 'Au apărut probleme în încercarea de a urmări Curatorul Steam.' );
		}
	);
	return false;
}
...
function IgnoreCuratorWithCallback( clanID, bIgnore, onComplete )
{
	...
	if ( !bHaveUser )
	{
		ShowAlertDialog("Te rug\u0103m s\u0103 te autentifici", "Trebuie s\u0103 fii autentificat \u00eenainte de a putea ignora un curator Steam");
		return;
	}
	...
	$J.post(
		...
		{
			ShowAlertDialog( 'Eroare', 'There was a problem trying to ignore the Steam Curator.' );
		}
	);
	return false;
}
