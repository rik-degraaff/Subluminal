...
function GameHover( elem, event, divHover, rgHoverData )
{
	...
	else if ( ( !$Hover.is(':visible') || g_HoverState.target != $Elem[0] ) && !oElemState.timer )
	{
		...
		if ( !$HoverData.length && !oElemState.bAjaxRequestMade )
		{
			...
			{
				// is cc needed?
				rgAjaxParams = { /*cc: rgHoverData['cc'],*/ l: 'french' };
			}
			if ( rgHoverData['v6'] )
			{
		...
		.done( function( data ) {
			...
			if ( data && data.success ) {
				...
				if ( data.saleTaskCompleted ) {
					NewStickerPackModal( 'Ajoutez des titres à votre liste de souhaits' );
				}
			}
			else {
	...
	$J.post( url, {sessionid: g_sessionID, appid: appid} )
		.done( function( data ) {
			if ( data && data.success )
			{
				...
				$J('span',button).text("Dans la liste de souhaits");
			}
			else
			{
				$J('span',button).text( "Oups, d\u00e9sol\u00e9..." );
			}
...
function HideRecommendation( type, itemid, divBtn, elemContainer )
{
	...
	new Ajax.Request( 'http://store.steampowered.com/recommended/ignorerecommendation/', {
		...
		onSuccess: function( transport ) {
			...
			{
				if ( $(elemContainer) )
					$(elemContainer).update( '<div class="recommendation_ignored">D\'accord, nous vous recommanderons autre chose la prochaine fois</div>' );
			}
			else
			{
				$(divBtn).show();
...
function EnsureStoreMenuTagsLoaded( strId )
{
	...
	if ( !$Element.data('tags-loaded') )
	{
		...
		$J.get( url, {ll: 'french'} ).done( function( data ) {
			...
			if ( rgYourPopularTags.length > 0 )
			{
				...
				for( var i = 0; i < rgYourPopularTags.length && i < 5; i++ )
				{
					...
					var $Link = $J('<a/>', {'class': 'popup_menu_item', 'href': 'http://store.steampowered.com/tag/fr/' + encodeURIComponent( tag.name ) });
					$Link.text( tag.name );
					$Element.append( $Link );
				}
			}
...
function AddFreeLicense( subid, strDisplayName )
{
	...
	$J.post( 'http://store.steampowered.com/checkout/addfreelicense/' + subid, { ajax: true, sessionid: g_sessionID }).done( function() {
		ShowAlertDialog(
			strDisplayName,
			'%s a été ajouté à votre compte. Ce produit est maintenant disponible dans votre bibliothèque Steam.'.replace( /%s/, strDisplayName )
		).done( function() {
			window.location.reload();
		});
		if ( typeof GDynamicStore != 'undefined' )
		...
		if ( data && data.purchaseresultdetail == 9 )
			ShowAlertDialog( strDisplayName, 'Ce produit est déjà disponible dans votre bibliothèque Steam.' );
		else
			ShowAlertDialog( strDisplayName, 'Un problème est survenu lors de l\'ajout de ce produit à votre compte. Veuillez réessayer plus tard.' );
	}).always( function () {
...
function ChangeLanguage( strTargetLanguage, bStayOnPage )
{
	...
	.done( function() {
		...
		Modal.Dismiss();
		ShowAlertDialog( 'Changer la langue', 'Une erreur est survenue lors de l\'enregistrement de vos modifications. Veuillez réessayer plus tard.' );
	});
}
...
function FollowCuratorWithCallback( clanID, bFollow, onComplete )
{
	...
	if ( !bHaveUser )
	{
		ShowAlertDialog("Veuillez vous connecter", "Vous devez \u00eatre connect\u00e9 pour suivre un curateur");
		return;
	}
	...
	$J.post(
		...
		{
			ShowAlertDialog( 'Erreur', 'Un problème est survenu lors du suivi d\'un curateur Steam.' );
		}
	);
	return false;
}
...
function IgnoreCuratorWithCallback( clanID, bIgnore, onComplete )
{
	...
	if ( !bHaveUser )
	{
		ShowAlertDialog("Veuillez vous connecter", "Vous devez \u00eatre connect\u00e9(e) pour ignorer un curateur Steam.");
		return;
	}
	...
	$J.post(
		...
		{
			ShowAlertDialog( 'Erreur', 'Un problème s\'est produit pendant que vous essayiez d\'ignorer ce curateur.' );
		}
	);
	return false;
}
