...
GDynamicStore = {
		...
		$DynamicElements.each( function() {
			...
			if ( !$El.hasClass('ds_no_flags') )
			{
				...
				if ( bOwned )
				{
					...
					$El.append( '<div class="ds_flag ds_owned_flag">KÖNYVTÁRBAN&nbsp;&nbsp;</div>');
				}
				else if ( bWanted )
				{
					$El.addClass( 'ds_flagged ds_wishlist' );
					$El.append( '<div class="ds_flag ds_wishlist_flag">KÍVÁNSÁGLISTÁN&nbsp;&nbsp;</div>');
				}

				if ( bInCart )
				{
					$El.addClass( 'ds_flagged ds_incart' );
					$El.append( '<div class="ds_flag ds_incart_flag">KOSÁRBAN&nbsp;&nbsp;</div>');
				}
		...
		if( strAppIDs )
		{
			...
			if (!bOnWishlist)
			{
				var fnClick = function ( event )
				{
					...
					$elSource.append( '<div class="ds_flag ds_wishlist_flag">KÍVÁNSÁGLISTÁN&nbsp;&nbsp;</div>');
				...
				var strText = "K\u00edv\u00e1ns\u00e1glist\u00e1hoz ad\u00e1s";
			...
			var $elPreferences = $J ( '<a/>' ).attr('href', 'https://store.steampowered.com/account/preferences' ).text ( "Testreszab\u00e1s" ).addClass( 'option' );
	...
	UpdateDynamicBundleElements: function( Bundle, $El )
	{
		...
		if ( !Bundle.m_bIsCommercial && ( !Bundle.m_rgBundleItems.length || ( Bundle.m_bMustPurchaseAsSet && Bundle.m_cUserItemsInBundle < Bundle.m_cTotalItemsInBundle ) ) )
		{
			...
			var strTooltip = 'Ezt a csomagot nem lehet megvásárolni a fiókoddal, mert a benne szereplő tételek mindegyike megvan már neked.';

			if ( Bundle.m_bMustPurchaseAsSet && !Bundle.m_bRestrictGifting )
			{
				strTooltip = 'Ez az ajánlat csak akkor érvényes, ha egyszerre veszed meg mind a(z) %s elemet. De így is megvásárolhatod a csomagot ajándékként egy barátnak.'.replace(/%s/, Bundle.m_cTotalItemsInBundle);
				$CartBtn.find('span').text( 'Ajándékba vásárolom' );
			}
			else
			{
		...
		else if ( !Bundle.m_bMustPurchaseAsSet )
		{
			...
			if ( !Bundle.m_bContainsDiscountedPackage )
			{
				...
				$DiscountBlocks.find('.discount_final_price' ).addClass('your_price' ).empty().append($J('<div/>', {'class': 'your_price_label'} ).text('A te árad:'), $J('<div/>' ).text( strFormattedFinalPrice ) );
			}
			else
			{
		...
		if ( $El.is( '.package_totals_area' ) && !Bundle.m_bMustPurchaseAsSet )
		{
			...
			else
			{
				...
				{
					$El.find('.bundle_final_package_price_desc' ).text( 'A(z) %s tétel egyedi ára, ami még nincs meg neked:'.replace( '%s', Bundle.m_cUserItemsInBundle ) );
				}
	...
	BuildBundleDescription: function( Bundle, $Description )
	{
		if ( Bundle.m_cUserItemsInBundle == 0 )
		{
			...
			$Description.html( '<span class="collectionComplete">Teljes a gyűjtemény!</span> %1$s/%2$s tétel ebből a gyűjteményből már a könyvtáradban van.'.replace( '%1$s', Bundle.m_cTotalItemsInBundle ).replace( '%2$s', Bundle.m_cTotalItemsInBundle ) );
		}
		else if ( Bundle.m_cUserItemsInBundle < Bundle.m_cTotalItemsInBundle )
		{
			// own some but not all.
			$Description.html( '<div>%1$s/%2$s tétel ebből a csomagból már a könyvtáradban van.</div>'.replace( '%1$s', Bundle.m_cTotalItemsInBundle - Bundle.m_cUserItemsInBundle ).replace( '%2$s', Bundle.m_cTotalItemsInBundle ) );
			$Description.append( '<div>Vásárold meg ezt a csomagot, hogy %1$s%%-ot spórolj a(z) %2$s tétel árából, ami még nincs meg!</div>'.replace( '%1$s', Bundle.m_nDiscountPct ).replace( '%2$s', Bundle.m_cUserItemsInBundle ).replace( '%%', '%' ) );

			// add "complete the set" flag
			$Description.parents('.dynamic_bundle_description' ).append( $J('<div/>', {'class': 'ds_flag ds_completetheset'} ).text('TEDD TELJESSÉ GYŰJTEMÉNYED!') );
		}
		else
		{
			$Description.html( '<div>Vásárold meg ezt a csomagot, hogy %1$s%%-ot spórolj mind a(z) %2$s tétel árából!</div>'.replace( '%1$s', Bundle.m_nDiscountPct ).replace( '%2$s', Bundle.m_cTotalItemsInBundle ).replace( '%%', '%' ) );
		}
	...
	PopulateRecommendedTagList: function()
	{
		...
		for( var i = 0; i < GDynamicStore.s_rgRecommendedTags.length && i < 5; i++ )
		{
			var tag = GDynamicStore.s_rgRecommendedTags[i];
							var $Link = $J('<a/>', {'class': 'popup_menu_item', 'href': 'http://store.steampowered.com/tags/hu/' + encodeURIComponent( tag.name ) });
						$Link.text( tag.name );
			$Element.append( $Link );
		}
	},
	...
	DisplayBundleSimulator: function( unBundleID )
	{
		...
		$Form.append( $J('<h2/>').text('Jelöld be a „birtokolandó” tételeket') );
		$Form.append( $J('<div/>', {'class': 'bundle_simulator_secondary' } ).append(
			$J('<a/>', {'href': 'javascript:void(0);'} ).click( function() { $Form.find('input').prop('checked', true ); } ).text( "Összes kijelölése" ),
			' - ',
			$J('<a/>', {'href': 'javascript:void(0);'} ).click( function() { $Form.find('input').prop('checked', false ); } ).text( "Egyik sem" )
		) );
		...
		Modal = ShowConfirmDialog( '', $Form, 'Megjelenítés frissítése' ).done( function() {
...
function ShowHowDoDiscoveryQueuesWorkDialog()
{
	$J.get(
		...
		function( data )
		{
			ShowAlertDialog( 'Hogy működik?', data );
		}
	);
}
