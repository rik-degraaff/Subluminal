...
HighlightPlayer.prototype.LoadMovie = function( $Container, bUserAction )
{
	...
	else
	{
		...
		var strMessage = 'Při přehrávání tohoto videa došlo k chybě. Zkontrolujte, zda je Váš prohlížeč aktuální.';
...
HighlightPlayer.prototype.ShowScreenshotPopup = function( screenshotid )
{
	...
	var $Title = $J('<a/>' );
	if ( Steam.BIsUserInSteamClient() )
		$Title.text( 'Zobrazit v plné velikosti v prohlížeči' );
	else
		$Title.text( 'Stáhnout v plné velikosti' );
	...
	var $BtnPrev = $J('<div/>', {'class': 'btnv6_blue_hoverfade btn_medium previous'}).append( $J('<span/>').text( 'Předchozí' ) );
	var $BtnNext = $J('<div/>', {'class': 'btnv6_blue_hoverfade btn_medium next'}).append( $J('<span/>').text( 'Další' ) );
	...
	var fnUpdateFooter = function()
	{
		...
		$ScreenshotCount.text( '%1$s z %2$s snímků obrazovky'.replace( /%1\$s/, iCurIndex + 1 ).replace( /%2\$s/, rgScreenshotIDs.length ) );
	};
	...
	$.fn.videoControls = function( options ) {
		...
		var overlaySrc = '<div class="html5_video_overlay">' +
				...
				'<div class="control_container">' +
					...
					'<div class="autoplay_label">Automatické přehrávání videí</div>' +
				'</div>' +
