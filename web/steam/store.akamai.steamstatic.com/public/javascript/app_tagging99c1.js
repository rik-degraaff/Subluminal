...
function InitAppTagModal( appid, rgAppTags, rgUserTags, strTagLinkSNR, strYourTagSNR, bShowBanOption )
{
	...
	var fnSetCheckboxTooltip = function( $Checkbox )
	{
		if ( $Checkbox.hasClass('checked') )
			$Checkbox.data('store-tooltip', 'Du har lagt till denna tagg. Klicka för att ta bort.' );
		else
			$Checkbox.data('store-tooltip', 'Klicka här för att använda den här taggen.' );
	};

	var fnSetReportTooltip = function( $ReportFlag )
	{
		if ( $ReportFlag.hasClass('reported') )
			$ReportFlag.data('store-tooltip', 'Du har anmält den här taggen som inkorrekt applicerad på denna produkt. Klicka igen för att ta bort anmälningen.' );
		else
			$ReportFlag.data('store-tooltip', 'Klicka för att anmäla den här taggen som inkorrekt applicerad på denna produkt.' );
	};
	...
	var fnMakeTag = function( tagid, tag, checked, bPopular, bReported, bBrowsable )
	{
		...
		if ( bPopular )
		{
			...
			if ( $YourTags.length )
			{
				...
				if ( bShowBanOption )
				{
					...
					$Ban.data('store-tooltip', 'Klicka här för att ta bort denna tagg från produkten. Detta alternativ är begränsat till utvecklare och Steam-personal.' );
					BindStoreTooltip( $Ban );
					$Tag.append( $Ban );

					$Ban.click( function() {
						ShowConfirmDialog( 'Förbjud tagg', 'Är du säker på att du vill ta bort taggen "%s" från denna produkt?'.replace( /%s/, tag )).done( function()
						{
		...
		else
		{
			...
			$Link.data('store-tooltip', 'Inte tillräckligt många artiklar taggade med "%s" ännu'.replace( /%s/, $Link.html() ) );
	...
	{
		var $Tag = $J('<div/>', {'class': 'app_tag previous_tag', 'data-tagid': tagid, 'data-store-tooltip': 'Klicka här för att använda den här taggen.' }).text( tag ).click( function() { fnApplyTag( tag ) } );
	...
	var fnBuildTagDisplay = function( Modal )
	{
		...
		if ( rgYourPopularTags === null )
		{
			...
			if ( !bYourPopularTagsRequested )
			{
				...
				$J.get( 'http://store.steampowered.com/tagdata/populartags/swedish').done( function ( data ) {
		...
		if ( rgUserTags.length )
		{
			...
			for( var i = 0; i < rgUserTags.length; i++ )
			{
				if ( !rgUserTags[i].is_reported &&  !rgUserTags[i].browseable )
				{
					...
					$AppTag.data('store-tooltip', 'Inte tillräckligt många artiklar taggade med "%s" ännu'.replace( /%s/, $AppTag.html() ) );
	...
	window.ShowAppTagBanModal = function() {
		...
		$BanModal.find( '.app_tag_report').click( function() {
			...
			ShowConfirmDialog( 'Sluta förbjuda tagg',
				'Är du säker på att du vill sluta förbjuda taggen "%s"?'.replace( /%s/, tag ),
				'Sluta förbjuda tagg')
				.done( function() {
					var rgParams = {
						appid: appid,
