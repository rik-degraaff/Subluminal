...
function ShowEmbedWidget( )
{
	...
	var Modal = _BuildDialog( "Skap widget for innkapsling", $Content, [], fnOK, {} );
...
function ShowShareDialog( )
{
	...
	ShowAlertDialog( "Del", $Content).always(
		function() {
			// save it away again for later
			$Content.hide();
			$J(document.body).append( $Content );
...
function InitQueueControls( store_appid, steamworks_appid, next_in_queue_appid, snr )
{
	...
	$IgnoreBtn.click( function() {
		$J.post( 'http://store.steampowered.com/recommended/ignorerecommendation/', {
			...
			ShowAlertDialog( 'Ikke interessert', 'Det oppstod et problem ved lagring av dine endringer. Vennligst prøv igjen senere.' );
		});
	});
	...
	$FollowBtn.click( function() {
		$J.post( 'http://store.steampowered.com/explore/followgame/', {
			...
			ShowAlertDialog( 'Følg', 'Det oppstod et problem ved lagring av dine endringer. Vennligst prøv igjen senere.' );
		});
	});
		...
		CDiscoveryQueue.ShowCustomizeDialog( function( data ) {
				...
				( next_in_queue_appid && data.queue.indexOf( next_in_queue_appid ) == -1 ) ) )
			{
				ShowConfirmDialog( 'Customize Your Discovery Queue', 'We\'ve built you a new Discovery Queue by applying your customizations.  What would you like to do?',
					'Begynn å utforske den nye køen','Bli her'
				).done( function() {
...
function BuildReviewHistogram()
{
	...
	$J.get( 'http://store.steampowered.com/appreviewhistogram/' + appid, { l: 'norwegian' }
	).done( function( data ) {
...
function UpdateActiveFilters()
{
	...
	else if ( $J( "#purchase_type_non_steam" ).attr( "checked" ) )
	{
		bAnyActiveFilters = true;
		$J( "#reviews_filter_purchase_type" ).show();
		$J( "#reviews_filter_purchase_type" ).text( 'Ikke kjøpt på Steam' );
	}
	else
	{
	...
	if ( $J( "#review_language_mine" ).attr( "checked" ) )
	{
		...
		$J( "#reviews_filter_language" ).text( 'Dine språk' );
	}
	else
	{
...
function ShowReportDialog( nAppId )
{
	...
	content.append( $J('<div/>', {'class': 'app_report_dialog_intro' } ).text('Vennligst velg en grunn til at du rapporterer dette produktet.') );

		var rgReportOptions = {"1":"Svindel - <span class=\"sub\">Denne programvaren fors\u00f8ker uredelig \u00e5 samle sensitiv informasjon, som din Steam-legitimasjon eller \u00f8konomiske opplysninger (f.eks. kredittkortinformasjon).<\/span>","2":"Skadelig - <span class=\"sub\">Denne programvaren endrer kundens datamaskin p\u00e5 uventede eller skadelige m\u00e5ter (f.eks. malware eller et virus)<\/span>","3":"Hatefulle ytringer - <span class=\"sub\">Inneholder hatefulle ytringer, dvs. ytringer som fremmer hat, vold eller diskriminering mot grupper eller mennesker basert p\u00e5 etnisitet, religion, kj\u00f8nn, alder, funksjonshemming eller seksuell orientering<\/span>","4":"Pornografi - <span class=\"sub\">Inneholder pornografi<\/span>","5":"Voksent innhold - <span class=\"sub\">Inneholder voksent innhold som ikke er hensiktsmessig merket med aldersanbefaling<\/span>","6":"Krenkende - <span class=\"sub\">Inneholder \u00e6rekrenkelse eller nedsettende uttalelser<\/span>","7":"St\u00f8tende - <span class=\"sub\">Inneholder innhold som er \u00e5penbart st\u00f8tende eller ment for \u00e5 sjokkere eller avsky seere<\/span>","8":"Utnyttelse av barn - <span class=\"sub\">Inneholder innhold som utnytter barn p\u00e5 noe vis<\/span>","11":"Brudd p\u00e5 loven - <span class=\"sub\">Inneholder innhold som bryter lovene innenfor din jurisdiksjon<\/span>","13":"\u00d8delagt \u2013 <span class=\"sub\">Lastes ikke ned, starter ikke eller kj\u00f8rer ikke som det skal, selv p\u00e5 en maskin som m\u00f8ter minimumskravene.<\/span>"};
	...
	content.append( $J('<div/>', {'class': 'app_report_dialog_intro' } ).text('Du kan legge til ytterligere informasjon som du føler er relevant her:') );
	content.append( textArea );

	content.append( $J('<div/>', {'class': 'app_report_dialog_dmca' } ).html('Hvis du vil rapportere et brudd på opphavsretten og er rettighetshaveren, vennligst fortsett til vårt klageskjema for krenkelse av opphavsrett under DMCA <a href="http://steamcommunity.com/dmca/create/">her</a>.') );

	var dialog = ShowConfirmDialog( 'Rapporter dette produktet', content, 'Rapporter');
	...
	dialog.done( function() {
		...
		if ( eReportTypeSelected )
		{
			$J.post(
				'http://store.steampowered.com/appreport/' + nAppId + '/report/',
				{
					...
					if ( json.success == 29 )
					{
						ShowAlertDialog( 'Feil', 'Du har allerede rapportert dette produktet!' );
						gReportedApp = true;
						$J( "#ReportAppBtn").addClass( 'btn_active' );
					}
					else
					{
						ShowAlertDialog( 'Feil', 'Det oppstod et problem med å lagre rapporten din. Vennligst forsøk igjen senere.' );
					}
		...
		{
			ShowAlertDialog( 'Feil', 'Du må velge en grunn til at du rapporterer dette produktet!' );
		}
	});
}
...
{
		var $ModalContent = $J("<div class=\"gotsteamModal\">\r\n\t<div class=\"got_steam_ctn\">\r\n\t<div class=\"got_steam_box\">\r\n\t\t<h1>Har du Steam?<\/h1>\r\n\t\t<p>Du m\u00e5 ha <a href=\"http:\/\/store.steampowered.com\/about\/\">programmet Steam<\/a> installert f\u00f8r du kan installere og starte <strong class=\"gotSteam_AppName\"><\/strong>. Har du Steam installert p\u00e5 denne datamaskinen?<\/p>\r\n\t\t<div class=\"gotsteam_buttons\">\r\n\t\t\t<a class=\"gotSteam_SteamURL btn_blue leftbtn\" href=\"\">\r\n\t\t\t\t<h3>Ja, Steam er installert<\/h3>\r\n\t\t\t\t<h5 class=\"gotsteam_action\"><\/h5>\r\n\t\t\t<\/a>\r\n\t\t\t<a href=\"http:\/\/store.steampowered.com\/about\/\" class=\"btn_blue\">\r\n\t\t\t\t<h3>Nei, jeg trenger Steam<\/h3>\r\n\t\t\t\t<h5>Les om og last ned Steam<\/h5>\r\n\t\t\t<\/a>\r\n\t\t\t<div style=\"clear: left;\"><\/div>\r\n\t\t<\/div>\r\n\t\t<div class=\"got_steam_low_block\">\r\n\t\t\t<div class=\"gotsteam_steam_ico\"><img src=\"http:\/\/store.akamai.steamstatic.com\/public\/images\/v6\/steam_ico.png\" width=\"40\" height=\"40\" border=\"0\" \/><\/div>\r\n\t\t\tSteam er den beste plattformen for spill p\u00e5 skrivebordet. Det er gratis \u00e5 skaffe og lett \u00e5 bruke. <a href=\"http:\/\/store.steampowered.com\/about\/\">Les mer om Steam.<\/a>\r\n\t\t<\/div><\/div>\r\n\t<\/div>\r\n<\/div>");
