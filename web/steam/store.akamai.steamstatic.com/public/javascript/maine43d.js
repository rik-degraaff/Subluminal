...
function GameHover( elem, event, divHover, rgHoverData )
{
	...
	else if ( ( !$Hover.is(':visible') || g_HoverState.target != $Elem[0] ) && !oElemState.timer )
	{
		...
		if ( !$HoverData.length && !oElemState.bAjaxRequestMade )
		{
			...
			{
				// is cc needed?
				rgAjaxParams = { /*cc: rgHoverData['cc'],*/ l: 'bulgarian' };
			}
			if ( rgHoverData['v6'] )
			{
		...
		.done( function( data ) {
			...
			if ( data && data.success ) {
				...
				if ( data.saleTaskCompleted ) {
					NewStickerPackModal( 'Добавете към своя „Списък с желания“' );
				}
			}
			else {
	...
	$J.post( url, {sessionid: g_sessionID, appid: appid} )
		.done( function( data ) {
			if ( data && data.success )
			{
				...
				$J('span',button).text("\u0412 \u0441\u043f\u0438\u0441\u044a\u043a\u0430 \u0441 \u0436\u0435\u043b\u0430\u043d\u0438\u044f");
			}
			else
			{
				$J('span',button).text( "\u041e\u043f\u0430, \u0441\u044a\u0436\u0430\u043b\u044f\u0432\u0430\u043c\u0435!" );
			}
...
function HideRecommendation( type, itemid, divBtn, elemContainer )
{
	...
	new Ajax.Request( 'http://store.steampowered.com/recommended/ignorerecommendation/', {
		...
		onSuccess: function( transport ) {
			...
			{
				if ( $(elemContainer) )
					$(elemContainer).update( '<div class="recommendation_ignored">Добре, ще препоръчаме нещо друго следващия път.</div>' );
			}
			else
			{
				$(divBtn).show();
...
function EnsureStoreMenuTagsLoaded( strId )
{
	...
	if ( !$Element.data('tags-loaded') )
	{
		...
		$J.get( url, {ll: 'bulgarian'} ).done( function( data ) {
			...
			if ( rgYourPopularTags.length > 0 )
			{
				...
				for( var i = 0; i < rgYourPopularTags.length && i < 5; i++ )
				{
					...
					var $Link = $J('<a/>', {'class': 'popup_menu_item', 'href': 'http://store.steampowered.com/tag/bg/' + encodeURIComponent( tag.name ) });
					$Link.text( tag.name );
					$Element.append( $Link );
				}
			}
...
function AddFreeLicense( subid, strDisplayName )
{
	...
	$J.post( 'http://store.steampowered.com/checkout/addfreelicense/' + subid, { ajax: true, sessionid: g_sessionID }).done( function() {
		ShowAlertDialog(
			strDisplayName,
			'%s беше добавена към Вашия акаунт. Вече е достъпна от Steam библиотеката Ви.'.replace( /%s/, strDisplayName )
		).done( function() {
			window.location.reload();
		});
		if ( typeof GDynamicStore != 'undefined' )
		...
		if ( data && data.purchaseresultdetail == 9 )
			ShowAlertDialog( strDisplayName, 'Този продукт вече е наличен във Вашата Steam библиотека.' );
		else
			ShowAlertDialog( strDisplayName, 'Имаше проблем при добавянето на този продукт към акаунта Ви. Моля, опитайте отново по-късно.' );
	}).always( function () {
...
function ChangeLanguage( strTargetLanguage, bStayOnPage )
{
	...
	.done( function() {
		...
		Modal.Dismiss();
		ShowAlertDialog( 'Смяна на езикa', 'Имаше проблем при запазването на промените Ви. Моля, опитайте отново по-късно.' );
	});
}
...
function FollowCuratorWithCallback( clanID, bFollow, onComplete )
{
	...
	if ( !bHaveUser )
	{
		ShowAlertDialog("\u041c\u043e\u043b\u044f, \u0432\u043b\u0435\u0437\u0442\u0435", "\u0422\u0440\u044f\u0431\u0432\u0430 \u0434\u0430 \u0441\u0442\u0435 \u0432\u043b\u0435\u0437\u043b\u0438, \u0437\u0430 \u0434\u0430 \u0441\u043b\u0435\u0434\u0432\u0430\u0442\u0435 \u043a\u0443\u0440\u0430\u0442\u043e\u0440");
		return;
	}
	...
	$J.post(
		...
		{
			ShowAlertDialog( 'Грешка', 'Имаше проблем при опита да следвате Steam куратора.' );
		}
	);
	return false;
}
...
function IgnoreCuratorWithCallback( clanID, bIgnore, onComplete )
{
	...
	if ( !bHaveUser )
	{
		ShowAlertDialog("\u041c\u043e\u043b\u044f, \u0432\u043b\u0435\u0437\u0442\u0435", "\u0422\u0440\u044f\u0431\u0432\u0430 \u0434\u0430 \u0441\u0442\u0435 \u0432\u043b\u0435\u0437\u043b\u0438, \u043f\u0440\u0435\u0434\u0438 \u0434\u0430 \u0435 \u0432\u044a\u0437\u043c\u043e\u0436\u043d\u043e \u0438\u0433\u043d\u043e\u0440\u0438\u0440\u0430\u043d\u0435\u0442\u043e \u043d\u0430 Steam \u043a\u0443\u0440\u0430\u0442\u043e\u0440");
		return;
	}
	...
	$J.post(
		...
		{
			ShowAlertDialog( 'Грешка', 'Имаше проблем при опита за игнориране на този Steam куратор.' );
		}
	);
	return false;
}
