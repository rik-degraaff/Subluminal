...
function GameHover( elem, event, divHover, rgHoverData )
{
	...
	else if ( ( !$Hover.is(':visible') || g_HoverState.target != $Elem[0] ) && !oElemState.timer )
	{
		...
		if ( !$HoverData.length && !oElemState.bAjaxRequestMade )
		{
			...
			{
				// is cc needed?
				rgAjaxParams = { /*cc: rgHoverData['cc'],*/ l: 'greek' };
			}
			if ( rgHoverData['v6'] )
			{
...
function AddToWishlistButton( button, appid, navref )
{
	...
	$J.post( url, {sessionid: g_sessionID, appid: appid} )
		.done( function( data ) {
			...
			{
				$J('span',button).text( "\u039f\u03c5\u03c0\u03c2, \u03c3\u03c5\u03b3\u03b3\u03bd\u03ce\u03bc\u03b7!" );
			}
		...
		$J.get( url, {ll: 'greek'} ).done( function( data ) {
			...
			if ( rgYourPopularTags.length > 0 )
			{
				...
				for( var i = 0; i < rgYourPopularTags.length && i < 5; i++ )
				{
					...
					var $Link = $J('<a/>', {'class': 'popup_menu_item', 'href': 'http://store.steampowered.com/tag/el/' + encodeURIComponent( tag.name ) });
					$Link.text( tag.name );
					$Element.append( $Link );
				}
			}
...
function AddFreeLicense( subid, strDisplayName )
{
	...
	$J.post( 'http://store.steampowered.com/checkout/addfreelicense/' + subid, { ajax: true, sessionid: g_sessionID }).done( function() {
		...
		if ( data && data.purchaseresultdetail == 9 )
			...
			ShowAlertDialog( strDisplayName, 'Υπήρξε ένα πρόβλημα κατά τη προσθήκη αυτού του προϊόντος στον λογαριασμό σας. Παρακαλούμε δοκιμάστε ξανά αργότερα.' );
	}).always( function () {
...
function ChangeLanguage( strTargetLanguage, bStayOnPage )
{
	...
	.done( function() {
		...
		Modal.Dismiss();
		ShowAlertDialog( 'Αλλαγή γλώσσας', 'Υπήρξε ένα πρόβλημα με την αποθήκευση των αλλαγών σας. Παρακαλούμε δοκιμάστε ξανά αργότερα.' );
	});
}
...
function FollowCuratorWithCallback( clanID, bFollow, onComplete )
{
	...
	if ( !bHaveUser )
	{
		ShowAlertDialog("\u03a0\u03b1\u03c1\u03b1\u03ba\u03b1\u03bb\u03bf\u03cd\u03bc\u03b5 \u03c3\u03c5\u03bd\u03b4\u03b5\u03b8\u03b5\u03af\u03c4\u03b5", "\u03a0\u03c1\u03ad\u03c0\u03b5\u03b9 \u03bd\u03b1 \u03b5\u03af\u03c3\u03c4\u03b5 \u03c3\u03c5\u03bd\u03b4\u03b5\u03b4\u03b5\u03bc\u03ad\u03bd\u03bf\u03b9 \u03b3\u03b9\u03b1 \u03bd\u03b1 \u03b1\u03ba\u03bf\u03bb\u03bf\u03c5\u03b8\u03ae\u03c3\u03b5\u03c4\u03b5 \u03ad\u03bd\u03b1\u03bd \u03b5\u03c0\u03b9\u03bc\u03b5\u03bb\u03b7\u03c4\u03ae");
		return;
	}
	...
	$J.post(
		...
		{
			ShowAlertDialog( 'Σφάλμα', 'Υπήρξε ένα πρόβλημα στην προσπάθεια ακολούθησης του επιμελητή Steam.' );
		}
	);
	return false;
}
...
function IgnoreCuratorWithCallback( clanID, bIgnore, onComplete )
{
	...
	$J.post(
		...
		{
			ShowAlertDialog( 'Σφάλμα', 'There was a problem trying to ignore the Steam Curator.' );
		}
	);
	return false;
}
