...
function InitAppTagModal( appid, rgAppTags, rgUserTags, strTagLinkSNR, strYourTagSNR, bShowBanOption )
{
	...
	var fnSetCheckboxTooltip = function( $Checkbox )
	{
		if ( $Checkbox.hasClass('checked') )
			$Checkbox.data('store-tooltip', 'Έχετε εφαρμόσει αυτή την ετικέτα. Πατήστε εδώ για διαγραφή.' );
		else
			$Checkbox.data('store-tooltip', 'Πατήστε εδώ για εφαρμογή αυτής της ετικέτας.' );
	};

	var fnSetReportTooltip = function( $ReportFlag )
	{
		if ( $ReportFlag.hasClass('reported') )
			$ReportFlag.data('store-tooltip', 'Αναφέρατε αυτή την ετικέτα ως λανθασμένα εφαρμοσμένη σε αυτό το προϊόν. Πατήστε ξανά για να αφαιρέσετε την αναφορά.' );
		else
			$ReportFlag.data('store-tooltip', 'Πατήστε εδώ για να αναφέρετε την ετικέτα ως λανθασμένα εφαρμοσμένη σε αυτό το προϊόν.' );
	};
	...
	var fnMakeTag = function( tagid, tag, checked, bPopular, bReported, bBrowsable )
	{
		...
		if ( bPopular )
		{
			...
			if ( $YourTags.length )
			{
				...
				if ( bShowBanOption )
				{
					...
					$Ban.data('store-tooltip', 'Πατήστε εδώ για να αφαιρέσετε αυτή την ετικέτα από το προϊόν. Αυτή η επιλογή είναι διαθέσιμη μόνο σε δημιουργούς και στο προσωπικό του Steam.' );
					BindStoreTooltip( $Ban );
					$Tag.append( $Ban );

					$Ban.click( function() {
						ShowConfirmDialog( 'Αποκλεισμός ετικέτας', 'Θέλετε σίγουρα να αφαιρέσετε την ετικέτα «%s» από αυτό το προϊόν;'.replace( /%s/, tag )).done( function()
						{
		...
		else
		{
			...
			$Link.data('store-tooltip', 'Δεν υπάρχουν αρκετά αντικείμενα με την ετικέτα «%s» ακόμα'.replace( /%s/, $Link.html() ) );
	...
	{
		var $Tag = $J('<div/>', {'class': 'app_tag previous_tag', 'data-tagid': tagid, 'data-store-tooltip': 'Πατήστε εδώ για εφαρμογή αυτής της ετικέτας.' }).text( tag ).click( function() { fnApplyTag( tag ) } );
	...
	var fnBuildTagDisplay = function( Modal )
	{
		...
		if ( rgYourPopularTags === null )
		{
			...
			if ( !bYourPopularTagsRequested )
			{
				...
				$J.get( 'http://store.steampowered.com/tagdata/populartags/greek').done( function ( data ) {
		...
		else
		{
			...
			{
				$YourTags.append( $J('<div/>', {'class': 'app_tag_control no_tags_yet'}).text('Χωρίς ετικέτες ακόμα' ) );
			}
		...
		if ( rgUserTags.length )
		{
			...
			for( var i = 0; i < rgUserTags.length; i++ )
			{
				if ( !rgUserTags[i].is_reported &&  !rgUserTags[i].browseable )
				{
					...
					$AppTag.data('store-tooltip', 'Δεν υπάρχουν αρκετά αντικείμενα με την ετικέτα «%s» ακόμα'.replace( /%s/, $AppTag.html() ) );
	...
	var fnApplyTag = function( tag, withdraw, bPopularClick, eReportType, unTagID )
	{
		...
		$J.post( 'http://store.steampowered.com/tagdata/tagapp', rgParams ).done( function( data ) {
			...
			{
				ShowAlertDialog( 'Αναφορά ετικέτας', 'Σας ευχαριστούμε για την αναφορά σας. Οι ετικέτες που λαμβάνουν ένα συγκεκριμένο όριο αναφορών θα αφαιρούνται από το προϊόν.');
			}
			...
			ShowAlertDialog( 'Προβολή και επεξεργασία ετικετών για το προϊόν', 'There was a problem adding your tag.' );
		});
	};
	...
	window.ShowAppTagModal = function( appid )
	{
		...
		$AppTagModal.show();
		var Modal = ShowAlertDialog( 'Προβολή και επεξεργασία ετικετών για το προϊόν', $AppTagModal, 'Κλείσιμο' );
	...
	var fnDisplayTag = function( unTagID, strTagName ) {
		if ( !rgTagResults[unTagID] )
		{
			...
			$J.get( 'http://store.steampowered.com/tagdata/gettaggames/' + strTagLanguage + '/' + unTagID + '/', rgParams ).done( function ( html ) {
				...
				$GamesElement.find( '.browse_tag_loading').text( 'Συγγνώμη, παρουσιάστηκε ένα πρόβλημα κατά τη φόρτωση των αντικειμένων. Παρακαλούμε δοκιμάστε ξανά αργότερα.' );
			});
...
function InitBannedTagModal( appid, $BanModal )
{
	...
	window.ShowAppTagBanModal = function() {
		$BanModal.show();
		ShowAlertDialog( 'Αποκλεισμένες ετικέτες', $BanModal).always( function() {
			if ( bBansChanged )
				window.location.reload();
		} );
		...
		$BanModal.find( '.app_tag_report').click( function() {
			...
			ShowConfirmDialog( 'Άρση αποκλεισμού ετικέτας',
				'Θέλετε σίγουρα να αναιρέσετε τον αποκλεισμό της ετικέτας «%s»;'.replace( /%s/, tag ),
				'Άρση αποκλεισμού ετικέτας')
				.done( function() {
					var rgParams = {
						appid: appid,
						sessionid: g_sessionID,
					...
					$J.post( 'http://store.steampowered.com/tagdata/tagapp', rgParams ).done( function( data ) {
						...
						ShowAlertDialog( 'Άρση αποκλεισμού ετικέτας', 'Υπήρξε ένα πρόβλημα κατά τον αποκλεισμό ή την άρση αποκλεισμού αυτής της ετικέτας. Παρακαλούμε δοκιμάστε ξανά αργότερα.' );
					});
					bBansChanged = true;
				});
		});
