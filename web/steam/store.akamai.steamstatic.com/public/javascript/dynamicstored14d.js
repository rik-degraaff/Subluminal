...
GDynamicStore = {
	...
	CapsuleSettingsMenu: function( elSource )
	{
		...
		if( strAppIDs )
		{
			...
			if (!bOnWishlist)
			{
				...
				var strText = "Zur Wunschliste hinzuf\u00fcgen";
			...
			if (!bIgnored)
			{
				...
				var strText = "Nicht interessiert";
	...
	UpdateDynamicBundleElements: function( Bundle, $El )
	{
		...
		if ( !Bundle.m_bIsCommercial && ( !Bundle.m_rgBundleItems.length || ( Bundle.m_bMustPurchaseAsSet && Bundle.m_cUserItemsInBundle < Bundle.m_cTotalItemsInBundle ) ) )
		{
			...
			var strTooltip = 'Sie können dieses Paket nicht mit Ihrem Account kaufen, da dieser bereits alle im Paket enthaltenen Spiele besitzt.';

			if ( Bundle.m_bMustPurchaseAsSet && !Bundle.m_bRestrictGifting )
			{
				strTooltip = 'Dieses Angebot ist nur gültig, wenn Sie alle %s Artikel gleichzeitig kaufen. Sie können das Paket trotzdem als Geschenk für einen Freund kaufen.'.replace(/%s/, Bundle.m_cTotalItemsInBundle);
	...
	BuildBundleDescription: function( Bundle, $Description )
	{
		...
		else if ( Bundle.m_cUserItemsInBundle < Bundle.m_cTotalItemsInBundle )
		{
			...
			$Description.html( '<div>%1$s von %2$s Artikeln in diesem Paket befinden sich bereits in Ihrer Bibliothek.</div>'.replace( '%1$s', Bundle.m_cTotalItemsInBundle - Bundle.m_cUserItemsInBundle ).replace( '%2$s', Bundle.m_cTotalItemsInBundle ) );
			$Description.append( '<div>Kaufen Sie dieses Paket, um %1$s %% von allen %2$s Artikeln, die Sie noch nicht besitzen, zu sparen!</div>'.replace( '%1$s', Bundle.m_nDiscountPct ).replace( '%2$s', Bundle.m_cUserItemsInBundle ).replace( '%%', '%' ) );
		...
		{
			$Description.html( '<div>Kaufen Sie dieses Paket, um %1$s% bei %2$s Artikeln zu sparen!</div>'.replace( '%1$s', Bundle.m_nDiscountPct ).replace( '%2$s', Bundle.m_cTotalItemsInBundle ).replace( '%%', '%' ) );
		}
	...
	PopulateRecommendedTagList: function()
	{
		...
		for( var i = 0; i < GDynamicStore.s_rgRecommendedTags.length && i < 5; i++ )
		{
			var tag = GDynamicStore.s_rgRecommendedTags[i];
							var $Link = $J('<a/>', {'class': 'popup_menu_item', 'href': 'http://store.steampowered.com/tags/de/' + encodeURIComponent( tag.name ) });
						$Link.text( tag.name );
			$Element.append( $Link );
		}
	},
	...
	DisplayBundleSimulator: function( unBundleID )
	{
		...
		$Form.append( $J('<h2/>').text('Wählen Sie Produkte aus, um Sie zu "besitzen"') );
