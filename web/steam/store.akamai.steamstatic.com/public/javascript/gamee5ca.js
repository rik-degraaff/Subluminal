...
function ShowShareDialog( )
{
	...
	ShowAlertDialog( "Teilen", $Content).always(
		function() {
			// save it away again for later
			$Content.hide();
			$J(document.body).append( $Content );
...
function InitQueueControls( store_appid, steamworks_appid, next_in_queue_appid, snr )
{
	...
	$IgnoreBtn.click( function() {
		$J.post( 'http://store.steampowered.com/recommended/ignorerecommendation/', {
			...
			ShowAlertDialog( 'Nicht interessiert', 'Beim Speichern der Änderungen ist ein Fehler aufgetreten. Bitte versuchen Sie es später erneut.' );
		});
	});
...
function UserReviewVoteUp( id )
{
	UserReview_Rate( id, true, 'http://store.steampowered.com/',
		function( rgResults ) {
			if ( rgResults.nSaleTaskCompleted ) { NewStickerPackModal( 'Ein Review als hilfreich bewerten... oder nicht hilfreich');}
...
function BuildReviewHistogram()
{
	...
	$J.get( 'http://store.steampowered.com/appreviewhistogram/' + appid, { l: 'german' }
	).done( function( data ) {
		...
		var seriesRollup = [ { label: "Positiv", color: "#66c0f4", fillColor: "#66c0f4", data: chartDataPositive }, { label: "Negativ", color: "#A34C25", fillColor: "#A34C25", data: chartDataNegative } ];
		...
		if ( data.results.recent )
		{
			...
			seriesRecent = [ { color: "#66c0f4", label: "Positiv", data: chartDataPositive }, { color: "#A34C25", label: "Negativ", data: chartDataNegative } ];
		}
...
function ShowReportDialog( nAppId )
{
	...
	content.append( $J('<div/>', {'class': 'app_report_dialog_intro' } ).text('Bitte wählen Sie einen Grund für die Meldung dieses Produkts aus.') );
		...
		var rgReportOptions = {"1":"Betrug \u2013 <span class=\"sub\">Diese Software versucht, pers\u00f6nliche Daten zu ermitteln, wie etwa die Zugangsdaten Ihres Steam-Accounts oder finanzielle Informationen (z.&nbsp;B. Kreditkarteninformationen).<\/span>","2":"Sch\u00e4dlich \u2013 <span class=\"sub\">Diese Software modifiziert den Computer eines Kunden auf unerwartete oder sch\u00e4dliche Art (enth\u00e4lt z.&nbsp;B. Malware oder Viren).<\/span>","3":"Hassreden \u2013 <span class=\"sub\">Enth\u00e4lt Hassreden, d.&nbsp;h. Reden, die Hass, Gewalt oder Diskriminierung von Gruppen oder Personen aufgrund von ethnischer Herkunft, Religion, Geschlecht, Alter, Behinderung oder sexueller Orientierung f\u00f6rdert.<\/span>","4":"Pornographie \u2013 <span class=\"sub\">Enth\u00e4lt Pornographie<\/span>","5":"Nicht jugendfreie Inhalte \u2013 <span class=\"sub\">Enth\u00e4lt nicht jugendfreie Inhalte, die nicht durch ad\u00e4quate Altersbeschr\u00e4nkungen gekennzeichnet sind.<\/span>","6":"Diffamierung \u2013 <span class=\"sub\">Enth\u00e4lt beleidigende oder verleumderische Aussagen.<\/span>","7":"Anst\u00f6\u00dfig \u2013 <span class=\"sub\">Enth\u00e4lt Inhalte, die offensichtlich anst\u00f6\u00dfig sind oder beabsichtigen, Zuschauer zu schockieren oder anzuwidern.<\/span>","8":"Kindesmisshandlung \u2013 <span class=\"sub\">Enth\u00e4lt Inhalte, die Kindesmisshandlung jeglicher Art umfassen.<\/span>","11":"Rechtsverletzung \u2013 <span class=\"sub\">Enth\u00e4lt Inhalte, die die Gesetze in Ihrem Land verletzen.<\/span>","13":"Fehlerhaft \u2013 <span class=\"sub\">Kann nicht heruntergeladen, gestartet oder richtig ausgef\u00fchrt werden, auch wenn die Mindestsystemanforderungen vom Computer erf\u00fcllt werden.<\/span>"};
	...
	content.append( $J('<div/>', {'class': 'app_report_dialog_dmca' } ).html('Falls Sie eine Urheberrechtsverletzung melden möchten und selbst der Rechteinhaber sind, fahren Sie bitte mit <a href="http://steamcommunity.com/dmca/create/">dieser</a> DMCA-konformen Benachrichtigung über eine Urheberrechtsverletzung fort.') );
...
{
		var $ModalContent = $J("<div class=\"gotsteamModal\">\r\n\t<div class=\"got_steam_ctn\">\r\n\t<div class=\"got_steam_box\">\r\n\t\t<h1>Ist Steam auf Ihrem Computer installiert?<\/h1>\r\n\t\t<p>Die <a href=\"http:\/\/store.steampowered.com\/about\/\">Steam-Desktopanwendung<\/a> muss installiert sein, bevor Sie <strong class=\"gotSteam_AppName\"><\/strong> installieren und starten k\u00f6nnen. Haben Sie Steam auf diesem Computer installiert?<\/p>\r\n\t\t<div class=\"gotsteam_buttons\">\r\n\t\t\t<a class=\"gotSteam_SteamURL btn_blue leftbtn\" href=\"\">\r\n\t\t\t\t<h3>Ja, Steam ist installiert<\/h3>\r\n\t\t\t\t<h5 class=\"gotsteam_action\"><\/h5>\r\n\t\t\t<\/a>\r\n\t\t\t<a href=\"http:\/\/store.steampowered.com\/about\/\" class=\"btn_blue\">\r\n\t\t\t\t<h3>Nein, bitte herunterladen<\/h3>\r\n\t\t\t\t<h5>Steam herunterladen und installieren<\/h5>\r\n\t\t\t<\/a>\r\n\t\t\t<div style=\"clear: left;\"><\/div>\r\n\t\t<\/div>\r\n\t\t<div class=\"got_steam_low_block\">\r\n\t\t\t<div class=\"gotsteam_steam_ico\"><img src=\"http:\/\/store.akamai.steamstatic.com\/public\/images\/v6\/steam_ico.png\" width=\"40\" height=\"40\" border=\"0\" \/><\/div>\r\n\t\t\tSteam ist die f\u00fchrende Desktop-Spieleplattform. Steam ist kostenlos und einfach zu bedienen. <a href=\"http:\/\/store.steampowered.com\/about\/\">Erfahren Sie mehr \u00fcber Steam.<\/a>\r\n\t\t<\/div><\/div>\r\n\t<\/div>\r\n<\/div>");
...
{
	var strTemplate = "<div class=\"recommended_more_info_modal\">\r\n\t\t\t\t\t\t\t\t<p class=\"intro\">Sie haben etwas gefunden, das keinem der Dinge, die Sie in der Vergangenheit genutzt haben, \u00e4hnelt. Steam wird \u00fcber die Spiele, die Sie spielen, die Filme, die Sie schauen, und die Software, die Sie auf Steam nutzen, Ihre Pr\u00e4ferenzen kennenlernen. Wir werden diese Aktivit\u00e4ten nutzen, um die Empfehlungen an Ihren Geschmack anzupassen.<\/p>\r\n\t\t\t\t\t\t\t\t<h2>Gef\u00e4llt Ihnen dieser Artikel?<\/h2>\r\n\t\t\t\t\t\t\t\t<p>Wenn Ihnen dieser Artikel gef\u00e4llt und ihn dann kaufen und spielen, werden wir das f\u00fcr zuk\u00fcnfigte Empfehlungen ber\u00fccksichtigen<\/p>\r\n\t\t\t\t\t\t\t\t<h2>Nicht an solchen Artikeln interessiert?<\/h2>\r\n\t\t\t\t\t\t\t\t<p>Sie k\u00f6nnen Ihre Shoppr\u00e4ferenzen au\u00dferdem selbst konfigurieren, um Steam \u00fcber Tags oder Produkttypen, die Sie nicht interessieren, zu informieren. <a href=\"https:\/\/store.steampowered.com\/account\/preferences\/\">Shoppr\u00e4ferenzen anzeigen<\/a>.<\/p>\r\n\t\t\t\t\t\t\t<\/div>";
