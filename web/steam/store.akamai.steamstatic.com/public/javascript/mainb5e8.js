...
function GameHover( elem, event, divHover, rgHoverData )
{
	...
	else if ( ( !$Hover.is(':visible') || g_HoverState.target != $Elem[0] ) && !oElemState.timer )
	{
		...
		if ( !$HoverData.length && !oElemState.bAjaxRequestMade )
		{
			...
			{
				// is cc needed?
				rgAjaxParams = { /*cc: rgHoverData['cc'],*/ l: 'norwegian' };
			}
			if ( rgHoverData['v6'] )
			{
	...
	$J.post( url, {sessionid: g_sessionID, appid: appid} )
		.done( function( data ) {
			if ( data && data.success )
			{
				...
				$J('span',button).text("P\u00e5 \u00f8nskelisten");
			}
			else
			{
				$J('span',button).text( "Oops, unnskyld!" );
			}
...
function HideRecommendation( type, itemid, divBtn, elemContainer )
{
	...
	new Ajax.Request( 'http://store.steampowered.com/recommended/ignorerecommendation/', {
		...
		onSuccess: function( transport ) {
			...
			{
				if ( $(elemContainer) )
					$(elemContainer).update( '<div class="recommendation_ignored">Ok, vi vil anbefale noe annet her neste gang</div>' );
			}
			else
			{
				$(divBtn).show();
...
function EnsureStoreMenuTagsLoaded( strId )
{
	...
	if ( !$Element.data('tags-loaded') )
	{
		...
		$J.get( url, {ll: 'norwegian'} ).done( function( data ) {
			...
			if ( rgYourPopularTags.length > 0 )
			{
				...
				for( var i = 0; i < rgYourPopularTags.length && i < 5; i++ )
				{
					...
					var $Link = $J('<a/>', {'class': 'popup_menu_item', 'href': 'http://store.steampowered.com/tag/no/' + encodeURIComponent( tag.name ) });
					$Link.text( tag.name );
					$Element.append( $Link );
				}
			}
...
function AddFreeLicense( subid, strDisplayName )
{
	...
	$J.post( 'http://store.steampowered.com/checkout/addfreelicense/' + subid, { ajax: true, sessionid: g_sessionID }).done( function() {
		ShowAlertDialog(
			strDisplayName,
			'%s har blitt lagt til i kontoen din. Den finnes nå tilgjengelig i ditt Steam-bibliotek.'.replace( /%s/, strDisplayName )
		).done( function() {
			window.location.reload();
		});
		if ( typeof GDynamicStore != 'undefined' )
		...
		if ( data && data.purchaseresultdetail == 9 )
			ShowAlertDialog( strDisplayName, 'Dette produktet er allerede tilgjengelig i ditt Steam-bibliotek.' );
		else
			ShowAlertDialog( strDisplayName, 'Det oppstod et problem med å legge til dette produktet i kontoen din. Prøv igjen senere.' );
	}).always( function () {
...
function ChangeLanguage( strTargetLanguage, bStayOnPage )
{
	...
	.done( function() {
		...
		Modal.Dismiss();
		ShowAlertDialog( 'Bytt språk', 'Det oppstod et problem ved lagring av dine endringer. Vennligst prøv igjen senere.' );
	});
}
...
function FollowCuratorWithCallback( clanID, bFollow, onComplete )
{
	...
	$J.post(
		...
		{
			ShowAlertDialog( 'Feil', 'Det oppstod et problem med å følge Steam-kuratoren.' );
		}
	);
	return false;
}
...
function IgnoreCuratorWithCallback( clanID, bIgnore, onComplete )
{
	...
	$J.post(
		...
		{
			ShowAlertDialog( 'Feil', 'There was a problem trying to ignore the Steam Curator.' );
		}
	);
	return false;
}
