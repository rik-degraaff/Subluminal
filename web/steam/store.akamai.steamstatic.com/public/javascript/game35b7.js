...
function ShowShareDialog( )
{
	...
	ShowAlertDialog( "Compartir", $Content).always(
		function() {
			// save it away again for later
			$Content.hide();
			$J(document.body).append( $Content );
...
function InitQueueControls( store_appid, steamworks_appid, next_in_queue_appid, snr )
{
	...
	$IgnoreBtn.click( function() {
		$J.post( 'http://store.steampowered.com/recommended/ignorerecommendation/', {
			...
			ShowAlertDialog( 'No me interesa', 'Ha habido un problema al guardar tus cambios. Por favor, inténtalo de nuevo más tarde.' );
		});
	});
	...
	$FollowBtn.click( function() {
		$J.post( 'http://store.steampowered.com/explore/followgame/', {
			...
			ShowAlertDialog( 'Seguir', 'Ha habido un problema al guardar tus cambios. Por favor, inténtalo de nuevo más tarde.' );
		});
	});
...
function UserReviewVoteUp( id )
{
	UserReview_Rate( id, true, 'http://store.steampowered.com/',
		function( rgResults ) {
			if ( rgResults.nSaleTaskCompleted ) { NewStickerPackModal( 'Indica si un análisis ha sido útil... o no');}
...
function BuildReviewHistogram()
{
	...
	$J.get( 'http://store.steampowered.com/appreviewhistogram/' + appid, { l: 'spanish' }
	).done( function( data ) {
		...
		var seriesRollup = [ { label: "Positivos", color: "#66c0f4", fillColor: "#66c0f4", data: chartDataPositive }, { label: "Negativos", color: "#A34C25", fillColor: "#A34C25", data: chartDataNegative } ];
		...
		if ( data.results.recent )
		{
			...
			seriesRecent = [ { color: "#66c0f4", label: "Positivos", data: chartDataPositive }, { color: "#A34C25", label: "Negativos", data: chartDataNegative } ];
		}
...
function UpdateActiveFilters()
{
	...
	if ( $J( "#review_type_positive" ).attr( "checked" ) )
	{
		...
		$J( "#reviews_filter_type" ).text( 'Positivos' );
	}
	else if ( $J( "#review_type_negative" ).attr( "checked" ) )
	{
		bAnyActiveFilters = true;
		$J( "#reviews_filter_type" ).show();
		$J( "#reviews_filter_type" ).text( 'Negativos' );
	}
	else
	{
...
function ShowReportDialog( nAppId )
{
	...
	content.append( $J('<div/>', {'class': 'app_report_dialog_intro' } ).text('Por favor, elige un motivo por el que estás denunciando este producto.') );

		var rgReportOptions = {"1":"Fraude - <span class=\"sub\">Este software intenta recabar, de forma fraudulenta, informaci\u00f3n confidencial como tus credenciales de Steam o datos financieros (por ejemplo, informaci\u00f3n de tarjetas de cr\u00e9dito).<\/span>","2":"Da\u00f1ino - <span class=\"sub\">Este software modifica el equipo de un cliente de una forma no esperada o da\u00f1ina (por ejemplo, es malware o un virus)<\/span>","3":"Discurso de odio - <span class=\"sub\">Incluye discurso de odio, es decir, comunicaci\u00f3n que promueve el odio, la violencia o la discriminaci\u00f3n de grupos o personas por motivos de etnia, religi\u00f3n, g\u00e9nero, edad, discapacidad u orientaci\u00f3n sexual<\/span>","4":"Pornograf\u00eda - <span class=\"sub\">Contiene pornograf\u00eda<\/span>","5":"Contenido para adultos - <span class=\"sub\"> Incluye contenido para adultos que no est\u00e1 etiquetado y restringido por edad de forma apropiada<\/span>","6":"Difamatorio - <span class=\"sub\">Incluye declaraciones difamatorias o calumniosas<\/span>","7":"Ofensivo - <span class=\"sub\">Incluye contenido claramente ofensivo o cuya intenci\u00f3n es conmocionar o provocar asco a los espectadores<\/span>","8":"Explotaci\u00f3n infantil - <span class=\"sub\">Incluye contenido que de alg\u00fan modo se aprovecha de los ni\u00f1os<\/span>","11":"Violaci\u00f3n de la ley - <span class=\"sub\">Incluye contenido que viola las leyes en tu jurisdicci\u00f3n<\/span>","13":"No funciona - <span class=\"sub\">No descarga, se inicia o se ejecuta correctamente, incluso en un equipo que cumple los requisitos m\u00ednimos de sistema.<\/span>"};
	...
	content.append( $J('<div/>', {'class': 'app_report_dialog_dmca' } ).html('Si quieres denunciar una infracción de los derechos de autor y eres el propietario de los mismos, rellena el formulario de aviso de vulneración de copyright de la DMCA, disponible <a href="http://steamcommunity.com/dmca/create/">aquí</a>.') );
	...
	dialog.done( function() {
		...
		{
			ShowAlertDialog( 'Error', '¡Debes elegir un motivo por el que estás denunciando este producto!' );
		}
	});
}
...
{
		var $ModalContent = $J("<div class=\"gotsteamModal\">\r\n\t<div class=\"got_steam_ctn\">\r\n\t<div class=\"got_steam_box\">\r\n\t\t<h1>\u00bfTienes Steam?<\/h1>\r\n\t\t<p>Debes tener la <a href=\"http:\/\/store.steampowered.com\/about\/\">aplicaci\u00f3n de escritorio de Steam<\/a> instalada antes de que puedas instalar e iniciar <strong class=\"gotSteam_AppName\"><\/strong>. \u00bfEst\u00e1 Steam instalado en este equipo?<\/p>\r\n\t\t<div class=\"gotsteam_buttons\">\r\n\t\t\t<a class=\"gotSteam_SteamURL btn_blue leftbtn\" href=\"\">\r\n\t\t\t\t<h3>S\u00ed, Steam est\u00e1 instalado<\/h3>\r\n\t\t\t\t<h5 class=\"gotsteam_action\"><\/h5>\r\n\t\t\t<\/a>\r\n\t\t\t<a href=\"http:\/\/store.steampowered.com\/about\/\" class=\"btn_blue\">\r\n\t\t\t\t<h3>No, necesito Steam<\/h3>\r\n\t\t\t\t<h5>M\u00e1s informaci\u00f3n y descarga de Steam<\/h5>\r\n\t\t\t<\/a>\r\n\t\t\t<div style=\"clear: left;\"><\/div>\r\n\t\t<\/div>\r\n\t\t<div class=\"got_steam_low_block\">\r\n\t\t\t<div class=\"gotsteam_steam_ico\"><img src=\"http:\/\/store.akamai.steamstatic.com\/public\/images\/v6\/steam_ico.png\" width=\"40\" height=\"40\" border=\"0\" \/><\/div>\r\n\t\t\tSteam es la principal plataforma de videojuegos para ordenador. Unirse es gratis y su uso, sencillo. <a href=\"http:\/\/store.steampowered.com\/about\/\">Obt\u00e9n m\u00e1s informaci\u00f3n sobre Steam.<\/a>\r\n\t\t<\/div><\/div>\r\n\t<\/div>\r\n<\/div>");
