...
function ShowEmbedWidget( )
{
	...
	var Modal = _BuildDialog( "Be\u00e1gyazhat\u00f3 widget l\u00e9trehoz\u00e1sa", $Content, [], fnOK, {} );
...
function ShowShareDialog( )
{
	...
	ShowAlertDialog( "Megoszt\u00e1s", $Content).always(
		function() {
			// save it away again for later
			$Content.hide();
			$J(document.body).append( $Content );
...
function InitQueueControls( store_appid, steamworks_appid, next_in_queue_appid, snr )
{
	...
	$IgnoreBtn.click( function() {
		$J.post( 'http://store.steampowered.com/recommended/ignorerecommendation/', {
			...
			if ( data && data.nSaleTaskCompleted ) { NewStickerPackModal( 'Jelölj valamit „Nem érdekellel”' ); } // SummerSale2017
		}).fail( function( jqXHR ) {
			ShowAlertDialog( 'Nem érdekel', 'Probléma adódott változtatásaid mentése során. Próbáld újra később.' );
		});
	});
	...
	$FollowBtn.click( function() {
		$J.post( 'http://store.steampowered.com/explore/followgame/', {
			...
			ShowAlertDialog( 'Követés', 'Probléma adódott változtatásaid mentése során. Próbáld újra később.' );
		});
	});
	...
	$J('a.dq_settings_link').click( function() {
		CDiscoveryQueue.ShowCustomizeDialog( function( data ) {
			...
			{
				ShowConfirmDialog( 'Szabd testre a Felfedezési Várólistád', 'Új Felfedezési várólistát készítettünk neked, beállításaidat használva. Mit szeretnél tenni?',
					'Új várólista felfedezésének megkezdése','Maradok itt'
				).done( function() {
...
function InitAutocollapse()
{
	$J('.game_page_autocollapse').each( function() {
		...
		var $ReadMore = $J('<div/>', {'class': 'game_page_autocollapse_readmore' }).text( 'TOVÁBB' );
...
function UserReviewVoteUp( id )
{
	UserReview_Rate( id, true, 'http://store.steampowered.com/',
		function( rgResults ) {
			if ( rgResults.nSaleTaskCompleted ) { NewStickerPackModal( 'Jelölj egy értékelést hasznosnak... vagy bárminek...');}
...
function BuildReviewHistogram()
{
	...
	$J.get( 'http://store.steampowered.com/appreviewhistogram/' + appid, { l: 'hungarian' }
	).done( function( data ) {
		...
		var seriesRollup = [ { label: "Pozitív", color: "#66c0f4", fillColor: "#66c0f4", data: chartDataPositive }, { label: "Negatív", color: "#A34C25", fillColor: "#A34C25", data: chartDataNegative } ];
		...
		if ( data.results.recent )
		{
			...
			seriesRecent = [ { color: "#66c0f4", label: "Pozitív", data: chartDataPositive }, { color: "#A34C25", label: "Negatív", data: chartDataNegative } ];
		}
...
function UpdateActiveFilters()
{
	...
	if ( $J( "#review_type_positive" ).attr( "checked" ) )
	{
		...
		$J( "#reviews_filter_type" ).text( 'Pozitív' );
	}
	else if ( $J( "#review_type_negative" ).attr( "checked" ) )
	{
		bAnyActiveFilters = true;
		$J( "#reviews_filter_type" ).show();
		$J( "#reviews_filter_type" ).text( 'Negatív' );
	}
	else
	{
	...
	if ( $J( "#purchase_type_steam" ).attr( "checked" ) )
	{
		...
		$J( "#reviews_filter_purchase_type" ).text( 'Steames vásárlók' );
	}
	else if ( $J( "#purchase_type_non_steam" ).attr( "checked" ) )
	{
		bAnyActiveFilters = true;
		$J( "#reviews_filter_purchase_type" ).show();
		$J( "#reviews_filter_purchase_type" ).text( 'Nem a Steamen vásárolva' );
	}
	else
	{
	...
	if ( $J( "#review_language_mine" ).attr( "checked" ) )
	{
		...
		$J( "#reviews_filter_language" ).text( 'Nyelveid' );
	}
	else
	{
	...
	if ( $J( "#review_date_range_histogram" ).attr( "checked" ) )
	{
		...
		$J( "#review_selected_histogram_date_range_prefix" ).text( 'Csak ' );
	}
	else if ( $J( "#review_date_range_exclude_histogram" ).attr( "checked" ) )
	{
		bAnyActiveFilters = true;
		$J( "#reviews_filter_graph" ).show();
		$J( "#review_selected_histogram_date_range_prefix" ).text( 'Kivéve ' );
	}
	else
	{
...
function ShowReportDialog( nAppId )
{
	...
	content.append( $J('<div/>', {'class': 'app_report_dialog_intro' } ).text('Kérjük, válassz egy okot, amiért jelented ezt a terméket.') );

		var rgReportOptions = {"1":"Csal\u00e1s - <span class=\"sub\">Ez a szoftver csal\u00e1rdul megk\u00eds\u00e9rel olyan \u00e9rz\u00e9keny inform\u00e1ci\u00f3katt gy\u0171jteni, mint a Steames fi\u00f3kinform\u00e1ci\u00f3id, vagy p\u00e9nz\u00fcgyi adataid (pl. bankk\u00e1rtya-inform\u00e1ci\u00f3id).<\/span>","2":"\u00c1rtalmas - <span class=\"sub\">Ez a szoftver v\u00e1ratlanul vagy k\u00e1rosan m\u00f3dos\u00edtja egy v\u00e1s\u00e1rl\u00f3 sz\u00e1m\u00edt\u00f3g\u00e9p\u00e9t (pl. rosszindulat\u00fa szoftver vagy v\u00edrus).<\/span>","3":"Gy\u0171l\u00f6letkelt\u00e9s - <span class=\"sub\">Gy\u0171l\u00f6letkelt\u00e9st tartalmaz, p\u00e9ld\u00e1ul gy\u0171l\u00f6letet, er\u0151szakot, vagy megk\u00fcl\u00f6nb\u00f6ztet\u00e9st sz\u00edt csoportokkal vagy emberekkel szemben etnikai hovatartoz\u00e1s, vall\u00e1s, nem, kor, fogyat\u00e9koss\u00e1g, vagy szexu\u00e1lis orient\u00e1ci\u00f3 alapj\u00e1n.<\/span>","4":"Pornogr\u00e1fia - <span class=\"sub\">Pornogr\u00e1f tartalom.<\/span>","5":"Feln\u0151tt tartalom - <span class=\"sub\">Olyan feln\u0151tt tartalom, amely nincs megfelel\u0151en jel\u00f6lve \u00e9s korhat\u00e1ros\u00edtva.<\/span>","6":"R\u00e1galmaz\u00e1s - <span class=\"sub\">Becs\u00fclets\u00e9rt\u0151 vagy r\u00e1galmaz\u00f3 kijelent\u00e9seket tartalmaz.<\/span>","7":"S\u00e9rt\u0151 - <span class=\"sub\">Nyilv\u00e1nval\u00f3an s\u00e9rt\u0151, illetve megbotr\u00e1nkoztat\u00e1s vagy undorkelt\u00e9s sz\u00e1nd\u00e9k\u00e1val k\u00f6zz\u00e9tett tartalom.<\/span>","8":"Gyerekek kihaszn\u00e1l\u00e1sa - <span class=\"sub\">Olyan tartalom, amely b\u00e1rmely m\u00f3don kihaszn\u00e1lja a gyerekeket.<\/span>","11":"Jogs\u00e9rt\u0151 - <span class=\"sub\">Olyan tartalmat tartalmaz, mely a te joghat\u00f3s\u00e1gi ter\u00fcleteden t\u00f6rv\u00e9nys\u00e9rt\u0151.<\/span>","13":"Hib\u00e1s - <span class=\"sub\">Nem t\u00f6lt\u0151d\u00f6tt le, indult el vagy futott megfelel\u0151en, m\u00e9g a minim\u00e1lis rendszerk\u00f6vetelm\u00e9nyeket teljes\u00edt\u0151 g\u00e9pen sem.<\/span>"};
	...
	content.append( $J('<div/>', {'class': 'app_report_dialog_intro' } ).text('Hozzáadhatsz további információt, amiről úgy érzed, ide tartozik:') );
	content.append( textArea );

	content.append( $J('<div/>', {'class': 'app_report_dialog_dmca' } ).html('Ha te vagy a jogtulajdonos, és szerzőijog-sértést szeretnél bejelenteni, lépj tovább a DMCA-nak megfelelő szerzőijog-sértési űrlapunkra <a href="http://steamcommunity.com/dmca/create/">itt</a>.') );

	var dialog = ShowConfirmDialog( 'Termék jelentése', content, 'Jelentés');
	...
	dialog.done( function() {
		...
		if ( eReportTypeSelected )
		{
			$J.post(
				'http://store.steampowered.com/appreport/' + nAppId + '/report/',
				{
					...
					if ( json.success == 29 )
					{
						ShowAlertDialog( 'Hiba', 'Már jelentetted ezt a terméket!' );
						gReportedApp = true;
						$J( "#ReportAppBtn").addClass( 'btn_active' );
					}
					else
		...
		{
			ShowAlertDialog( 'Hiba', 'Ki kell választanod egy okot, amiért jelented ezt a terméket!' );
		}
	});
}
...
{
		var $ModalContent = $J("<div class=\"gotsteamModal\">\r\n\t<div class=\"got_steam_ctn\">\r\n\t<div class=\"got_steam_box\">\r\n\t\t<h1>Van Steamed?<\/h1>\r\n\t\t<p>Sz\u00fcks\u00e9ged van a telep\u00edtett <a href=\"http:\/\/store.steampowered.com\/about\/\">Steam alkalmaz\u00e1sra<\/a>, miel\u0151tt telep\u00edthetn\u00e9d \u00e9s elind\u00edthatn\u00e1d a(z) <strong class=\"gotSteam_AppName\"><\/strong> term\u00e9ket. Telep\u00edtve van a Steam ezen a sz\u00e1m\u00edt\u00f3g\u00e9pen?<\/p>\r\n\t\t<div class=\"gotsteam_buttons\">\r\n\t\t\t<a class=\"gotSteam_SteamURL btn_blue leftbtn\" href=\"\">\r\n\t\t\t\t<h3>Igen, telep\u00edtve van a Steam<\/h3>\r\n\t\t\t\t<h5 class=\"gotsteam_action\"><\/h5>\r\n\t\t\t<\/a>\r\n\t\t\t<a href=\"http:\/\/store.steampowered.com\/about\/\" class=\"btn_blue\">\r\n\t\t\t\t<h3>Nincs, sz\u00fcks\u00e9gem van a Steamre<\/h3>\r\n\t\t\t\t<h5>Olvass a Steamr\u0151l, majd t\u00f6ltsd le<\/h5>\r\n\t\t\t<\/a>\r\n\t\t\t<div style=\"clear: left;\"><\/div>\r\n\t\t<\/div>\r\n\t\t<div class=\"got_steam_low_block\">\r\n\t\t\t<div class=\"gotsteam_steam_ico\"><img src=\"http:\/\/store.akamai.steamstatic.com\/public\/images\/v6\/steam_ico.png\" width=\"40\" height=\"40\" border=\"0\" \/><\/div>\r\n\t\t\tA Steam az els\u0151 sz\u00e1m\u00fa asztali j\u00e1t\u00e9kplatform. Ingyenes csatlakoz\u00e1s, k\u00f6nny\u0171 haszn\u00e1lat. <a href=\"http:\/\/store.steampowered.com\/about\/\">Tudj meg t\u00f6bbet a Steamr\u0151l.<\/a>\r\n\t\t<\/div><\/div>\r\n\t<\/div>\r\n<\/div>");
...
{
	var strTemplate = "<div class=\"recommended_more_info_modal\">\r\n\t\t\t\t\t\t\t\t<p class=\"intro\">Valami olyat tal\u00e1lt\u00e1l, ami nem hasonl\u00edt azokra, amiket kor\u00e1bban haszn\u00e1lt\u00e1l. A Steam abb\u00f3l, hogy milyen j\u00e1t\u00e9kokat j\u00e1tszol, milyen filmeket n\u00e9zel \u00e9s milyen szoftvereket haszn\u00e1lsz a Steamen, tanul arr\u00f3l, hogy mit r\u00e9szes\u00edtesz el\u0151nyben. Ezt az aktivit\u00e1st arra fogjuk haszn\u00e1lni, hogy aj\u00e1nl\u00e1saidat \u00edzl\u00e9sedre szabjuk.<\/p>\r\n\t\t\t\t\t\t\t\t<h2>Tetszik ez az elem?<\/h2>\r\n\t\t\t\t\t\t\t\t<p>Ha tetszik ez az elem, \u00e9s megveszed hogy j\u00e1tssz vele, figyelembe fogjuk azt venni, amikor a j\u00f6v\u0151ben valamit aj\u00e1nlunk.<\/p>\r\n\t\t\t\t\t\t\t\t<h2>Nem \u00e9rdekelnek az ilyen dolgok?<\/h2>\r\n\t\t\t\t\t\t\t\t<p>Konfigur\u00e1lhatod is \u00e1ruh\u00e1zi preferenci\u00e1idat, hogy megmondd a Steamnek, milyen c\u00edmk\u00e9k vagy term\u00e9kt\u00edpusok nem \u00e9rdekelnek. <a href=\"https:\/\/store.steampowered.com\/account\/preferences\/\">\u00c1ruh\u00e1z-be\u00e1ll\u00edt\u00e1sok felkeres\u00e9se<\/a>.<\/p>\r\n\t\t\t\t\t\t\t<\/div>";
	ShowAlertDialog( "T\u00f6bb az aj\u00e1nl\u00e1sokr\u00f3l", strTemplate);
