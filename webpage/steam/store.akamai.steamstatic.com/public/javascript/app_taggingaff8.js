...
function InitAppTagModal( appid, rgAppTags, rgUserTags, strTagLinkSNR, strYourTagSNR, bShowBanOption )
{
	...
	var fnSetCheckboxTooltip = function( $Checkbox )
	{
		if ( $Checkbox.hasClass('checked') )
			$Checkbox.data('store-tooltip', 'Sie haben dieses Tag hinzugefügt. Klicken Sie hier, um es wieder zu entfernen.' );
		else
			$Checkbox.data('store-tooltip', 'Klicken Sie hier, um dieses Tag hinzuzufügen.' );
	};

	var fnSetReportTooltip = function( $ReportFlag )
	{
		if ( $ReportFlag.hasClass('reported') )
			$ReportFlag.data('store-tooltip', 'Sie haben dieses Tag gemeldet als fälschlich zugeordnet für dieses Produkt gemeldet. Klicken Sie noch einmal, um diese Meldung zu wiederufen.' );
		else
			$ReportFlag.data('store-tooltip', 'Klicken Sie hier, wenn Sie der Meinung sind, dass dieses Tag falsch zugeordnet ist.' );
	};
	...
	var fnMakeTag = function( tagid, tag, checked, bPopular, bReported, bBrowsable )
	{
		...
		if ( bPopular )
		{
			...
			if ( $YourTags.length )
			{
				...
				if ( bShowBanOption )
				{
					...
					$Ban.data('store-tooltip', 'Klicken Sie hier, um dieses Tag von diesem Produkt zu entfernen. Diese Funktion ist Entwicklern und Valve-Mitarbeitern vorbehalten.' );
					BindStoreTooltip( $Ban );
					$Tag.append( $Ban );

					$Ban.click( function() {
						ShowConfirmDialog( 'Bann-Tag', 'Sind Sie sicher, dass Sie das "%s" Tag von diesem Produkt entfernen möchten?'.replace( /%s/, tag )).done( function()
						{
		...
		else
		{
			...
			$Link.data('store-tooltip', 'Noch nicht genug Produkte mit dem Tag "%s" versehen'.replace( /%s/, $Link.html() ) );
	...
	{
		var $Tag = $J('<div/>', {'class': 'app_tag previous_tag', 'data-tagid': tagid, 'data-store-tooltip': 'Klicken Sie hier, um dieses Tag hinzuzufügen.' }).text( tag ).click( function() { fnApplyTag( tag ) } );
	...
	var fnBuildTagDisplay = function( Modal )
	{
		...
		if ( rgYourPopularTags === null )
		{
			...
			if ( !bYourPopularTagsRequested )
			{
				...
				$J.get( 'http://store.steampowered.com/tagdata/populartags/german').done( function ( data ) {
		...
		if ( rgUserTags.length )
		{
			...
			for( var i = 0; i < rgUserTags.length; i++ )
			{
				if ( !rgUserTags[i].is_reported &&  !rgUserTags[i].browseable )
				{
					...
					$AppTag.data('store-tooltip', 'Noch nicht genug Produkte mit dem Tag "%s" versehen'.replace( /%s/, $AppTag.html() ) );
	...
	window.ShowAppTagBanModal = function() {
		...
		$BanModal.find( '.app_tag_report').click( function() {
			...
			ShowConfirmDialog( 'Tag entsperren',
				'Sind Sie sicher, dass Sie das Tag "%s" entsperren möchten?'.replace( /%s/, tag ),
				'Tag entsperren')
				.done( function() {
					var rgParams = {
						appid: appid,
					...
					$J.post( 'http://store.steampowered.com/tagdata/tagapp', rgParams ).done( function( data ) {
						...
						ShowAlertDialog( 'Tag entsperren', 'Dieser Tag konnte nicht gesperrt bzw. entsperrt werden. Bitte versuchen Sie es später erneut.' );
					});
					bBansChanged = true;
				});
		});
