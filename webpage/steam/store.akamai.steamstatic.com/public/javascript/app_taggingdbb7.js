...
function InitAppTagModal( appid, rgAppTags, rgUserTags, strTagLinkSNR, strYourTagSNR, bShowBanOption )
{
	...
	var fnSetCheckboxTooltip = function( $Checkbox )
	{
		if ( $Checkbox.hasClass('checked') )
			$Checkbox.data('store-tooltip', 'Ви застосували цей теґ. Клацніть, щоби вилучити.' );
		else
			$Checkbox.data('store-tooltip', 'Натисність тут, щоби застосувати цей теґ.' );
	};

	var fnSetReportTooltip = function( $ReportFlag )
	{
		if ( $ReportFlag.hasClass('reported') )
			$ReportFlag.data('store-tooltip', 'Ви оскаржили цей теґ як непридатний для цього продукту. Клацніть ще раз для скасування скарги.' );
		else
			$ReportFlag.data('store-tooltip', 'Клацніть для оскарження цього теґа через його непридатність для цього продукту.' );
	};
	...
	var fnMakeTag = function( tagid, tag, checked, bPopular, bReported, bBrowsable )
	{
		...
		if ( bPopular )
		{
			...
			if ( $YourTags.length )
			{
				...
				if ( bShowBanOption )
				{
					...
					$Ban.data('store-tooltip', 'Клацніть тут для вилучення цього теґа для цього продукту. Ця можливість доступна лише розробникам і персоналу Steam.' );
					BindStoreTooltip( $Ban );
					$Tag.append( $Ban );

					$Ban.click( function() {
						ShowConfirmDialog( 'Заблокувати теґ', 'Ви дійсно бажаєте вилучити теґ «%s» для цього продукту?'.replace( /%s/, tag )).done( function()
						{
		...
		else
		{
			...
			$Link.data('store-tooltip', 'Ще недостатньо продуктів позначено теґом «%s»'.replace( /%s/, $Link.html() ) );
	...
	{
		var $Tag = $J('<div/>', {'class': 'app_tag previous_tag', 'data-tagid': tagid, 'data-store-tooltip': 'Натисність тут, щоби застосувати цей теґ.' }).text( tag ).click( function() { fnApplyTag( tag ) } );
	...
	var fnBuildTagDisplay = function( Modal )
	{
		...
		if ( rgYourPopularTags === null )
		{
			...
			if ( !bYourPopularTagsRequested )
			{
				...
				$J.get( 'http://store.steampowered.com/tagdata/populartags/ukrainian').done( function ( data ) {
		...
		else
		{
			...
			{
				$YourTags.append( $J('<div/>', {'class': 'app_tag_control no_tags_yet'}).text('Ще не додано теґів' ) );
			}
		...
		if ( rgUserTags.length )
		{
			...
			for( var i = 0; i < rgUserTags.length; i++ )
			{
				if ( !rgUserTags[i].is_reported &&  !rgUserTags[i].browseable )
				{
					...
					$AppTag.data('store-tooltip', 'Ще недостатньо продуктів позначено теґом «%s»'.replace( /%s/, $AppTag.html() ) );
	...
	var fnApplyTag = function( tag, withdraw, bPopularClick, eReportType, unTagID )
	{
		...
		$J.post( 'http://store.steampowered.com/tagdata/tagapp', rgParams ).done( function( data ) {
			...
			{
				ShowAlertDialog( 'Скарга на теґ', 'Дякуємо вам за вашу скаргу. Теґи з достатньою кількістю скарг на них будуть вилучені для продукту.');
			}
			...
			ShowAlertDialog( 'Перегляд і редагування теґів для цього продукту', 'There was a problem adding your tag.' );
		});
	};
	...
	window.ShowAppTagModal = function( appid )
	{
		...
		$AppTagModal.show();
		var Modal = ShowAlertDialog( 'Перегляд і редагування теґів для цього продукту', $AppTagModal, 'Закрити' );
	...
	var fnDisplayTag = function( unTagID, strTagName ) {
		if ( !rgTagResults[unTagID] )
		{
			...
			$J.get( 'http://store.steampowered.com/tagdata/gettaggames/' + strTagLanguage + '/' + unTagID + '/', rgParams ).done( function ( html ) {
				...
				$GamesElement.find( '.browse_tag_loading').text( 'Перепрошуємо, виникла проблема під час завантаження предметів. Будь ласка, спробуйте знову пізніше.' );
			});
...
function InitBannedTagModal( appid, $BanModal )
{
	...
	window.ShowAppTagBanModal = function() {
		$BanModal.show();
		ShowAlertDialog( 'Заборонені теґи', $BanModal).always( function() {
			if ( bBansChanged )
				window.location.reload();
		} );
		...
		$BanModal.find( '.app_tag_report').click( function() {
			...
			ShowConfirmDialog( 'Розблокувати теґ',
				'Ви переконані, що хочете розблокувати теґ «%s»?'.replace( /%s/, tag ),
				'Розблокувати теґ')
				.done( function() {
					var rgParams = {
						appid: appid,
					...
					$J.post( 'http://store.steampowered.com/tagdata/tagapp', rgParams ).done( function( data ) {
						...
						ShowAlertDialog( 'Розблокувати теґ', 'Виникла проблема під час блокування чи розблокування цього теґу. Будь ласка, спробуйте знову пізніше.' );
					});
					bBansChanged = true;
				});
		});
