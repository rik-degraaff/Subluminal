...
function ShowShareDialog( )
{
	...
	ShowAlertDialog( "Jaa", $Content).always(
		function() {
			// save it away again for later
			$Content.hide();
			$J(document.body).append( $Content );
...
function InitQueueControls( store_appid, steamworks_appid, next_in_queue_appid, snr )
{
	...
	$IgnoreBtn.click( function() {
		$J.post( 'http://store.steampowered.com/recommended/ignorerecommendation/', {
			...
			ShowAlertDialog( 'Ei kiinnosta', 'Muutoksien tallentamisessa tapahtui virhe. Koeta uudestaan myöhemmin.' );
		});
	});
	...
	$FollowBtn.click( function() {
		$J.post( 'http://store.steampowered.com/explore/followgame/', {
			...
			ShowAlertDialog( 'Seuraa', 'Muutoksien tallentamisessa tapahtui virhe. Koeta uudestaan myöhemmin.' );
		});
	});
	...
	$J('a.dq_settings_link').click( function() {
		CDiscoveryQueue.ShowCustomizeDialog( function( data ) {
			...
			{
				ShowConfirmDialog( 'Muokkaa löytöjonoasi', 'Olemme rakentaneet sinulle uuden löytöjonon muokkaustesi perusteella. Mitä haluaisit tehdä?',
...
function UserReviewVoteUp( id )
{
	UserReview_Rate( id, true, 'http://store.steampowered.com/',
		function( rgResults ) {
			if ( rgResults.nSaleTaskCompleted ) { NewStickerPackModal( 'Merkitse arviointi hyödylliseksi... tai älä.');}
...
function BuildReviewHistogram()
{
	...
	$J.get( 'http://store.steampowered.com/appreviewhistogram/' + appid, { l: 'finnish' }
	).done( function( data ) {
		...
		var seriesRollup = [ { label: "Myönteinen", color: "#66c0f4", fillColor: "#66c0f4", data: chartDataPositive }, { label: "Kielteinen", color: "#A34C25", fillColor: "#A34C25", data: chartDataNegative } ];
		...
		if ( data.results.recent )
		{
			...
			seriesRecent = [ { color: "#66c0f4", label: "Myönteinen", data: chartDataPositive }, { color: "#A34C25", label: "Kielteinen", data: chartDataNegative } ];
		}
...
function ShowReportDialog( nAppId )
{
	...
	content.append( $J('<div/>', {'class': 'app_report_dialog_intro' } ).text('Valitse ilmoituksesi syy.') );
		...
		var rgReportOptions = {"1":"Petos - <span class=\"sub\">T\u00e4m\u00e4 sovellus yritt\u00e4\u00e4 petollisesti ker\u00e4t\u00e4 yksityisi\u00e4 tietoja, kuten Steam-tilin nimi\u00e4 ja salasanoja tai maksutietoja, kuten luottokorttitietoja.<\/span>","2":"Haitallinen - <span class=\"sub\">T\u00e4m\u00e4 sovellus muokkaa asiakkaan tietokonetta odottamattomalla tai haitallisella tavalla. (Sovellus on haittaohjelma tai virus.)<\/span>","3":"Vihapuhe - <span class=\"sub\">Sis\u00e4lt\u00e4\u00e4 vihapuhetta, eli puhetta, joka kehottaa vihaan, v\u00e4kivaltaan tai syrjint\u00e4\u00e4n henkil\u00f6n tai ryhm\u00e4n etnisen tai uskonnollisen taustan, i\u00e4n, sukupuolen, vamman tai seksuaalisen suuntautumisen perusteella<\/span>","4":"Pornografia - <span class=\"sub\">Sis\u00e4lt\u00e4\u00e4 pornografiaa<\/span>","5":"Aikuismateriaali - <span class=\"sub\">Sis\u00e4lt\u00e4\u00e4 aikuisille tarkoitettua materiaalia, jota ei olla merkitty tai sen k\u00e4ytt\u00f6\u00e4 estetty ik\u00e4tunnisteella <\/span>","6":"H\u00e4p\u00e4isev\u00e4 - <span class=\"sub\">Sis\u00e4lt\u00e4\u00e4 herjaavia tai kunniaa loukkaavia lausuntoja<\/span>","7":"Loukkaava - <span class=\"sub\">Sis\u00e4lt\u00e4\u00e4 selke\u00e4sti loukkaavaa tai katsojien loukkaamiseen tahallisesti luotua shokeeraavaa tai ep\u00e4asiallista materiaalia <\/span>","8":"Lasten hyv\u00e4ksik\u00e4ytt\u00f6 - <span class=\"sub\">Sis\u00e4lt\u00e4\u00e4 lasten hyv\u00e4ksik\u00e4ytt\u00f6\u00f6n liittyv\u00e4\u00e4 materiaalia<\/span>","11":"Lain rikkomus - <span class=\"sub\">Sis\u00e4lt\u00f6 rikkoo hallintoalueesi lakeja<\/span>","13":"Rikki - <span class=\"sub\">Ei lautaudu, k\u00e4ynnisty tai toimi oikein, ei edes minimivaatimukset t\u00e4ytt\u00e4v\u00e4ll\u00e4 laitteella.<\/span>"};
	...
	content.append( $J('<div/>', {'class': 'app_report_dialog_dmca' } ).html('Jos tahdot ilmoittaa tekijänoikeuden rikkomuksesta ja olet tekijänoikeuden haltija, siirry DMCA-ilmoituskaavakkeeseemme <a href="http://steamcommunity.com/dmca/create/">tänne</a>.') );
