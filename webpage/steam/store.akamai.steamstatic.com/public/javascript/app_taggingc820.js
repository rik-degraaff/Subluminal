...
function InitAppTagModal( appid, rgAppTags, rgUserTags, strTagLinkSNR, strYourTagSNR, bShowBanOption )
{
	...
	var fnSetCheckboxTooltip = function( $Checkbox )
	{
		if ( $Checkbox.hasClass('checked') )
			$Checkbox.data('store-tooltip', 'Olet lisännyt tunnisteen. Poista klikkaamalla.' );
		else
			$Checkbox.data('store-tooltip', 'Lisää tunniste klikkaamalla tästä.' );
	};

	var fnSetReportTooltip = function( $ReportFlag )
	{
		if ( $ReportFlag.hasClass('reported') )
			$ReportFlag.data('store-tooltip', 'Olet ilmoittanut tämän tunnisteen sopimattomaksi tälle tuotteelle. Poista ilmoitus napsauttamalla uudelleen.' );
		else
			$ReportFlag.data('store-tooltip', 'Klikkaamalla tunnistetta voit ilmoittaa, ettei se sovellu tähän tuotteeseen.' );
	};
	...
	var fnMakeTag = function( tagid, tag, checked, bPopular, bReported, bBrowsable )
	{
		...
		if ( bPopular )
		{
			...
			if ( $YourTags.length )
			{
				...
				if ( bShowBanOption )
				{
					...
					$Ban.data('store-tooltip', 'Poista tämä tunniste tästä tuotteesta napsauttamalla tästä. Tämä vaihtoehto on mahdollinen vain kehittäjille ja Valven työntekijöille.' );
					BindStoreTooltip( $Ban );
					$Tag.append( $Ban );

					$Ban.click( function() {
						ShowConfirmDialog( 'Estä tunniste', 'Haluatko varmasti poistaa tunnisteen "%s" tältä tuotteelta?'.replace( /%s/, tag )).done( function()
						{
		...
		else
		{
			...
			$Link.data('store-tooltip', 'Tunnisteella "%s" ei ole vielä merkitty tarpeeksi kohteita'.replace( /%s/, $Link.html() ) );
	...
	{
		var $Tag = $J('<div/>', {'class': 'app_tag previous_tag', 'data-tagid': tagid, 'data-store-tooltip': 'Lisää tunniste klikkaamalla tästä.' }).text( tag ).click( function() { fnApplyTag( tag ) } );
	...
	var fnBuildTagDisplay = function( Modal )
	{
		...
		if ( rgYourPopularTags === null )
		{
			...
			if ( !bYourPopularTagsRequested )
			{
				...
				$J.get( 'http://store.steampowered.com/tagdata/populartags/finnish').done( function ( data ) {
		...
		if ( rgUserTags.length )
		{
			...
			for( var i = 0; i < rgUserTags.length; i++ )
			{
				if ( !rgUserTags[i].is_reported &&  !rgUserTags[i].browseable )
				{
					...
					$AppTag.data('store-tooltip', 'Tunnisteella "%s" ei ole vielä merkitty tarpeeksi kohteita'.replace( /%s/, $AppTag.html() ) );
	...
	var fnDisplayTag = function( unTagID, strTagName ) {
		if ( !rgTagResults[unTagID] )
		{
			...
			$J.get( 'http://store.steampowered.com/tagdata/gettaggames/' + strTagLanguage + '/' + unTagID + '/', rgParams ).done( function ( html ) {
				...
				$GamesElement.find( '.browse_tag_loading').text( 'Pahoittelut, tunnisteiden lataamisessa ilmentyi virhe. Yritä myöhemmin uudestaan.' );
			});
	...
	window.ShowAppTagBanModal = function() {
		...
		$BanModal.find( '.app_tag_report').click( function() {
			...
			ShowConfirmDialog( 'Salli tunniste',
				'Tahdotko varmasti sallia tunnisteen "%s"?'.replace( /%s/, tag ),
				'Salli tunniste')
				.done( function() {
					var rgParams = {
						appid: appid,
					...
					$J.post( 'http://store.steampowered.com/tagdata/tagapp', rgParams ).done( function( data ) {
						...
						ShowAlertDialog( 'Salli tunniste', 'Tunnisteen estämisessä tai sallimisessa tapahtui virhe. Koeta myöhemmin uudestaan.' );
					});
					bBansChanged = true;
				});
		});
