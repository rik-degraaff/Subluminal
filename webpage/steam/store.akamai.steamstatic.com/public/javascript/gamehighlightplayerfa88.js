...
HighlightPlayer.prototype.LoadMovie = function( $Container, bUserAction )
{
	...
	else
	{
		...
		var strMessage = 'Videon toistossa tapahtui virhe. Varmistathan, että selaimesi päivitykset ovat ajan tasalla.';
...
HighlightPlayer.prototype.ShowScreenshotPopup = function( screenshotid )
{
	...
	var $Title = $J('<a/>' );
	if ( Steam.BIsUserInSteamClient() )
		$Title.text( 'Näytä täysikokoinen versio selaimessa' );
	else
		$Title.text( 'Lataa täysikokoinen versio' );
	...
	var fnUpdateFooter = function()
	{
		...
		$ScreenshotCount.text( '%1$s / %2$s kuvankaappauksesta'.replace( /%1\$s/, iCurIndex + 1 ).replace( /%2\$s/, rgScreenshotIDs.length ) );
	};
	...
	$.fn.videoControls = function( options ) {
		...
		var overlaySrc = '<div class="html5_video_overlay">' +
				...
				'<div class="control_container">' +
					...
					'<div class="autoplay_label">Toista videot automaattisesti</div>' +
				'</div>' +
