...
function InitAppTagModal( appid, rgAppTags, rgUserTags, strTagLinkSNR, strYourTagSNR, bShowBanOption )
{
	...
	var fnSetCheckboxTooltip = function( $Checkbox )
	{
		if ( $Checkbox.hasClass('checked') )
			$Checkbox.data('store-tooltip', '이 태그를 적용하셨습니다. 제거하려면 여기를 클릭하세요.' );
		else
			$Checkbox.data('store-tooltip', '이 태그를 적용하려면 여기를 클릭하세요.' );
	};

	var fnSetReportTooltip = function( $ReportFlag )
	{
		if ( $ReportFlag.hasClass('reported') )
			$ReportFlag.data('store-tooltip', '이 태그를 신고하셨습니다. 신고를 삭제하시려면 다시 클릭하세요.' );
		else
			$ReportFlag.data('store-tooltip', '이 태그를 신고하시려면 여기를 클릭하세요.' );
	};
	...
	var fnMakeTag = function( tagid, tag, checked, bPopular, bReported, bBrowsable )
	{
		...
		if ( bPopular )
		{
			...
			if ( $YourTags.length )
			{
				...
				if ( bShowBanOption )
				{
					...
					$Ban.data('store-tooltip', '제품에서 태그를 제거하기 위해 여기를 클릭하세요. 개발자와 Steam 직원만 실행할 수 있는 작업입니다.' );
					BindStoreTooltip( $Ban );
					$Tag.append( $Ban );

					$Ban.click( function() {
						ShowConfirmDialog( '태그 차단하기', '제품에서 "%s" 태그를 정말로 삭제하시겠습니까?'.replace( /%s/, tag )).done( function()
						{
		...
		else
		{
			...
			$Link.data('store-tooltip', '아직 "%s" 태그가 달린 항목이 부족합니다.'.replace( /%s/, $Link.html() ) );
	...
	{
		var $Tag = $J('<div/>', {'class': 'app_tag previous_tag', 'data-tagid': tagid, 'data-store-tooltip': '이 태그를 적용하려면 여기를 클릭하세요.' }).text( tag ).click( function() { fnApplyTag( tag ) } );
	...
	var fnBuildTagDisplay = function( Modal )
	{
		...
		if ( rgYourPopularTags === null )
		{
			...
			if ( !bYourPopularTagsRequested )
			{
				...
				$J.get( 'http://store.steampowered.com/tagdata/populartags/koreana').done( function ( data ) {
		...
		else
		{
			...
			{
				$YourTags.append( $J('<div/>', {'class': 'app_tag_control no_tags_yet'}).text('아직 태그가 입력되지 않았습니다' ) );
			}
		...
		if ( rgUserTags.length )
		{
			...
			for( var i = 0; i < rgUserTags.length; i++ )
			{
				if ( !rgUserTags[i].is_reported &&  !rgUserTags[i].browseable )
				{
					...
					$AppTag.data('store-tooltip', '아직 "%s" 태그가 달린 항목이 부족합니다.'.replace( /%s/, $AppTag.html() ) );
	...
	var fnApplyTag = function( tag, withdraw, bPopularClick, eReportType, unTagID )
	{
		...
		$J.post( 'http://store.steampowered.com/tagdata/tagapp', rgParams ).done( function( data ) {
			...
			{
				ShowAlertDialog( '태그 신고', '보고해 주셔서 감사합니다. 태그 신고 수가 많으면 제품에서 삭제됩니다.');
			}
			...
			ShowAlertDialog( '이 제품의 태그 보기 및 수정', 'There was a problem adding your tag.' );
		});
	};
	...
	window.ShowAppTagModal = function( appid )
	{
		...
		$AppTagModal.show();
		var Modal = ShowAlertDialog( '이 제품의 태그 보기 및 수정', $AppTagModal, '닫기' );
	...
	var fnDisplayTag = function( unTagID, strTagName ) {
		if ( !rgTagResults[unTagID] )
		{
			...
			$J.get( 'http://store.steampowered.com/tagdata/gettaggames/' + strTagLanguage + '/' + unTagID + '/', rgParams ).done( function ( html ) {
				...
				$GamesElement.find( '.browse_tag_loading').text( '죄송합니다, 항목을 불러오는 중 문제가 발생했습니다. 나중에 다시 시도해주세요.' );
			});
...
function InitBannedTagModal( appid, $BanModal )
{
	...
	window.ShowAppTagBanModal = function() {
		$BanModal.show();
		ShowAlertDialog( '금지된 태그', $BanModal).always( function() {
			if ( bBansChanged )
				window.location.reload();
		} );
		...
		$BanModal.find( '.app_tag_report').click( function() {
			...
			ShowConfirmDialog( '태그 금지 해제',
				'정말 "%s" 태그의 금지를 해제하시겠습니까?'.replace( /%s/, tag ),
				'태그 금지 해제')
				.done( function() {
					var rgParams = {
						appid: appid,
					...
					$J.post( 'http://store.steampowered.com/tagdata/tagapp', rgParams ).done( function( data ) {
						...
						ShowAlertDialog( '태그 금지 해제', '이 태그를 금지 또는 금지 해제하는 중 문제가 발생했습니다. 나중에 다시 시도해 주세요.' );
					});
					bBansChanged = true;
				});
		});
