...
GDynamicStore = {
	...
	CapsuleSettingsMenu: function( elSource )
	{
		...
		if( strAppIDs )
		{
			...
			var $elPreferences = $J ( '<a/>' ).attr('href', 'https://store.steampowered.com/account/preferences' ).text ( "Preferencias" ).addClass( 'option' );
	...
	BuildBundleDescription: function( Bundle, $Description )
	{
		...
		else if ( Bundle.m_cUserItemsInBundle < Bundle.m_cTotalItemsInBundle )
		{
			...
			$Description.html( '<div>%1$s de los %2$s artículos de este pack ya están en tu biblioteca.</div>'.replace( '%1$s', Bundle.m_cTotalItemsInBundle - Bundle.m_cUserItemsInBundle ).replace( '%2$s', Bundle.m_cTotalItemsInBundle ) );
			$Description.append( '<div>¡Compra este pack para ahorrar un %1$s%% en los %2$s artículos que aún no tienes!</div>'.replace( '%1$s', Bundle.m_nDiscountPct ).replace( '%2$s', Bundle.m_cUserItemsInBundle ).replace( '%%', '%' ) );
		...
		{
			$Description.html( '<div>¡Compra este pack para ahorrar un %1$s%% en los %2$s artículos!</div>'.replace( '%1$s', Bundle.m_nDiscountPct ).replace( '%2$s', Bundle.m_cTotalItemsInBundle ).replace( '%%', '%' ) );
		}
	...
	PopulateRecommendedTagList: function()
	{
		...
		for( var i = 0; i < GDynamicStore.s_rgRecommendedTags.length && i < 5; i++ )
		{
			var tag = GDynamicStore.s_rgRecommendedTags[i];
							var $Link = $J('<a/>', {'class': 'popup_menu_item', 'href': 'http://store.steampowered.com/tags/es/' + encodeURIComponent( tag.name ) });
						$Link.text( tag.name );
			$Element.append( $Link );
		}
	},
