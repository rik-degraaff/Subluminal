...
GDynamicStore = {
		...
		$DynamicElements.each( function() {
			...
			if ( !$El.hasClass('ds_no_flags') )
			{
				...
				if ( bOwned )
				{
					...
					$El.append( '<div class="ds_flag ds_owned_flag">ÎN COLECȚIE&nbsp;&nbsp;</div>');
				}
				else if ( bWanted )
				{
	...
	CapsuleSettingsMenu: function( elSource )
	{
		...
		if( strAppIDs )
		{
			...
			else
			{
				...
				var strText = "Pe lista de dorin\u021be";
			...
			var $elPreferences = $J ( '<a/>' ).attr('href', 'https://store.steampowered.com/account/preferences' ).text ( "Preferin\u021be" ).addClass( 'option' );
	...
	UpdateDynamicBundleElements: function( Bundle, $El )
	{
		...
		if ( !Bundle.m_bIsCommercial && ( !Bundle.m_rgBundleItems.length || ( Bundle.m_bMustPurchaseAsSet && Bundle.m_cUserItemsInBundle < Bundle.m_cTotalItemsInBundle ) ) )
		{
			...
			var strTooltip = 'Acest pachet nu este disponibil pentru achiziționare pe contul tău din moment ce ai deja toate articolele incluse.';

			if ( Bundle.m_bMustPurchaseAsSet && !Bundle.m_bRestrictGifting )
			{
				strTooltip = 'Această ofertă este valabilă doar în cazul în care cumperi toate cele %s obiecte simultan. Poți însă cumpăra acest pachet drept cadou pentru un prieten.'.replace(/%s/, Bundle.m_cTotalItemsInBundle);
	...
	BuildBundleDescription: function( Bundle, $Description )
	{
		if ( Bundle.m_cUserItemsInBundle == 0 )
		{
			...
			$Description.html( '<span class="collectionComplete">Colecție completă</span> %1$s din %2$s articole cuprinse în această colecție sunt deja în colecția ta.'.replace( '%1$s', Bundle.m_cTotalItemsInBundle ).replace( '%2$s', Bundle.m_cTotalItemsInBundle ) );
		}
		else if ( Bundle.m_cUserItemsInBundle < Bundle.m_cTotalItemsInBundle )
		{
			// own some but not all.
			$Description.html( '<div>%1$s din %2$s articole cuprinse în acest pachet sunt deja în colecția ta.</div>'.replace( '%1$s', Bundle.m_cTotalItemsInBundle - Bundle.m_cUserItemsInBundle ).replace( '%2$s', Bundle.m_cTotalItemsInBundle ) );
			$Description.append( '<div>Cumpără acest pachet și economisește %1$s%% din cele %2$s obiecte pe care încă nu le ai!</div>'.replace( '%1$s', Bundle.m_nDiscountPct ).replace( '%2$s', Bundle.m_cUserItemsInBundle ).replace( '%%', '%' ) );
		...
		{
			$Description.html( '<div>Cumpără acest pachet pentru a economisi %1$s%% din toate cele %2$s obiecte!</div>'.replace( '%1$s', Bundle.m_nDiscountPct ).replace( '%2$s', Bundle.m_cTotalItemsInBundle ).replace( '%%', '%' ) );
		}
	...
	PopulateRecommendedTagList: function()
	{
		...
		for( var i = 0; i < GDynamicStore.s_rgRecommendedTags.length && i < 5; i++ )
		{
			var tag = GDynamicStore.s_rgRecommendedTags[i];
							var $Link = $J('<a/>', {'class': 'popup_menu_item', 'href': 'http://store.steampowered.com/tags/ro/' + encodeURIComponent( tag.name ) });
						$Link.text( tag.name );
			$Element.append( $Link );
		}
	},
	...
	DisplayBundleSimulator: function( unBundleID )
	{
		...
		Modal = ShowConfirmDialog( '', $Form, 'Actualizează display-ul' ).done( function() {
