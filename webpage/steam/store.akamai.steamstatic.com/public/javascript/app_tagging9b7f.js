...
function InitAppTagModal( appid, rgAppTags, rgUserTags, strTagLinkSNR, strYourTagSNR, bShowBanOption )
{
	...
	var fnSetCheckboxTooltip = function( $Checkbox )
	{
		if ( $Checkbox.hasClass('checked') )
			$Checkbox.data('store-tooltip', 'คุณนำแท็กนี้ไปใช้แล้ว คลิกเพื่อลบออก' );
		else
			$Checkbox.data('store-tooltip', 'คลิกที่นี่เพื่อนำแท็กนี้ไปใช้' );
	};

	var fnSetReportTooltip = function( $ReportFlag )
	{
		if ( $ReportFlag.hasClass('reported') )
			$ReportFlag.data('store-tooltip', 'คุณได้รายงานแท็กนี้แล้วว่า นำมาใช้งานไม่ตรงกับผลิตภัณฑ์นี้ คลิกอีกครั้งเพื่อลบรายงานนี้' );
		else
			$ReportFlag.data('store-tooltip', 'คลิกเพื่อรายงานแท็กนี้ว่า นำมาใช้งานไม่ตรงกับผลิตภัณฑ์นี้' );
	};
	...
	var fnMakeTag = function( tagid, tag, checked, bPopular, bReported, bBrowsable )
	{
		...
		if ( bPopular )
		{
			...
			if ( $YourTags.length )
			{
				...
				if ( bShowBanOption )
				{
					...
					$Ban.data('store-tooltip', 'คลิกที่นี่เพื่อลบแท็กนี้ออกจากผลิตภัณฑ์นี้ ตัวเลือกนี้จำกัดเฉพาะผู้พัฒนาและเจ้าหน้าที่ Steam เท่านั้น' );
					BindStoreTooltip( $Ban );
					$Tag.append( $Ban );

					$Ban.click( function() {
						ShowConfirmDialog( 'แบนแท็ก', 'คุณแน่ใจหรือไม่ว่าต้องการลบแท็ก "%s" ออกจากผลิตภัณฑ์นี้?'.replace( /%s/, tag )).done( function()
						{
		...
		else
		{
			...
			$Link.data('store-tooltip', 'รายการที่ถูกแท็กด้วย "%s" ยังมีไม่มากพอ'.replace( /%s/, $Link.html() ) );
	...
	{
		var $Tag = $J('<div/>', {'class': 'app_tag previous_tag', 'data-tagid': tagid, 'data-store-tooltip': 'คลิกที่นี่เพื่อนำแท็กนี้ไปใช้' }).text( tag ).click( function() { fnApplyTag( tag ) } );
	...
	var fnBuildTagDisplay = function( Modal )
	{
		...
		if ( rgYourPopularTags === null )
		{
			...
			if ( !bYourPopularTagsRequested )
			{
				...
				$J.get( 'http://store.steampowered.com/tagdata/populartags/thai').done( function ( data ) {
		...
		else
		{
			...
			{
				$YourTags.append( $J('<div/>', {'class': 'app_tag_control no_tags_yet'}).text('ยังไม่มีแท็ก' ) );
			}
		...
		if ( rgUserTags.length )
		{
			...
			for( var i = 0; i < rgUserTags.length; i++ )
			{
				if ( !rgUserTags[i].is_reported &&  !rgUserTags[i].browseable )
				{
					...
					$AppTag.data('store-tooltip', 'รายการที่ถูกแท็กด้วย "%s" ยังมีไม่มากพอ'.replace( /%s/, $AppTag.html() ) );
	...
	var fnApplyTag = function( tag, withdraw, bPopularClick, eReportType, unTagID )
	{
		...
		$J.post( 'http://store.steampowered.com/tagdata/tagapp', rgParams ).done( function( data ) {
			...
			{
				ShowAlertDialog( 'รายงานแท็ก', 'ขอบคุณสำหรับรายงานของคุณ แท็กที่มีจำนวนรายงานถึงจำนวนที่กำหนดไว้จะถูกลบออกจากผลิตภัณฑ์นี้');
			}
			...
			ShowAlertDialog( 'ดูและแก้ไขแท็กสำหรับผลิตภัณฑ์นี้', 'There was a problem adding your tag.' );
		});
	};
	...
	window.ShowAppTagModal = function( appid )
	{
		...
		$AppTagModal.show();
		var Modal = ShowAlertDialog( 'ดูและแก้ไขแท็กสำหรับผลิตภัณฑ์นี้', $AppTagModal, 'ปิด' );
	...
	var fnDisplayTag = function( unTagID, strTagName ) {
		if ( !rgTagResults[unTagID] )
		{
			...
			$J.get( 'http://store.steampowered.com/tagdata/gettaggames/' + strTagLanguage + '/' + unTagID + '/', rgParams ).done( function ( html ) {
				...
				$GamesElement.find( '.browse_tag_loading').text( 'ขออภัย ตรวจพบปัญหาในการโหลดผลิตภัณฑ์ กรุณาลองใหม่ภายหลัง' );
			});
...
function InitBannedTagModal( appid, $BanModal )
{
	...
	window.ShowAppTagBanModal = function() {
		$BanModal.show();
		ShowAlertDialog( 'แท็กที่ถูกแบน', $BanModal).always( function() {
			if ( bBansChanged )
				window.location.reload();
		} );
		...
		$BanModal.find( '.app_tag_report').click( function() {
			...
			ShowConfirmDialog( 'เลิกแบนแท็ก',
				'คุณแน่ใจหรือไม่ว่าต้องการเลิกแบนแท็ก "%s"?'.replace( /%s/, tag ),
				'เลิกแบนแท็ก')
				.done( function() {
					var rgParams = {
						appid: appid,
					...
					$J.post( 'http://store.steampowered.com/tagdata/tagapp', rgParams ).done( function( data ) {
						...
						ShowAlertDialog( 'เลิกแบนแท็ก', 'ตรวจพบปัญหาในการแบนหรือเลิกแบนแท็กนี้ กรุณาลองใหม่ภายหลัง' );
					});
					bBansChanged = true;
				});
		});
