...
function InitAppTagModal( appid, rgAppTags, rgUserTags, strTagLinkSNR, strYourTagSNR, bShowBanOption )
{
	...
	var fnSetCheckboxTooltip = function( $Checkbox )
	{
		if ( $Checkbox.hasClass('checked') )
			$Checkbox.data('store-tooltip', 'Du har lagt til denne taggen. Klikk for å fjerne.' );
		else
			$Checkbox.data('store-tooltip', 'Klikk her for å bruke denne merkelappen.' );
	};

	var fnSetReportTooltip = function( $ReportFlag )
	{
		if ( $ReportFlag.hasClass('reported') )
			$ReportFlag.data('store-tooltip', 'Du har rapportert denne etiketten for å være feilaktig anvendt til dette produktet. Klikk igjen for å fjerne rapporteringen.' );
		else
			$ReportFlag.data('store-tooltip', 'Klikk for å rapportere denne taggen som feil for dette produktet.' );
	};
	...
	var fnMakeTag = function( tagid, tag, checked, bPopular, bReported, bBrowsable )
	{
		...
		else
		{
			...
			$Link.data('store-tooltip', 'Ikke nok gjenstander tagget "%s" ennå'.replace( /%s/, $Link.html() ) );
	...
	{
		var $Tag = $J('<div/>', {'class': 'app_tag previous_tag', 'data-tagid': tagid, 'data-store-tooltip': 'Klikk her for å bruke denne merkelappen.' }).text( tag ).click( function() { fnApplyTag( tag ) } );
	...
	var fnBuildTagDisplay = function( Modal )
	{
		...
		if ( rgYourPopularTags === null )
		{
			...
			if ( !bYourPopularTagsRequested )
			{
				...
				$J.get( 'http://store.steampowered.com/tagdata/populartags/norwegian').done( function ( data ) {
		...
		else
		{
			...
			{
				$YourTags.append( $J('<div/>', {'class': 'app_tag_control no_tags_yet'}).text('Ingen merker angitt ennå' ) );
			}
		...
		if ( rgUserTags.length )
		{
			...
			for( var i = 0; i < rgUserTags.length; i++ )
			{
				if ( !rgUserTags[i].is_reported &&  !rgUserTags[i].browseable )
				{
					...
					$AppTag.data('store-tooltip', 'Ikke nok gjenstander tagget "%s" ennå'.replace( /%s/, $AppTag.html() ) );
	...
	var fnApplyTag = function( tag, withdraw, bPopularClick, eReportType, unTagID )
	{
		...
		$J.post( 'http://store.steampowered.com/tagdata/tagapp', rgParams ).done( function( data ) {
			...
			{
				ShowAlertDialog( 'Rapporter en tagg', 'Thank you for your report.  Tags that reach a certain threshold of reports will be removed from the product.');
			}
			...
			ShowAlertDialog( 'Vis og rediger tagger for dette spillet', 'There was a problem adding your tag.' );
		});
	};
	...
	window.ShowAppTagModal = function( appid )
	{
		...
		$AppTagModal.show();
		var Modal = ShowAlertDialog( 'Vis og rediger tagger for dette spillet', $AppTagModal, 'Lukk' );
	...
	var fnDisplayTag = function( unTagID, strTagName ) {
		if ( !rgTagResults[unTagID] )
		{
			...
			$J.get( 'http://store.steampowered.com/tagdata/gettaggames/' + strTagLanguage + '/' + unTagID + '/', rgParams ).done( function ( html ) {
				...
				$GamesElement.find( '.browse_tag_loading').text( 'Beklager, det oppstod en feil under lasting av gjenstander. Vennligst prøv igjen senere.' );
			});
...
function InitBannedTagModal( appid, $BanModal )
{
	...
	window.ShowAppTagBanModal = function() {
		$BanModal.show();
		ShowAlertDialog( 'Utestengte etiketter', $BanModal).always( function() {
			if ( bBansChanged )
				window.location.reload();
		} );
		...
		$BanModal.find( '.app_tag_report').click( function() {
			...
			ShowConfirmDialog( 'Opphev forbud av tagg',
				'Er du sikker på at du vil oppheve forbudet av "%s"-taggen?'.replace( /%s/, tag ),
				'Opphev forbud av tagg')
				.done( function() {
					var rgParams = {
						appid: appid,
						sessionid: g_sessionID,
					...
					$J.post( 'http://store.steampowered.com/tagdata/tagapp', rgParams ).done( function( data ) {
						...
						ShowAlertDialog( 'Opphev forbud av tagg', 'Det oppstod et problem ved utestengning av eller åpning for denne etiketten. Prøv igjen senere.' );
					});
					bBansChanged = true;
				});
		});
