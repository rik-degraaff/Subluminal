...
function InitAppTagModal( appid, rgAppTags, rgUserTags, strTagLinkSNR, strYourTagSNR, bShowBanOption )
{
	...
	var fnSetCheckboxTooltip = function( $Checkbox )
	{
		if ( $Checkbox.hasClass('checked') )
			$Checkbox.data('store-tooltip', 'Has aplicado esta etiqueta. Haz clic para eliminar.' );
		else
			$Checkbox.data('store-tooltip', 'Haz clic aquí para aplicar esta etiqueta.' );
	};

	var fnSetReportTooltip = function( $ReportFlag )
	{
		if ( $ReportFlag.hasClass('reported') )
			$ReportFlag.data('store-tooltip', 'Has denunciado esta etiqueta por haber sido aplicada incorrectamente a este producto. Haz clic de nuevo para retirar la denuncia.' );
		else
			$ReportFlag.data('store-tooltip', 'Haz clic para denunciar esta etiqueta por haber sido aplicada incorrectamente a este producto.' );
	};
	...
	var fnMakeTag = function( tagid, tag, checked, bPopular, bReported, bBrowsable )
	{
		...
		if ( bPopular )
		{
			...
			if ( $YourTags.length )
			{
				...
				if ( bShowBanOption )
				{
					...
					$Ban.data('store-tooltip', 'Haz clic aquí para eliminar esta etiqueta de este producto. Esta opción está limitada a desarrolladores y personal de Steam.' );
					BindStoreTooltip( $Ban );
					$Tag.append( $Ban );

					$Ban.click( function() {
						ShowConfirmDialog( 'Prohibir etiqueta', '¿Estás seguro de que quieres eliminar la etiqueta "%s" de este producto?'.replace( /%s/, tag )).done( function()
						{
		...
		else
		{
			...
			$Link.data('store-tooltip', 'Aún no existen suficientes artículos etiquetados con "%s"'.replace( /%s/, $Link.html() ) );
	...
	{
		var $Tag = $J('<div/>', {'class': 'app_tag previous_tag', 'data-tagid': tagid, 'data-store-tooltip': 'Haz clic aquí para aplicar esta etiqueta.' }).text( tag ).click( function() { fnApplyTag( tag ) } );
	...
	var fnBuildTagDisplay = function( Modal )
	{
		...
		if ( rgYourPopularTags === null )
		{
			...
			if ( !bYourPopularTagsRequested )
			{
				...
				$J.get( 'http://store.steampowered.com/tagdata/populartags/spanish').done( function ( data ) {
		...
		if ( rgUserTags.length )
		{
			...
			for( var i = 0; i < rgUserTags.length; i++ )
			{
				if ( !rgUserTags[i].is_reported &&  !rgUserTags[i].browseable )
				{
					...
					$AppTag.data('store-tooltip', 'Aún no existen suficientes artículos etiquetados con "%s"'.replace( /%s/, $AppTag.html() ) );
	...
	var fnDisplayTag = function( unTagID, strTagName ) {
		if ( !rgTagResults[unTagID] )
		{
			...
			$J.get( 'http://store.steampowered.com/tagdata/gettaggames/' + strTagLanguage + '/' + unTagID + '/', rgParams ).done( function ( html ) {
				...
				$GamesElement.find( '.browse_tag_loading').text( 'Lo siento, ha habido un problema al cargar los artículos. Por favor, inténtalo de nuevo más tarde.' );
			});
	...
	window.ShowAppTagBanModal = function() {
		...
		$BanModal.find( '.app_tag_report').click( function() {
			...
			ShowConfirmDialog( 'Permitir etiqueta',
				'¿Estás seguro de que deseas permitir la etiqueta "%s"?'.replace( /%s/, tag ),
				'Permitir etiqueta')
				.done( function() {
					var rgParams = {
						appid: appid,
					...
					$J.post( 'http://store.steampowered.com/tagdata/tagapp', rgParams ).done( function( data ) {
						...
						ShowAlertDialog( 'Permitir etiqueta', 'Ha habido un problema al prohibir o permitir esta etiqueta. Por favor, inténtalo de nuevo más tarde.' );
					});
					bBansChanged = true;
				});
		});
