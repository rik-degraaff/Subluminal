...
function ShowEmbedWidget( )
{
	...
	var Modal = _BuildDialog( "Maak een insluitbare widget", $Content, [], fnOK, {} );
...
function ShowShareDialog( )
{
	...
	ShowAlertDialog( "Delen", $Content).always(
		function() {
			// save it away again for later
			$Content.hide();
			$J(document.body).append( $Content );
...
function InitQueueControls( store_appid, steamworks_appid, next_in_queue_appid, snr )
{
	...
	$IgnoreBtn.click( function() {
		$J.post( 'http://store.steampowered.com/recommended/ignorerecommendation/', {
			...
			if ( data && data.nSaleTaskCompleted ) { NewStickerPackModal( 'Markeer iets als \'Niet interessant\'' ); } // SummerSale2017
		}).fail( function( jqXHR ) {
			ShowAlertDialog( 'Niet interessant', 'Er is een probleem opgetreden bij het opslaan van je veranderingen. Probeer het later nog eens.' );
		});
	});
	...
	$FollowBtn.click( function() {
		$J.post( 'http://store.steampowered.com/explore/followgame/', {
			...
			ShowAlertDialog( 'Volgen', 'Er is een probleem opgetreden bij het opslaan van je veranderingen. Probeer het later nog eens.' );
		});
	});
	...
	$J('a.dq_settings_link').click( function() {
		CDiscoveryQueue.ShowCustomizeDialog( function( data ) {
			...
			{
				ShowConfirmDialog( 'Je ontdekkingswachtrij aanpassen', 'We hebben een nieuwe ontdekkingswachtrij gemaakt door je aanpassingen toe te passen. Wat wil je doen?',
					'De nieuwe wachtrij ontdekken','Hier blijven'
				).done( function() {
...
function InitAutocollapse()
{
	$J('.game_page_autocollapse').each( function() {
		...
		var $ReadMore = $J('<div/>', {'class': 'game_page_autocollapse_readmore' }).text( 'MEER INFORMATIE' );
...
function UserReviewVoteUp( id )
{
	UserReview_Rate( id, true, 'http://store.steampowered.com/',
		function( rgResults ) {
			if ( rgResults.nSaleTaskCompleted ) { NewStickerPackModal( 'Markeer een recensie als behulpzaam... of niet');}
...
function BuildReviewHistogram()
{
	...
	$J.get( 'http://store.steampowered.com/appreviewhistogram/' + appid, { l: 'dutch' }
	).done( function( data ) {
		...
		var seriesRollup = [ { label: "Positief", color: "#66c0f4", fillColor: "#66c0f4", data: chartDataPositive }, { label: "Negatief", color: "#A34C25", fillColor: "#A34C25", data: chartDataNegative } ];
		...
		if ( data.results.recent )
		{
			...
			seriesRecent = [ { color: "#66c0f4", label: "Positief", data: chartDataPositive }, { color: "#A34C25", label: "Negatief", data: chartDataNegative } ];
		}
...
function UpdateActiveFilters()
{
	...
	if ( $J( "#review_type_positive" ).attr( "checked" ) )
	{
		...
		$J( "#reviews_filter_type" ).text( 'Positief' );
	}
	else if ( $J( "#review_type_negative" ).attr( "checked" ) )
	{
		bAnyActiveFilters = true;
		$J( "#reviews_filter_type" ).show();
		$J( "#reviews_filter_type" ).text( 'Negatief' );
	}
	else
	{
	...
	if ( $J( "#purchase_type_steam" ).attr( "checked" ) )
	{
		...
		$J( "#reviews_filter_purchase_type" ).text( 'Kopers op Steam' );
	}
	else if ( $J( "#purchase_type_non_steam" ).attr( "checked" ) )
	{
		bAnyActiveFilters = true;
		$J( "#reviews_filter_purchase_type" ).show();
		$J( "#reviews_filter_purchase_type" ).text( 'Niet op Steam gekocht' );
	}
	else
	{
	...
	if ( $J( "#review_language_mine" ).attr( "checked" ) )
	{
		...
		$J( "#reviews_filter_language" ).text( 'Jouw talen' );
	}
	else
	{
	...
	if ( $J( "#review_date_range_histogram" ).attr( "checked" ) )
	{
		...
		$J( "#review_selected_histogram_date_range_prefix" ).text( 'Alleen bekijken ' );
	}
	else if ( $J( "#review_date_range_exclude_histogram" ).attr( "checked" ) )
	{
		bAnyActiveFilters = true;
		$J( "#reviews_filter_graph" ).show();
		$J( "#review_selected_histogram_date_range_prefix" ).text( 'Uitsluiten ' );
	}
	else
	{
...
function ShowReportDialog( nAppId )
{
	...
	content.append( $J('<div/>', {'class': 'app_report_dialog_intro' } ).text('Geef aan waarom je dit product wilt rapporteren.') );

		var rgReportOptions = {"1":"Fraude - <span class=\"sub\">Deze software probeert op frauduleuze wijze gevoelige informatie te verzamelen, zoals je Steam-gegevens of financi\u00eble gegevens (bijvoorbeeld creditcardinformatie).<\/span>","2":"Schadelijk - <span class=\"sub\">Deze software past de computer van de gebruiker aan op een onverwachte of schadelijke wijze (het is bijvoorbeeld malware of een virus)<\/span>","3":"Aanzetten tot haat - <span class=\"sub\">Bevat aanzetting tot haat, bijvoorbeeld haatzaaiende tekst, geweld of discriminatie tegen bepaalde groepen of mensen op basis van etniciteit, religie, geslacht, leeftijd, handicap of seksuele geaardheid<\/span>","4":"Pornografisch - <span class=\"sub\">Bevat pornografie<\/span>","5":"Inhoud voor volwassenen - <span class=\"sub\">Bevat inhoud voor volwassenen die niet op geschikte wijze is aangeduid en afgeschermd<\/span>","6":"Lasterlijk - <span class=\"sub\">Bevat smadelijke of lasterlijke uitspraken<\/span>","7":"Beledigend - <span class=\"sub\">Bevat inhoud die duidelijk beledigend is of bedoeld is om kijkers te choqueren of te doen walgen<\/span>","8":"Uitbuiting van kinderen - <span class=\"sub\">Bevat inhoud die kinderen in een of andere vorm uitbuit<\/span>","11":"Wetsovertreding - <span class=\"sub\">Bevat inhoud die niet in overeenstemming is met de wetten in je rechtsgebied<\/span>","13":"Niet functioneel - <span class=\"sub\">Kan niet worden gedownload, opgestart of gespeeld, zelfs op een machine die voldoet aan de minimum systeemeisen.<\/span>"};
	...
	content.append( $J('<div/>', {'class': 'app_report_dialog_intro' } ).text('Je kunt hier eventueel extra relevante informatie toevoegen:') );
	content.append( textArea );

	content.append( $J('<div/>', {'class': 'app_report_dialog_dmca' } ).html('Als je als rechthebbende auteursrechtenschending wilt melden, dien dan een <a href="http://steamcommunity.com/dmca/create/">DMCA-verwijderingsverzoek</a> in.') );

	var dialog = ShowConfirmDialog( 'Dit product rapporteren', content, 'Rapporteren');
	...
	dialog.done( function() {
		...
		if ( eReportTypeSelected )
		{
			$J.post(
				'http://store.steampowered.com/appreport/' + nAppId + '/report/',
				{
					...
					if ( json.success == 29 )
					{
						ShowAlertDialog( 'Fout', 'Je hebt dit product al gerapporteerd!' );
						gReportedApp = true;
						$J( "#ReportAppBtn").addClass( 'btn_active' );
					}
					else
		...
		{
			ShowAlertDialog( 'Fout', 'Je moet aangeven waarom je dit product wilt rapporteren!' );
		}
	});
}
...
{
		var $ModalContent = $J("<div class=\"gotsteamModal\">\r\n\t<div class=\"got_steam_ctn\">\r\n\t<div class=\"got_steam_box\">\r\n\t\t<h1>Heb je Steam al?<\/h1>\r\n\t\t<p>Je moet de <a href=\"http:\/\/store.steampowered.com\/about\/\">Steam-applicatie<\/a> ge\u00efnstalleerd hebben voordat je <strong class=\"gotSteam_AppName\"><\/strong> kunt installeren en spelen. Heb je Steam al ge\u00efnstalleerd op deze computer?<\/p>\r\n\t\t<div class=\"gotsteam_buttons\">\r\n\t\t\t<a class=\"gotSteam_SteamURL btn_blue leftbtn\" href=\"\">\r\n\t\t\t\t<h3>Ja, Steam is ge\u00efnstalleerd<\/h3>\r\n\t\t\t\t<h5 class=\"gotsteam_action\"><\/h5>\r\n\t\t\t<\/a>\r\n\t\t\t<a href=\"http:\/\/store.steampowered.com\/about\/\" class=\"btn_blue\">\r\n\t\t\t\t<h3>Nee, ik moet Steam installeren<\/h3>\r\n\t\t\t\t<h5>Meer informatie over Steam en nu downloaden<\/h5>\r\n\t\t\t<\/a>\r\n\t\t\t<div style=\"clear: left;\"><\/div>\r\n\t\t<\/div>\r\n\t\t<div class=\"got_steam_low_block\">\r\n\t\t\t<div class=\"gotsteam_steam_ico\"><img src=\"http:\/\/store.akamai.steamstatic.com\/public\/images\/v6\/steam_ico.png\" width=\"40\" height=\"40\" border=\"0\" \/><\/div>\r\n\t\t\tSteam is het allereerste gamingplatform voor de pc. Het is gratis en gebruiksvriendelijk. <a href=\"http:\/\/store.steampowered.com\/about\/\">Meer informatie over Steam.<\/a>\r\n\t\t<\/div><\/div>\r\n\t<\/div>\r\n<\/div>");
...
{
	var strTemplate = "<div class=\"recommended_more_info_modal\">\r\n\t\t\t\t\t\t\t\t<p class=\"intro\">Je hebt iets gevonden dat niet lijkt op de dingen die je in het verleden hebt gebruikt. Steam zal over je voorkeuren leren door de spellen die je speelt, de films die je kijkt en de software die je gebruikt op Steam. We zullen die activiteit gebruiken om de aanbevelingen aan te passen aan je smaak.<\/p>\r\n\t\t\t\t\t\t\t\t<h2>Vind je dit artikel leuk?<\/h2>\r\n\t\t\t\t\t\t\t\t<p>Als je dit artikel leuk vindt en het vervolgens koopt en speelt, zullen we er rekening mee houden bij toekomstige aanbevelingen.<\/p>\r\n\t\t\t\t\t\t\t\t<h2>Niet ge\u00efnteresseerd in zulke artikelen?<\/h2>\r\n\t\t\t\t\t\t\t\t<p>Je kunt je winkelvoorkeur ook instellen om Steam te laten weten in welke tags of soorten producten je niet ge\u00efnteresseerd bent. <a href=\"https:\/\/store.steampowered.com\/account\/preferences\/\">Winkelvoorkeuren bezoeken<\/a>.<\/p>\r\n\t\t\t\t\t\t\t<\/div>";
	ShowAlertDialog( "Meer over aanbevelingen", strTemplate);
