...
function InitQueueControls( store_appid, steamworks_appid, next_in_queue_appid, snr )
{
	...
	$IgnoreBtn.click( function() {
		$J.post( 'http://store.steampowered.com/recommended/ignorerecommendation/', {
			...
			if ( data && data.nSaleTaskCompleted ) { NewStickerPackModal( 'Dispense algum jogo' ); } // SummerSale2017
		}).fail( function( jqXHR ) {
	...
	$J('a.dq_settings_link').click( function() {
		CDiscoveryQueue.ShowCustomizeDialog( function( data ) {
			...
			{
				ShowConfirmDialog( 'Personalize a lista de descobrimento', 'Criamos uma nova lista de descobrimento com as suas personalizações. O que deseja fazer?',
					'Começar a explorar a nova lista','Ficar aqui'
				).done( function() {
...
function UserReviewVoteUp( id )
{
	UserReview_Rate( id, true, 'http://store.steampowered.com/',
		function( rgResults ) {
			if ( rgResults.nSaleTaskCompleted ) { NewStickerPackModal( 'Marque uma análise como útil ... ou não');}
...
function BuildReviewHistogram()
{
	...
	$J.get( 'http://store.steampowered.com/appreviewhistogram/' + appid, { l: 'brazilian' }
	).done( function( data ) {
...
function UpdateActiveFilters()
{
	...
	if ( $J( "#purchase_type_steam" ).attr( "checked" ) )
	{
		...
		$J( "#reviews_filter_purchase_type" ).text( 'Compra no Steam' );
	}
	else if ( $J( "#purchase_type_non_steam" ).attr( "checked" ) )
	{
		bAnyActiveFilters = true;
		$J( "#reviews_filter_purchase_type" ).show();
		$J( "#reviews_filter_purchase_type" ).text( 'Não comprado no Steam' );
	}
	else
	{
	...
	if ( $J( "#review_language_mine" ).attr( "checked" ) )
	{
		...
		$J( "#reviews_filter_language" ).text( 'Idiomas preferidos' );
	}
	else
	{
...
{
	var strTemplate = "<div class=\"recommended_more_info_modal\">\r\n\t\t\t\t\t\t\t\t<p class=\"intro\">Voc\u00ea achou algo nada similar com o que tem usado no passado. O Steam aprender\u00e1 as suas prefer\u00eancias atrav\u00e9s dos jogos que joga, filmes que assiste e softwares que usa no Steam. Usaremos esta atividade para adaptar as recomenda\u00e7\u00f5es aos seus gostos.<\/p>\r\n\t\t\t\t\t\t\t\t<h2>Gostou deste produto?<\/h2>\r\n\t\t\t\t\t\t\t\t<p>Caso tenha gostado desse produto, compre-o e use-o, levaremos isso em conta em recomenda\u00e7\u00f5es futuras<\/p>\r\n\t\t\t\t\t\t\t\t<h2>N\u00e3o tem interesse em produtos como este?<\/h2>\r\n\t\t\t\t\t\t\t\t<p>Voc\u00ea tamb\u00e9m pode configurar as prefer\u00eancias da loja para dizer ao Steam quais marcadores ou tipos de produtos nos quais voc\u00ea n\u00e3o est\u00e1 interessado. <a href=\"https:\/\/store.steampowered.com\/account\/preferences\/\">Acesse as prefer\u00eancias da loja<\/a>.<\/p>\r\n\t\t\t\t\t\t\t<\/div>";
