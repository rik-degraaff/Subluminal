...
function InitAppTagModal( appid, rgAppTags, rgUserTags, strTagLinkSNR, strYourTagSNR, bShowBanOption )
{
	...
	var fnSetCheckboxTooltip = function( $Checkbox )
	{
		if ( $Checkbox.hasClass('checked') )
			$Checkbox.data('store-tooltip', 'Du har anvendt dette tag. Klik for at fjerne det.' );
		else
			$Checkbox.data('store-tooltip', 'Klik her for at anvende dette tag.' );
	};

	var fnSetReportTooltip = function( $ReportFlag )
	{
		if ( $ReportFlag.hasClass('reported') )
			$ReportFlag.data('store-tooltip', 'Du har rapporteret dette tag som værende forkert anvendt på dette produkt. Klik igen for at fjerne rapporteringen.' );
		else
			$ReportFlag.data('store-tooltip', 'Klik for at rapportere dette tag som værende forkert anvendt på dette produkt.' );
	};
	...
	var fnMakeTag = function( tagid, tag, checked, bPopular, bReported, bBrowsable )
	{
		...
		if ( bPopular )
		{
			...
			if ( $YourTags.length )
			{
				...
				if ( bShowBanOption )
				{
					...
					$Ban.data('store-tooltip', 'Klik her for at fjerne dette tag fra dette produkt. Denne valgmulighed er begrænset til udviklere og Steam-personale.' );
					BindStoreTooltip( $Ban );
					$Tag.append( $Ban );

					$Ban.click( function() {
						ShowConfirmDialog( 'Udeluk tag', 'Er du sikker på, du vil fjerne tagget "%s" fra dette produkt?'.replace( /%s/, tag )).done( function()
						{
		...
		else
		{
			...
			$Link.data('store-tooltip', 'Ikke nok emner mærket med "%s" endnu'.replace( /%s/, $Link.html() ) );
	...
	{
		var $Tag = $J('<div/>', {'class': 'app_tag previous_tag', 'data-tagid': tagid, 'data-store-tooltip': 'Klik her for at anvende dette tag.' }).text( tag ).click( function() { fnApplyTag( tag ) } );
	...
	var fnBuildTagDisplay = function( Modal )
	{
		...
		if ( rgYourPopularTags === null )
		{
			...
			if ( !bYourPopularTagsRequested )
			{
				...
				$J.get( 'http://store.steampowered.com/tagdata/populartags/danish').done( function ( data ) {
		...
		else
		{
			...
			{
				$YourTags.append( $J('<div/>', {'class': 'app_tag_control no_tags_yet'}).text('Ingen tags indtastet endnu' ) );
			}
		...
		if ( rgUserTags.length )
		{
			...
			for( var i = 0; i < rgUserTags.length; i++ )
			{
				if ( !rgUserTags[i].is_reported &&  !rgUserTags[i].browseable )
				{
					...
					$AppTag.data('store-tooltip', 'Ikke nok emner mærket med "%s" endnu'.replace( /%s/, $AppTag.html() ) );
	...
	var fnApplyTag = function( tag, withdraw, bPopularClick, eReportType, unTagID )
	{
		...
		$J.post( 'http://store.steampowered.com/tagdata/tagapp', rgParams ).done( function( data ) {
			...
			{
				ShowAlertDialog( 'Rapporter et tag', 'Tak for din rapportering. Tags, der når et vist antal rapporteringer, fjernes fra produktet.');
			}
			...
			ShowAlertDialog( 'Vis og rediger tags for dette produkt', 'There was a problem adding your tag.' );
		});
	};
	...
	window.ShowAppTagModal = function( appid )
	{
		...
		$AppTagModal.show();
		var Modal = ShowAlertDialog( 'Vis og rediger tags for dette produkt', $AppTagModal, 'Luk' );
	...
	var fnDisplayTag = function( unTagID, strTagName ) {
		if ( !rgTagResults[unTagID] )
		{
			...
			$J.get( 'http://store.steampowered.com/tagdata/gettaggames/' + strTagLanguage + '/' + unTagID + '/', rgParams ).done( function ( html ) {
				...
				$GamesElement.find( '.browse_tag_loading').text( 'Beklager, der er sket en fejl med at indlæse emner. Prøv venligst igen senere.' );
			});
...
function InitBannedTagModal( appid, $BanModal )
{
	...
	window.ShowAppTagBanModal = function() {
		$BanModal.show();
		ShowAlertDialog( 'Udelukkede tags', $BanModal).always( function() {
			if ( bBansChanged )
				window.location.reload();
		} );
		...
		$BanModal.find( '.app_tag_report').click( function() {
			...
			ShowConfirmDialog( 'Fjern udelukkelse af tag',
				'Er du sikker på, at du vil fjerne udelukkelsen af "%s"-tagget?'.replace( /%s/, tag ),
				'Fjern udelukkelse af tag')
				.done( function() {
					var rgParams = {
						appid: appid,
						sessionid: g_sessionID,
					...
					$J.post( 'http://store.steampowered.com/tagdata/tagapp', rgParams ).done( function( data ) {
						...
						ShowAlertDialog( 'Fjern udelukkelse af tag', 'Der opstod et problem med at udelukke eller fjerne udelukkelsen af dette tag. Prøv venligst igen senere.' );
					});
					bBansChanged = true;
				});
		});
