...
GDynamicStore = {
	...
	CapsuleSettingsMenu: function( elSource )
	{
		...
		if( strAppIDs )
		{
			...
			var $elPreferences = $J ( '<a/>' ).attr('href', 'https://store.steampowered.com/account/preferences' ).text ( "Preferenze" ).addClass( 'option' );
	...
	UpdateDynamicBundleElements: function( Bundle, $El )
	{
		...
		if ( !Bundle.m_bIsCommercial && ( !Bundle.m_rgBundleItems.length || ( Bundle.m_bMustPurchaseAsSet && Bundle.m_cUserItemsInBundle < Bundle.m_cTotalItemsInBundle ) ) )
		{
			...
			var strTooltip = 'Questo pacchetto non è disponibile per l\'acquisto sul tuo account poiché già possiedi tutti gli articoli inclusi.';

			if ( Bundle.m_bMustPurchaseAsSet && !Bundle.m_bRestrictGifting )
			{
				strTooltip = 'L\'offerta è valida solo per l\'acquisto di tutti i %s articoli insieme. Puoi comunque acquistare il pacchetto per regalarlo ad un amico.'.replace(/%s/, Bundle.m_cTotalItemsInBundle);
		...
		if ( $El.is( '.package_totals_area' ) && !Bundle.m_bMustPurchaseAsSet )
		{
			...
			else
			{
				...
				{
					$El.find('.bundle_final_package_price_desc' ).text( 'Prezzo individuale dei %s oggetti che ancora ti mancano:'.replace( '%s', Bundle.m_cUserItemsInBundle ) );
				}
	...
	BuildBundleDescription: function( Bundle, $Description )
	{
		...
		else if ( Bundle.m_cUserItemsInBundle < Bundle.m_cTotalItemsInBundle )
		{
			...
			$Description.html( '<div>%1$s oggetti su %2$s di questo pacchetto sono già nella tua libreria.</div>'.replace( '%1$s', Bundle.m_cTotalItemsInBundle - Bundle.m_cUserItemsInBundle ).replace( '%2$s', Bundle.m_cTotalItemsInBundle ) );
			$Description.append( '<div>Acquista questo pacchetto e risparmia il %1$s%% per i %2$s oggetti che ancora ti mancano!</div>'.replace( '%1$s', Bundle.m_nDiscountPct ).replace( '%2$s', Bundle.m_cUserItemsInBundle ).replace( '%%', '%' ) );
		...
		{
			$Description.html( '<div>Acquista questo pacchetto e risparmia il %1$s%% su tutti i %2$s oggetti!</div>'.replace( '%1$s', Bundle.m_nDiscountPct ).replace( '%2$s', Bundle.m_cTotalItemsInBundle ).replace( '%%', '%' ) );
		}
	...
	PopulateRecommendedTagList: function()
	{
		...
		for( var i = 0; i < GDynamicStore.s_rgRecommendedTags.length && i < 5; i++ )
		{
			var tag = GDynamicStore.s_rgRecommendedTags[i];
							var $Link = $J('<a/>', {'class': 'popup_menu_item', 'href': 'http://store.steampowered.com/tags/it/' + encodeURIComponent( tag.name ) });
						$Link.text( tag.name );
			$Element.append( $Link );
		}
	},
