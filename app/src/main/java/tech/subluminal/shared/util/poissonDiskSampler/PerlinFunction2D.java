...
public class PerlinFunction2D extends RealFunction2D {
  ...
  public PerlinFunction2D(int width, int height, int octaves, double persistence,
      boolean normalize) {
    ...
    for (int i = 0; i < octaves; i++) {
      textures[i] = new SmoothNoise2D(width + samplingPeriodMax + 1,
          height + samplingPeriodMax + 1);
    }
  }
